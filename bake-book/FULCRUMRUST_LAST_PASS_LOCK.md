...
fulcrumRust owns the port docs: `docs/STEAL_MAP.md`, `docs/AXIS.md`, `docs/TERRAIN.md`, `docs/MILESTONE_01_PLAYABLE.md`, `docs/CHANNELS.md`, plus `STAMPS.md` / `GROWTH_POC.md` / `PEDON.md` / `PROBES.md` / `SPAWNS.md` / `PVP_DIAL_SHEET.md` / `LANDMARK_RIDE.md` / `BIPED_3P_DIAL_SHEET.md` / `WOUND_FEEL_DIAL_SHEET.md` / `STAMP_PBR_DIAL_SHEET.md` / `DEATH_DIAL_SHEET.md` / `CANTED_OPTIC_DIAL_SHEET.md` / `HOLO_MYCELIUM_DIAL_SHEET.md` / `HEAT_WARP_DIAL_SHEET.md` / `MP_WEAPON_DIAL_SHEET.md` / `SOFT_RIM_DIAL_SHEET.md` / `BALLISTICS_A_DIAL_SHEET.md` / `OPTIONS_MOUSE_SENS_DIAL_SHEET.md` / `AIM_TUNE_LIVE_SAVE.md` / `OPTIONS_GRAPHICS_DIAL_SHEET.md` / `PREFERRED_HAND_A_DIAL_SHEET.md` / `OPTIONS_DEFAULT_HIP_DIAL_SHEET.md` / `VOIDSPORE_DIAL_SHEET.md` / `OPTIONS_TEXTURE_DIAL_SHEET.md`. Patch A feedback checkpoint is repo-root `patch notes A.txt` (#72) — `X` / `~` / `*` / `·` ledger + seat owners; not a replacement for STEAL_MAP or MILESTONE. House-docs bake-book is the **dial shelf** — readable enough for seats without reading every `.rs`. Steal from the shelf + steal map. Not chat scroll. Vector mag dump (Range Tech muzzle / FX — **live steal landed #89**) → `VECTOR_MAG_DUMP.md`. Heat cards (Range Tech **live #129** CE tip 0.2.8 on Hypha #66; #71 blend DNA) → `../heat-card-dial-sheet.md`. Pixellation / floor-warp (Hypha post dial **landed #90**; Augury aesthetic only) → this sheet Menus / Post / Graphics dump + sibling **Pixellation / floor-warp**. **1P viewmodel ≠ 3P biped gun** (MP honesty — **partial shipped #91** peer hip gun stub + **#153** held 3P kit clone on PeerBody lean; hands / gear / Mixamo still open) → this sheet + `PEEK_FINDINGS.md`. **Scope glass** (when LPVO — holding / **not shipped**) + **Evan asset-ask path** → this sheet + `PEEK_FINDINGS.md`. **CE Home debugger** (Augury **landed #110**; Home LOGS COPY / tracker toggles **landed #121**; Home occlusion + 3D probes + COLL/PERF/SPWN **landed #125**; Home binds lock + spawn + denser STREAM **landed #128**; End stays AIM TUNE) → this sheet + `PEEK_FINDINGS.md`. **Stream hitch amortize** (Hypha **landed #108**; worker STREAM extract+paint deepen **landed #123**; hitch *visibility* stays Augury #110 / #121 / **#128** denser STREAM tags) → this sheet + `PEEK_FINDINGS.md` Closed by #108 / Closed by #123 + `TERRAIN_NORTHSTAR.md`. **Play STREAM 11×11 + warm hold** (Hypha **landed #137**; `STREAM_RINGS` **5** · prefetch=5+heading · shipped hold=2/12; live hold **later landed #151** 4/16; #108/#123 hitch layers stay; dial sheet fulcrumRust `docs/TERRAIN.md`) → this sheet + `PEEK_FINDINGS.md` Closed by #137 / Closed by #151 + `TERRAIN_NORTHSTAR.md`. **4× world + pend coalesce** (Hypha **landed #142**; 37×37 / 592 m / 350 464 m² · rings 18 · STREAM held 11×11 · `coalesce=2/2` · rim **288 m**; #81 stays the prior 8× / 19×19 fact; dial sheet fulcrumRust `docs/TERRAIN.md` / `docs/SPAWNS.md`) → this sheet + `PEEK_FINDINGS.md` Closed by #142 + `TERRAIN_NORTHSTAR.md`. **STREAM past extract / desert / soft rim** (Hypha **landed #151**; fine window stays #137 **11×11** · live hold **4/16** was 2/12 · cheap LOD-3 `visible=5/19` · flat desert `desert=8` / 128 m · soft rim ~422 m `hook=augury` · `StreamPolicy` window/wide/resident; dial sheet fulcrumRust `docs/TERRAIN.md`) → this sheet + `PEEK_FINDINGS.md` Closed by #151 + `TERRAIN_NORTHSTAR.md`. **Transvoxel UV consume** (Hypha **landed #112**; Lab-Rat owns the #101 sheet) → this sheet + `PEEK_FINDINGS.md` Closed by #112. **Subtract crawl pad network** (Lab-Rat **landed #114**; shallow enterable network, not a tunnel sim) → this sheet + `PEEK_FINDINGS.md` Closed by #114 + `STAMP_FEEL_LOCK.md`. **Probe consume** (Lab-Rat **landed #127**; `FULCRUM_PROBES` / cwd `fulcrum.probes` → `probes.rs` → `apply_to_layers` at bake; not a second mesher) → this sheet + `PEEK_FINDINGS.md` Closed by #127 + fulcrumRust `docs/PROBES.md`. **Sandbox pedon** (Lab-Rat **landed #130**; off-stream CHANNELS leftover slab; tap **F** rebake; STREAM stays Hypha #123; pad crawl stays #114) → this sheet + `PEEK_FINDINGS.md` Closed by #130 + fulcrumRust `docs/PEDON.md`. **Extract player spawn loci** (Lab-Rat **#132**) → this sheet + `PEEK_FINDINGS.md` Closed by #132 + fulcrumRust `docs/SPAWNS.md`. **Dirt Hit pool + world FX mono fold** (Range Tech **landed #134**; `Slot::HIT_POOL` distant dirt trio; `DecodeFold::WorldMono` on `Bus::Fx`; Music/Voice keep stereo; Augury still owns spatial/reverb) → this sheet + `PEEK_FINDINGS.md` Closed by #134 + `EXTRACTION_AUDIO_LOCK.md`. **Kit handling / ergo / MOA** (Range Tech **landed #113**; `HandlingStats` / `FeelSheet::handling`; ADS `blend_speed` **6.4** × kit ergo; `hold_spring` **7.0** stays sibling) → this sheet + `PEEK_FINDINGS.md` Closed by #113. **AIM TUNE PX travel** (Range Tech **landed #138**; End WEAPON PX **±0.50** via `shoulder_x_max`; leftover +0.226 is not an End cap; H still `shoulder_x_min` only) → this sheet + `PEEK_FINDINGS.md` Closed by #138 + fulcrumRust `docs/AIM_TUNE_X_DIAL_SHEET.md`. **AIM TUNE PX hideout wall** (Range Tech **landed #159**; `TUNE_POS_X_ABS` **0.50** floor · End WEAPON home nudge snaps `hold_blend` · ATTACH same box; leftover +0.226 ≠ End box; hideout LIVE **+0.2258** Fine +X leaves; PreferredHand live in AIM TUNE dialogue is follow-up) → this sheet + `PEEK_FINDINGS.md` Closed by #159 + fulcrumRust `docs/AIM_TUNE_X_DIAL_SHEET.md`. **World/sim leftover** (Beabim **landed #119**; KIND_SHOT / KIND_LOCUS / KIND_BODY; peer shot muzzle = biped hip stub; joiner plants soles only) → this sheet + `PEEK_FINDINGS.md` Closed by #119. **Hatch/elevator glasses UX** (Augury **landed #115**; hatch toggle + shaft ride **X**; timed surface kill still `~`) → this sheet + `PEEK_FINDINGS.md` Closed by #115 + `AESTHETIC_DIEGETIC_LOCK.md`. **Door / extract cancel chrome** (Augury **landed #120**; `GATE_SECS` **2.20** / `COOL_SECS` **0.55**; walk-away cancels; timed surface kill still `~`) → this sheet + `PEEK_FINDINGS.md` Closed by #120 + `AESTHETIC_DIEGETIC_LOCK.md` + fulcrumRust `docs/GATE_DIAL_SHEET.md`. **No-pause live sim + KIND_RAID shared instance** (Beabim **landed #122**; mute local pawn only; leftover = `GATE_SECS` **2.20**; honors `Cancelled`; glasses stay Augury) → this sheet + `PEEK_FINDINGS.md` Closed by #122 + fulcrumRust `docs/GATE_DIAL_SHEET.md` KIND_RAID row. **PVP leftover** (Beabim **landed #133**; KIND_PVP default **off** · hide names when on · KIND_BRASS host-relay · eye **1.60** / head **1.62** · rim respawn **1.20 s**) → this sheet + `PEEK_FINDINGS.md` Closed by #133 + fulcrumRust `docs/PVP_DIAL_SHEET.md`. **HOST session board / no-127 invite** (Beabim **landed #139**; title HOST PVP radio pre-enter · settings lock · LAN bind prefer — no 127 seed) → this sheet + `PEEK_FINDINGS.md` Closed by #139 + fulcrumRust `docs/PVP_DIAL_SHEET.md`. **PreferredHand + new-profile onboard** (Hypha **landed #116**; Right default · NEW PROFILE gate · `project.json` permanent vs live · death clears live · extract→stash stub · H seats Range crossover, no mesh flip) → this sheet + `PEEK_FINDINGS.md` Closed by #116. **Landmark AABB ride** (Hypha **landed #136**; `RIDE_STEP` **0.50** · `RIDE_SKIN` **0.06** · `SUPPORT_STEP` **0.25** unchanged; `ride_surface_y` = max(heightfield, AABB top); walls-as-floor only in the ride band; Locus hurtboxes stay walls; peers `plant_simple_root_on` same column; #88 FOLLOW/DEADZONE stay) → this sheet + `PEEK_FINDINGS.md` Closed by #136 + fulcrumRust `docs/LANDMARK_RIDE.md`. **3P biped / PeerBody** (Hypha **landed #131**; `EYE_Y` **1.60** · `HEAD_H` **1.62** · `HEAD_HALF_H` **0.11** · XZ **0** left-offset killed; crouch squat **0.62** · ragdoll flop **0.55 s** · pose lean unused byte 2; lean match **later landed #145** torso peek **0.5 m** · hinge **0.55** · feet planted) → this sheet + `PEEK_FINDINGS.md` Closed by #131 + Closed by #145 + fulcrumRust `docs/BIPED_3P_DIAL_SHEET.md`. **3P torso lean match** (Hypha **landed #145**; height-weighted torso peek `LEAN_LATERAL` **0.5** · `LEAN_HINGE_Y` **0.55** · `LEAN_ANGLE` **0.52** · `LEAN_SMOOTH` **9.5** · pelvis/spine/chest; Range `lean_offset` read only) → this sheet + `PEEK_FINDINGS.md` Closed by #145 + fulcrumRust `docs/BIPED_3P_DIAL_SHEET.md`. **PVP honesty** (Beabim **landed #141**; `PeerBody::hurtboxes()` leftover · hurt skin **0.06** + swept AABB + **4** substeps · KIND_PVP Hit/Sync/WELCOME remaining HP/AR · unique rim pads host **0** / rotate / occupancy **16 m**) → this sheet + `PEEK_FINDINGS.md` Closed by #141 + fulcrumRust `docs/PVP_DIAL_SHEET.md`. **PVP leftover ray** (Beabim **landed #147**; `LEFTOVER_HIT_M` / `first_leftover_hit` **500 m** was 80 · flat `SMG_PELLET` **14** · Locus yard keeps own 80 · Range falloff parked · eye PeerBody + hurt skin/sweep held) → this sheet + `PEEK_FINDINGS.md` Closed by #147 + fulcrumRust `docs/PVP_DIAL_SHEET.md`. **Ballistics A leftover/net** (Beabim **landed #160**; `KIND_SHOT.dir` HOST launch loft · default `ballistic_A` · hitscan stash via `FULCRUM_LEFTOVER` / `leftover_hit` / `Session::set_leftover_hit_model` · eye **1.60** · **500 m** / `SMG_PELLET` **14** held · arc chord **0.17 m**) → this sheet + `PEEK_FINDINGS.md` Closed by #160 + house `BALLISTICS_SPINE.md` + fulcrumRust `docs/BALLISTICS_A_DIAL_SHEET.md`. **Ballistics A loft DNA + AIM TUNE stash** (Range Tech **landed #161**; 1P `muzzle_and_launch` + `hip_honest_dir` + MOA is `KIND_SHOT.dir` · tracer streak steals `feel::step_ballistic` · End **MODEL** writes `leftover_hit` · TELE/PERF `LEFTOVER` reads it · no `ballistic_model`; Powder **B later landed #166**) → this sheet + `PEEK_FINDINGS.md` Closed by #161 + Closed by #166 + house `BALLISTICS_SPINE.md` + fulcrumRust `docs/BALLISTICS_A_DIAL_SHEET.md`. **Powder B slightly hot service** (Range Tech **landed #166**; SR-25 muzzle **785 → 810** steal M24 · ½v² 616225 → 656100 **+6.47%** · gravity **9.8** / zero **100 m** / flat pellet **14** held · vacuum drop @ 420 m **1.403 → 1.317** · @ 500 m **1.988 → 1.867** · M24 already 810 · MP9-Z 300 / 14.0 held · `PowderKind` **B** tip · **A** restoreable · **C** arcade parked `rifle_muzzle_speed() = None` · leftover_hit stash untouched) → this sheet + `PEEK_FINDINGS.md` Closed by #166 + house `BALLISTICS_SPINE.md` + fulcrumRust `docs/BALLISTICS_A_DIAL_SHEET.md`. **Options mouse V/H + hip/ADS** (Range Tech **landed #163**; Controls **MOUSE H** / **MOUSE V** / **HIP** / **ADS** on `LOOK_MUL` **0.25–2.00** / step **0.05** / default **1.00**; persist `look_mul_h` / `look_mul_v` / `hip_look_mul` / `ads_look_mul`; feel ADS **0.86** / blend **6.4** stay DNA; Options READY HIP **#165** already landed (sibling). Powder **B later landed #166**) → this sheet + `PEEK_FINDINGS.md` Closed by #163 + house `OPTIONS_SHELF.md` + house `OPTIONS_MOUSE_SENS.md` + fulcrumRust `docs/OPTIONS_MOUSE_SENS_DIAL_SHEET.md`. **AIM TUNE LIVE per-kit persist** (Range Tech **landed #167** + two-exe **#169**; `aim_live` + `aim_tune.*` unchanged; per-kit pull must not clobber other guns with authored defaults; apply snaps `hold_blend`; hideout+raid share one `project.json`; absolute `FULCRUM_SETTINGS` / cwd path · flush absorb · `project.json.lock`; two-instance is **not** a soft follow) → this sheet + `PEEK_FINDINGS.md` Closed by #167 + Closed by #169 + house `AIM_TUNE_LIVE_SAVE.md` + fulcrumRust `docs/AIM_TUNE_LIVE_SAVE.md`. **Options Graphics** (Hypha **landed #164**; RES **NATIVE** + Evan **3440×1440** + Steam survey **2560×1600 / 1920×1200 / 1366×768** · FOV hip base **90** · AA **FXAA/OFF** · AA STR live mix **0.5** · AO **OFF/LOW/HIGH** · POST **OFF/LOW/HIGH**; pane FOV 70–110 / step 1 is chrome — **not** tip-locked) → this sheet + `PEEK_FINDINGS.md` Closed by #164 + house `OPTIONS_SHELF.md` + house `OPTIONS_GRAPHICS.md` + fulcrumRust `docs/OPTIONS_GRAPHICS_DIAL_SHEET.md`. **Options TEXTURE + MIPMAP** (Lab-Rat **landed #172**; **TEXTURE** default **HIGH** LOW 16² · MED 64² · HIGH 256² · ULTRA 256² · **MIPMAP** default **MED** LOW +1 · MED identity · HIGH −1 · ULTRA −2; persist `texture_quality` / `mipmap_quality`; #152 crawl lock held; chunk metres **16**) → this sheet + `PEEK_FINDINGS.md` Closed by #172 + house `OPTIONS_SHELF.md` + house `OPTIONS_TEXTURE.md` + fulcrumRust `docs/OPTIONS_TEXTURE_DIAL_SHEET.md`. **PreferredHand A** (Range Tech **landed #168**; **HAND R/L** from live hold X · one right bank + sagittal **−X/−yaw/−roll** · AIM TUNE **HAND · kit · pose** · crosshair hides `ads_factor` **> 0.5**; Powder **B later landed #166**) → this sheet + `PEEK_FINDINGS.md` Closed by #168 + house `HOLD_POSE_SPINE.md` + house `PREFERRED_HAND_A.md` + fulcrumRust `docs/PREFERRED_HAND_A_DIAL_SHEET.md`. **Wound feel / 1P screen-react** (Range Tech **landed #143**; suppress-near soft short blur · armour jostle + soft blur no red · HP stronger jostle + blur + red fade · envelope `--===--------` · never full-strength blur · 1P local only · Hypha `post.wound` [blur, red] · hooks Beabim KIND_PVP Hit not Sync · Death/Slain **later landed #146**) → this sheet + `PEEK_FINDINGS.md` Closed by #143 + fulcrumRust `docs/WOUND_FEEL_DIAL_SHEET.md`. **Death / Slain chrome** (Augury **landed #146** + **#162**; PvE bleed-out envelope Red `DEATH_RED_SECS` **0.55** wash 0.78 · FadeBlack **0.70** · Beat **1.00** · Slain `SLAIN_FADE_SECS` **0.78** plate `assets/images/ui/deathscreens/slain.jpg` · Prompt **0.60** PRESS SPACE after `SLAIN_INTERRUPT_SECS` **5.0** → `claim_place_spawn` / #141 pads; hit leftover `HIT_GLITCH` **0.18** · `DOWN_BAR` full black · play HUD strip · hit flash 0.18/0.20 · downed wash 0.48 · `BAR_CATCH` **6.5** · `GLITCH_PROMPT_FLOOR` **0.12**; PVP down does not play Slain; Beabim 1.20s leftover / #153 hub protect untouched) → this sheet + `PEEK_FINDINGS.md` Closed by #146 + #162 + fulcrumRust `docs/DEATH_DIAL_SHEET.md`. **Canted optic silhouette + AIM TUNE ATTACH** (Range Tech **landed #150**; 1P only; magnified ACOG/SCOPE draws 45° offset holo silhouette; iron/holo solo; ATTACH cycle OPTIC→CANTED→CAN; live-save `attachments.canted`; rail roll **−0.785**; MP9-Z ACOG only · SR-25 ACOG/SCOPE · M24 SCOPE only; `ads_cant` / PreferredHand / 3P / STREAM / net / Slain left alone) → this sheet + `PEEK_FINDINGS.md` Closed by #150 + fulcrumRust `docs/CANTED_OPTIC_DIAL_SHEET.md`. **Stamp / building / terrain PBR polish** (Lab-Rat **landed #144**; rocks 3-lobe shade/face/chip · buildings Concrete grade + face UVs · default `pbr=vendor` 256² COL · five thumbs + concrete NRM · tighter tiles / grit on building faces; texture+UV only; chunk **16 m** held; Hypha STREAM/geo untouched) → this sheet + `PEEK_FINDINGS.md` Closed by #144 + fulcrumRust `docs/STAMP_PBR_DIAL_SHEET.md`. **Grit/COL LOD luma crawl** (Lab-Rat **landed #152**; `STABLE_LOCUS_RES` **16** · `MATERIAL_HOLD_M` **2.5** · `LUMA_GRAIN` ±**0.08**; height/DISP stay bilinear; chunk **16** held; off STREAM) → this sheet + `PEEK_FINDINGS.md` Closed by #152 + fulcrumRust `docs/STAMP_PBR_DIAL_SHEET.md` (LOD luma crawl row) + `docs/CHANNELS.md` + `docs/TERRAIN.md`. **HOLO_MYCELIUM hideout leftover** (Lab-Rat **landed #154**; matrix **12×8×12** · sphere **0.010 / 0.022** m · tip purple `[0.68, 0.20, 0.86]` · body blue `[0.16, 0.42, 0.92]` · scanline **0.55** Hz · update **10** Hz · brightness **0.22 → 0.96** × scan **0.62** · `FULCRUM_HOLO=0` / `off` kills) → this sheet + `PEEK_FINDINGS.md` Closed by #154 + fulcrumRust `docs/HOLO_MYCELIUM_DIAL_SHEET.md`. **Locus Voidspore yard leftover** (Lab-Rat **landed #171**; matrix **10×4×10** · max live **48** · max tips **8** · update **10** Hz · tip purple `[0.68, 0.20, 0.86]` · origin XZ **−3.55, 2.35** · `FULCRUM_VOIDSPORE=0` / `off` kills) → this sheet + `PEEK_FINDINGS.md` Closed by #171 + house `VOIDSPORE.md` + fulcrumRust `docs/VOIDSPORE_DIAL_SHEET.md`. **Desert skirt vendor sand COL** (Lab-Rat **landed #157**; vendor `pbr_sand_col` on Hypha #151 skirt · tile **2.8 m** · skirt quads **8→4 m** · far `lod_mips` grit wear · no rock lobes; off STREAM / RimHook) → this sheet + `PEEK_FINDINGS.md` Closed by #157 + fulcrumRust `docs/STAMP_PBR_DIAL_SHEET.md` / `docs/TERRAIN.md` / `docs/STAMPS.md`. **Visible barrel heat warp** (Range Tech **landed #155**; `haze_strength` **0** off / **0.01** CE enable floor / **0.11** stolen max; post `visual × lattice × 1.35` at the floor · × **1.20** at 0.11; shader gate **0.001**; UV scale **0.010**; radius **0.40 / 0.08**; `scale_x` / `scale_y` **0.28 / 0.86**; Options **WARP** is Hypha pixellation) → this sheet + `HEAT_WARP_DIAL_SHEET.md` + `PEEK_FINDINGS.md` Closed by #155 + fulcrumRust `docs/HEAT_WARP_DIAL_SHEET.md`. **MP weapon honesty + hideout hub** (Beabim **landed #153**; `KIND_LOOT_WORLD` **15** · mag/reserves/attach persist no magic · held 3P kit on PeerBody lean · hideout PeerBodies · hub no-loot downs · `HUB_PROTECT_SECS` **15**) → this sheet + `PEEK_FINDINGS.md` Closed by #153 + fulcrumRust `docs/MP_WEAPON_DIAL_SHEET.md` + house `HIDEOUT_HUB.md` (metres still pending). **Soft rim leftover** (Augury **landed #156**; thin hold **#158** spring-stop `approaching && t >= 0.995` stays BLOCK / `CARRIER  LOST` / glitch **0.36** + crackle — Hypha `blocked` is crossing-frame only; glasses `SIGNAL  THINS` / `CARRIER  LOST` + death-DNA glitch + FX `Slot::Crackle`; Hypha `RimHook` physics read-only `RIM_SOFT_M` **24** / world rim ~**422**; approach **0.08** → peak **0.28** · block **0.36** · chroma **0.012**; no Slain plate) → this sheet + `PEEK_FINDINGS.md` Closed by #156 + fulcrumRust `docs/SOFT_RIM_DIAL_SHEET.md`. **Options READY HIP + two-pose play** (Range Tech **landed #165**; Gameplay **CHEST** tip default / **LOW HIP**; persist `default_hip`; in-play **U** is chosen hip ↔ canted only; authored `hip` Y **−0.2128** / `hip_low` Y **−0.3528**; shotgun Low-later hook only — not shipped; Controls mouse V/H + hip/ADS stay planted **#163** — do not restamp those rows; AIM TUNE LIVE **#167** already landed) → this sheet + `PEEK_FINDINGS.md` Closed by #165 + house `OPTIONS_SHELF.md` + fulcrumRust `docs/OPTIONS_DEFAULT_HIP_DIAL_SHEET.md`. Overnight cooks steal from the shelf.
## Day-one kit
...
- **G** cycles MP9-Z → SR-25 → M24; **4 / 5 / 6** seat directly; **U** is two-pose play **#165** — Options READY HIP (Chest tip default / Low) ↔ canted only (no Chest → LowHip → Canted ladder; glasses **LOW HIP** when Low is the seat · **CANT 45** on Canted · **CANT ADS** in `ads_cant`); authored `hip` / `hip_low` stay **#98 / #99**; **1 / 2 / 3** stay Lab-Rat curl
...
2. Small **interior hideout** (drawers, tables, lights, pickups, door) — geometry mostly authored. Cheap looping hologram leftover **landed #154** (left desk; blue leftover pixels + purple tips + scanline; `FULCRUM_HOLO=0` kills; table AABB placeholder — no T·H·L invent)
...
- **LOD-tied grit / material mips** (Hypha #60): `lod_mips.rs` BC4-class 8-bit height/rough on Transvoxel rings — near **256²** (#58 vendor) / mid **64²** / far **16²**; far drops grain hashes; `sample_channels` + `stamp_wear_scale` pick the ring from world XZ; in-repo `grit_*.png` until more grit cooks. Smoke `grit_mips=256/64/16 n=196608 f=768`. **#112** `promote_for_uv` keeps a finer existing pack when `FULCRUM_UV` scale > 1 (scale 2 → mid 64²; scale 4 → near 256²) — never a new mip, never remesh. **#152** grit/COL luma crawl stop: color/wear/COL snap to far **16²** loci (`STABLE_LOCUS_RES` **16**) · `MATERIAL_HOLD_M` **2.5** past the mesh face · `LUMA_GRAIN` ±**0.08**; height/DISP stay bilinear
...
- **Play STREAM 11×11 + warm hold** (Hypha #137): **landed**. Radius + warm-return layer on #108/#123 hitch — not a new map size. `STREAM_RINGS` **5** / **11×11** (was 4 / 9×9; leading edge 80 m). Prefetch **5 m** face + **walk heading** (reverse included); one cheap lod-3 look-ahead past the hot ring. Resident hold GPU halo `HOLD_RINGS` **2** + CPU TTL `RESIDENT_TTL_SECS` **12** (`hold=2/12`) — return walk promotes/republishes, no extract. Flush/smoke still drop (rim spawn-cold honest). Soft LOD pop OK. Cook/hitch held from #123: cook=1/2 · cook_ms 8/16 · defer=worker/paint/gpu. Hole-fill dirty jobs (heading side first) before LOD upgrades. Halo-exit still sets `hold_publish` so extract0 / gpu stay on separate play ticks. TELE MAX ~30 ms apply/queue. Smoke `prefetch=5+heading` `hold=2/12` `defer=worker/paint/gpu`. Home LOGS `STREAM` still keys `resident=` / `cold=` / `pend=` (held leftovers count as resident until TTL). Dial sheet: fulcrumRust `docs/TERRAIN.md`. 4× world + pend coalesce **later landed #142** (STREAM held). Live hold / horizon / desert / soft rim **later landed #151** (`hold=4/16` — do **not** read #137 `hold=2/12` as the live tip). See `PEEK_FINDINGS.md` Closed by #137 / Closed by #142 / Closed by #151 + `TERRAIN_NORTHSTAR.md`
- **4× world + pend coalesce** (Hypha #142): **landed**. Next ~4× area expand on the #81 host — **not** a STREAM radius bump. Extract **37×37 / 592 m / 350 464 m²** (was 19×19 / 304 m / 92 416 m²; linear ~1.95×). World rings **9 → 18**. Spawn stays origin. Play STREAM **held 11×11** (`STREAM_RINGS` 5 / r=5). Prefetch **5+heading** same. Resident hold **2/12** same at ship (live **later landed #151** `hold=4/16`). Cook/hitch **1/2 · 8/16 · defer=worker/paint/gpu** same (#108/#123/#137 layers stay). **NEW:** pend coalesce admit **2** / cap **2** (`coalesce=2/2`) — play truncates heading/hole sort so mid-drain does not admit another leading-edge storm. Home SPIKE is `pendΔ ≥ 2` (Lab-Rat walk dump max=319.4 ms · warn=15 · spike=4 when pend jumped +8/+16/+18). **NEW:** walk-forward GPU concat appends new hole-fills (full rebuild when leftover leaves GPU halo or resident remeshes). Rim pads **144 → 288 m** via `probes::rim_radius_m()` / `GRID_ORIGIN`. Inside `playable_half_m` 295.25. Smoke `extract_m2=350464` `rings=18` `prefetch=5+heading` `hold=2/12` `defer=worker/paint/gpu` `coalesce=2/2` `spawns=rim=8`. Stamp pad stays **7×7**. Lab-Rat CHANNELS / pedon left alone. Range / Beabim net/PVP/HOST / Augury Home chrome left alone. Dial sheet: fulcrumRust `docs/TERRAIN.md` + `docs/SPAWNS.md`. Far-4 / void-past-extract **later landed #151**. See `PEEK_FINDINGS.md` Closed by #142 / Closed by #151 + `TERRAIN_NORTHSTAR.md`
- **STREAM past extract / desert / soft rim** (Hypha #151): **landed**. Horizon / exterior / hold / rim-hook on the #142 37×37 host — **not** a fine STREAM radius bump. Fine window stays #137 **11×11** (`STREAM_RINGS` 5 / r=5). Cheap LOD-3 `visible=5/19` (void → coarse floor). Flat desert `desert=8` / 128 m (heightfield + skirt; sand stamp hook / height only — no rock lobes; Lab-Rat COL later). Hold **2/12 → 4/16**. `StreamPolicy` `window` / `wide` / `resident` (`FULCRUM_STREAM` or Options Graphics **STREAM**; wide = r=18 hold=8/30; resident = whole-extract fine + hold=18/inf). Soft rim at extract+desert (~422 m): damp outward vel; spring back; no clip slap. `RimHook` / `Debugger::note_rim` → `RIM APPROACH|BLOCK|CLEAR hook=augury` — Augury glasses glitch + radio crackle; do **not** invent Slain chrome. Augury chrome BLOCK hold is **#158** on the #156 shelf (spring-stop `approaching && t >= 0.995`; Hypha `blocked` is crossing-frame only). Prefetch/cook/hitch/#123 worker + #142 `coalesce=2/2` held. Extract still 37×37 / 592 m / 350464 m². Stamp pad 7×7. Smoke `hold=4/16` `stream=window` `visible=5/19` `desert=8` `coalesce=2/2` `defer=worker/paint/gpu` `extract_m2=350464`. Dial sheet: fulcrumRust `docs/TERRAIN.md`. See `PEEK_FINDINGS.md` Closed by #151 + `TERRAIN_NORTHSTAR.md`
...
- **Stamp / building / terrain PBR polish** (Lab-Rat #144): **landed**. Texture + UV only — **never** geo / **never** Transvoxel remesh / **never** chunk shrink. Rocks = offset 3-lobe density + 4-box silhouette (shade / face / chip); `Solid.grade = Rock`. Buildings = `Solid.grade = Concrete`; face UVs (walls = vertical plane + NRM luma, lids = XZ). Terrain default **`pbr=vendor`** — in-repo 256² COL thumbs for dirt / sand / rock / concrete / organic + concrete NRM (not 4k). Tiles: concrete **3.2** · rock **3.6** · dirt **3.8** · sand **4.0** · organic **3.2**. Grit **4.6 / 2.6 / 3.4**. COL mix `0.46 + 1.08·sample`. Extra grit on **building faces only**. Chunk metres **16**. Stamp pad **7×7**. Hypha STREAM / 37×37 / hitch / landmark ride untouched. GPU NRM sampler on `fs_world` parked. Grit/COL LOD luma crawl stop **later landed #152**. See `PEEK_FINDINGS.md` Closed by #144 / Closed by #152 + fulcrumRust `docs/STAMP_PBR_DIAL_SHEET.md`
- **Grit / COL LOD luma crawl** (Lab-Rat #152): **landed**. Texture + UV / LOD sample stability only — **never** STREAM / extract / hold / Transvoxel remesh / landmark ride / pend. Root: grit/COL bilinear on promote reshuffled darks at the same world XZ. `STABLE_LOCUS_RES` **16** · `MATERIAL_HOLD_M` **2.5** · `LUMA_GRAIN` ±**0.08**. Height/DISP stay bilinear. Chunk **16** held. Extra grit still on building faces only. Off STREAM. See `PEEK_FINDINGS.md` Closed by #152 + fulcrumRust `docs/STAMP_PBR_DIAL_SHEET.md` (LOD luma crawl row) + `docs/CHANNELS.md` + `docs/TERRAIN.md`
- **Desert skirt vendor sand COL** (Lab-Rat #157): **landed**. Texture + UV / vertex paint only — **never** STREAM / RimHook / remesh / chunk shrink / rock lobes. Hypha #151 left a height-only skirt (`desert=8` / 128 m · `DESERT_FLOOR_Y`). Yard sand tile **4.0** aliased the old **8 m** quads onto one COL texel (bare wash). Vendor `pbr_sand_col` at **2.8 m**; skirt quads **8→4 m**; wear = far `lod_mips` 16² grit (`stamp_wear_scale_lod` + `edge_wear`). Material stays `Sand`. Extra grit still building-faces-only. Smoke `pbr=vendor` `desert=8/col=2.8`. See `PEEK_FINDINGS.md` Closed by #157 + fulcrumRust `docs/STAMP_PBR_DIAL_SHEET.md` / `docs/TERRAIN.md` / `docs/STAMPS.md`
- **HOLO_MYCELIUM hideout leftover** (Lab-Rat #154): **landed**. Cheap looping board-room hologram leftover for the pre-raid hideout hub. **Not** pycelium GPU mesocosm. **Not** Hypha STREAM / extract remesh / chunk metres / #151 sand COL. **Not** Beabim KIND / PeerBody. Matrix **12×8×12** · max live **96** + 1 scanline slab · max tips **10** · update **10** Hz · sphere **0.010 / 0.022** m · tip purple `[0.68, 0.20, 0.86]` · body blue `[0.16, 0.42, 0.92]` · brightness **0.22 → 0.96** × scan **0.62** · scanline **0.55** Hz · cycle **9.0** s or starve · warm **22** steps. Seated on existing hideout **left desk** (table AABB placeholder — no T·H·L invent). `FULCRUM_HOLO=0` / `off` kills. Smoke `holo=gen=`. Peek: Deploy → look left. Hideout upload reuses extract pedon leftover buffers (places never overlap). See `PEEK_FINDINGS.md` Closed by #154 + fulcrumRust `docs/HOLO_MYCELIUM_DIAL_SHEET.md`
- **Locus Voidspore yard leftover** (Lab-Rat #171): **landed**. Cheap looping growth leftover on the extract yard — dark / inked webbing + purple tips. **Not** a fourth named plot. **Not** pycelium GPU mesocosm. **Not** Hypha STREAM / remesh / chunk 16 / RimHook / landmark ride. **Not** Range powder / AIM TUNE / PreferredHand. **Not** Beabim KIND / net. **Not** Augury Slain / glasses. Quiet grit thumbs stay. Matrix **10×4×10** · max live **48** · max tips **8** · max solids **152** · update **10** Hz · cell **0.016 / 0.034** m · pitch **0.085** m · life **36** · density gate **0.12** · lobe **0.42** · tip purple `[0.68, 0.20, 0.86]` · body ink `[0.10, 0.04, 0.12]` / void `[0.16, 0.05, 0.14]` · brightness **0.16 → 0.78** × grit rough · cycle **11.0** s or starve · warm **18** · origin XZ **−3.55, 2.35** · floor `terrain.height_at`. Growth leftover GPU **768 / 192** KiB (was 640 / 160). `FULCRUM_VOIDSPORE=0` / `off` kills. Smoke `voidspore=gen=`. Peek: Deploy → extract, look left. See `PEEK_FINDINGS.md` Closed by #171 + house `VOIDSPORE.md` + fulcrumRust `docs/VOIDSPORE_DIAL_SHEET.md`
...
## Downed / revive
- HP→0 **downs** (prone crawl + thin bleed) — not menu death. Bleed-out ~`BLEED_SECS` **22.0**; extra hits while downed shave `BLEED_HIT_SECS` **6.0**. Clock expiry → `DEAD` + dark bag. Shipped Augury #36; death cam / Slain chrome **later landed #146** + **#162** (`HIT_GLITCH` **0.18** · `DOWN_BAR` full black · play HUD strip · `SLAIN_INTERRUPT_SECS` **5.0`); teammate net / full loot loop still parked.
...
- **X** = prone
- Canted hold + high/low ready from aim-offset. **U** = chosen Options READY HIP ↔ Canted only (**landed #165** — two poses; no Chest → LowHip → Canted ladder). Chest is the tip default (`hip` Y **−0.2128**). Low is the Options other (`hip_low` Y **−0.3528**; **#99** shotgun low-ready; glasses **LOW HIP**). Canted glasses **CANT 45**. Viewmodel eases via `hold_spring` **7.0** (**landed #109**); glasses still snap CHEST / LOW HIP / CANT 45. RMB from Chest/Low = seated optic ads (Low still irons — **#99**). **U** → CANT + RMB → `ads_cant` @ 60° CQC (**landed #100**). hold-**Mouse5** from the default hip → `ads_cant` @ 60° CQC (does not steal U / RMB / melee). Glasses in `ads_cant` **CANT ADS**. Spawn / hideout ready / deploy / respawn / quit-to-menu seat the chosen hip. Shotgun Low-later is a parked hook — not shipped
- **H** = viewmodel **crossover shoulder / left-corner peek** on the existing FoW H bind (travel landed #59; tilt path **landed #84**; RH hip bias + slight straighten **landed #94**; one more body-width + ready-hip Y **landed #98**). Authored hip +X ~**0.24** (right; live **0.2403 / −0.2128 / −0.1833** — ready hold, not chin-weld); `shoulder_cross_x` **−0.281** / `shoulder_cross_y` **0.032**; springs across the chest to a partial left (~**−0.041** X / ~**−0.181** Y, cap `shoulder_x_min` **−0.055**). Leftover +0.226 (`−shoulder_cross_x + shoulder_x_min`) is H dest math, **not** an End +X wall (**#138** PX box `shoulder_x_max` **±0.50**; **#159** `TUNE_POS_X_ABS` **0.50** floor + End home snap; H still floors at `shoulder_x_min` only). ADS keeps **0.32**. Extra left probe `shoulder_viewmodel` **0.12**. Left hold pitch/yaw/roll **0.04 / 0.10 / 0.08** (supersedes #84 chest-cross 0.08/0.32/0.39) on existing ViewmodelDials / ADS cant DNA (`apply_shoulder_crossover` → `PoseOffset` → `pose_basis`). Live hip_low is **#99** shotgun low-ready (MP9-Z **0.2403 / −0.3528 / −0.1513** / pitch **0.145**; glasses **LOW HIP**; LowHip ADS still irons). Live `ads_cant` is **#100** Greyzone CQC (MP9-Z **0.0423 / −0.148 / −0.136** / yaw **0.11**; glasses **CANT 45** / **CANT ADS**). Not a capsule/eye slide, not a mesh mirror, no `scale.x = −1` / mirrored kit boxes. Help remaps off H
- **End** = live aim-offset / attachment tuner (fulcrumRust #97). Thin in-raid panel + glasses `AIM TUNE`. Feel-lab Home remapped like X→Z. Insert = WEAPON → ATTACH → **MODEL** (**#161**) · PageDown = pose / attachment / leftover target (ATTACH **OPTIC → CANTED → CAN** **later landed #150**; MODEL cycles `leftover_hit` `hitscan` / `ballistic_A`) · PageUp = step size · ↑↓ axis · ←→ / num± nudge · Delete = paste-ready JSON (feel-lab `example_smg` object + attachments). Writes the **same** `ViewmodelDials` + `AttachmentOffsets` — not a second pose system. #97 End tuner nudges **#98** ready-hip / H + **#99** hip_low + **#100** `ads_cant`. **AIM TUNE LIVE persist landed #103** (Hypha `project.json` `aim_live` default **true** + `aim_tune.example_smg` / `example_rifle` / `example_sniper`; pose `{x,y,z,rotX,rotY,rotZ}`; attach optic/can/`canted` (**#150**); load on Deploy; each LIVE nudge flushes; partial merge keeps #98/#99/#100; #97 End / Delete dump stay; RECORD stub — no bind). **AIM TUNE LIVE per-kit pull landed #167** (authored-sheet pull must not clobber other guns; apply snaps `hold_blend`; schema unchanged). **AIM TUNE two-exe persist landed #169** (absolute `FULCRUM_SETTINGS` / cwd path · flush absorb undirty · `project.json.lock`; two-instance is **not** a soft follow). **AIM TUNE PX travel landed #138** (`shoulder_x_max` **±0.50**; leftover +0.226 must not cap End +X; H still `shoulder_x_min` only; authored hip **+0.2403** / H dest **~−0.041** / #103 schema / `hold_spring` / ADS blend untouched). **AIM TUNE PX hideout wall landed #159** (`TUNE_POS_X_ABS` **0.50** floor · End WEAPON home nudge snaps `hold_blend` · ATTACH same box; leftover +0.226 ≠ End box; PreferredHand A chrome **landed #168** — AIM TUNE shows **HAND · kit · pose**; chrome **HAND R/L** from live hold X)
...
- **Locked** by fulcrumRust #12 + #51 + #59 + **#66** + **#67** + **#71** + **#76** + **#78** + **#79** + **#84** + **#85** + **#89** + **#94** + **#97** + **#98** + **#99** + **#100** + **#103** + **#109** + **#110** + **#113** + **#115** + **#116** + **#120** + **#121** + **#125** + **#128** + **#129** + **#134** + **#138** + **#143** + **#146** + **#150** + **#155** + **#159** + **#160** + **#161** + **#162** + **#163** + **#164** + **#165** + **#166** + **#167** + **#168** + **#169**: FoW scheme + aim-offset feel with Evan bind overrides above. #51 dizzy-play is the live look / strafe / door. **#59** landed Q/E flip + deepen, CE hop + air hop + land overlay, H viewmodel crossover, heat v77 look (spatial input), tracers-until-impact + FX `hit` (**#134** dirt pool). **#84** landed H tilt path (not a mesh mirror). **#94** landed Patch A RH hip bias first +0.08 + left hold **0.04 / 0.10 / 0.08** (supersedes #84 chest-cross 0.08/0.32/0.39). **#97** landed the live **End** aim-offset / attachment tuner on those same ViewmodelDials + kit_mesh sockets (Home debugger **later landed #110**; glasses `AIM TUNE`). **#103** landed AIM TUNE LIVE persist into Hypha `project.json` (`aim_live` default **true**; partial merge keeps #98/#99/#100; #97 End / Delete dump stay; RECORD stub — no bind). **#167** landed AIM TUNE LIVE per-kit pull (authored-sheet pull must not clobber other guns; apply snaps `hold_blend`; schema unchanged). **#169** landed two-exe persist (absolute `FULCRUM_SETTINGS` / cwd path · flush absorb undirty · `project.json.lock`; two-instance is **not** a soft follow). **#138** landed AIM TUNE PX travel (`shoulder_x_max` **±0.50**; leftover +0.226 must not cap End +X; H still `shoulder_x_min` only). **#159** landed AIM TUNE PX hideout wall (`TUNE_POS_X_ABS` **0.50** floor · End WEAPON home nudge snaps `hold_blend` · ATTACH same box; leftover +0.226 ≠ End box). **#143** landed 1P screen-react / wound feel (suppress-near soft short blur · armour jostle + soft blur no red · HP stronger jostle + blur + red fade · envelope `--===--------` · never full-strength blur · 1P local; Hypha `post.wound` [blur, red]; hooks Beabim KIND_PVP Hit not Sync; Death/Slain **later landed #146**). **#146** landed Augury CE Slain death flow (PvE envelope · Slain plate · PRESS SPACE → `claim_place_spawn` / #141 pads; PVP down does not play Slain). **#150** landed canted optic silhouette + AIM TUNE ATTACH (1P only; ATTACH OPTIC→CANTED→CAN; live-save `attachments.canted`; rail roll **−0.785**; `ads_cant` numbers held). **#155** landed visible hold-J barrel heat warp (`haze_strength` **0** off / **0.01** CE enable floor / **0.11** stolen max; post `visual × lattice × 1.35` at the floor; shader gate **0.001**; tip cards `scale_x/y` **0.28/0.86**; Options **WARP** stays Hypha pixellation). **#110** landed the Augury CE Home debugger (**Home** toggle · LOGS / TELE / CHEAT · hitch WARN/HITCH; glasses `DEBUG`; End stays AIM TUNE). **#125** landed Home occlusion + 3D probes + COLL/PERF/SPWN (**P** look-at · Enter `fulcrum.probes`; End stays AIM TUNE). **#128** locked Home binds (cycle off→tabs→off · arrows/Enter · Ins pages · no WASD/Space) + `name:ground` + `kind:spawn` via SPWN+P + denser WALK/STREAM hang tags. **#98** landed one more body-width + ready-hip Y (MP9-Z hip **0.2403 / −0.2128 / −0.1833**; `shoulder_cross_x` **−0.281** / `shoulder_cross_y` **0.032**; #97 tuner still live). **#99** landed U-cycle `hip_low` shotgun low-ready (MP9-Z **0.2403 / −0.3528 / −0.1513** / pitch **0.145**; glasses **LOW HIP**; LowHip ADS still irons). **#100** landed canted 45° Greyzone CQC ADS on `ads_cant` (MP9-Z **0.0423 / −0.148 / −0.136** / yaw **0.11**; U+RMB / hold-Mouse5 @ 60° CQC; glasses **CANT 45** / **CANT ADS**). **#109** landed U-cycle hold springs (`hold_spring` **7.0**; same exp-approach as ADS / H / sprint; H `shoulder_spring` **8.0** / sprint **6.2** unchanged; glasses still snap; first-U seed-before-cycle; STEAL_MAP pose-ease → **in**; ADS `blend_speed` **6.4** stays its own dial — **#113** scales it by kit ergo). **#113** landed kit handling / ergo / MOA (`HandlingStats` / `FeelSheet::handling`; ADS blend **6.4** × ergo; handling → recoil `1/handling`; hip MOA after `hip_honest_dir`; ADS × **0.22**; `hold_spring` **7.0** stays sibling; STEAL_MAP handling → **in**). **#66** landed the live heat tell as colorless post UV warp (Hypha; lattice = post input only). **#67 Patch A** sits on top of **−/=** zero (#78; was **O** #33) + #76 SIM-only + #59 tracers: kit-tip spawn + hip aim-dir honesty + tip→impact streak clamp (did not fight #66). **#71** landed the Range Tech dump-dial blend on that #66 path (cooking/~ → landed/X; **live HeatDials later locked #129** CE tip 0.2.8). **#78** landed hold-**O** extract intent (Augury EXTRACT elbow card **landed #85**, no popup), **−/=** zero 50/100/200, grounded slide gate (no midair float-slide). **#115** landed hold-**F** hatch OPEN/CLOSED + shaft RIDE/EXIT (hold-O `EXTRACT  OPEN|CLOSED|SHAFT`; no modal popup; timed surface kill still `~`). **#120** landed door / extract cancel chrome (**F** arms `GATE_SECS` **2.20**; OPEN hatch arms extract-out; walk-away cancels; `COOL_SECS` **0.55**; timed surface kill still `~`). **#116** landed PreferredHand + new-profile onboard (Right default · NEW PROFILE gate · `project.json` `profile_onboarded` + `preferred_hand` permanent vs live · death clears live · extract→stash stub · `shoulder_t` 0 = authored RH / 1 = existing left dest — no mesh flip). **#164** landed Options Graphics (RES NATIVE + FOV hip **90** · AA FXAA/OFF · AA STR **0.5** · AO OFF/LOW/HIGH · POST OFF/LOW/HIGH). **#168** landed PreferredHand A (`HAND R/L` from live hold X · sagittal **−X/−yaw/−roll** · AIM TUNE HAND·kit·pose · hair hides `ads_factor` **> 0.5**). **#79** softened the same #59 land overlay (punch **0.028** · duck **0.08** · shake **0.14** gate **13** + inertia sway; hop 12/30/1 **unchanged**) and snapped brass / tracer ends / marks to extract heightfield / wall support (`first_hit` walls-only; AXIS_LOCK +Z unchanged). **#89** landed projectile feel on the same `TracerField` (rect slab + debris + slug/wake + punch/scuff flash + fire-pulse glyphs; visualLength **1.5 / 18**; SIM / HoB unchanged; HeatDials **later locked #129**). No remaining soft overlap on height / wheel. Lean / hop / H / hip-fire muzzle / U-cycle pose ease no longer cooking. **#165** landed Options READY HIP (Chest tip default / Low; persist `default_hip`) + two-pose **U** (chosen hip ↔ canted only — no three-pose ladder; shotgun Low-later hook only). **#166** landed Powder B slightly hot service (SR-25 **785 → 810** steal M24; gravity **9.8** / zero **100 m** held; **A** restoreable; **C** arcade parked). Orange world heat cards are **not** the live path.
...
- **#153** two-instance honesty: host `KIND_LOOT_WORLD` **15** snapshot (HELLO + ~4 Hz) so late join + packet loss recover. Throw vel on `LootEvent` (bytes 44–55). Mag / reserves / optic / can from `KitLoadout::snapshot` at drop — **no magic refill**. Dial sheet: fulcrumRust `docs/MP_WEAPON_DIAL_SHEET.md`
...
- Stream cook **#108** + **#123** + **#137** + **#142**: `COOK_BUDGET_PLAY` **1** / `COOK_BUDGET_LOAD` **2** · cook_ms **8/16** · prefetch **5 m face + heading** · splash-pumped load-in · `defer` **`worker/paint/gpu`** · hold **4/16** (was #137 2/12) · coalesce **2/2** · `visible=5/19` · `desert=8`. Same 11×11 — not a second window. STREAM past extract **#151**. Play hitch thread never `sample_channels`; skip far mask-only remesh when R climbs; return walk republishes held leftovers; mid-drain does not admit another leading-edge storm
...
- Parked: live octree / unconstrained Sync dump · residual soft LOD pop inside a cell · tunnels · runtime carve · Transvoxel rewrite / world sync · NRM/GLOSS GPU. **#114** is a shallow enterable pad network, not those parked cutouts. Amortized recook under budget **shipped #108**. Worker extract+paint deepen **shipped #123**. Play STREAM 11×11 + warm hold **shipped #137**. 4× world + pend coalesce **shipped #142**. STREAM past extract / desert / soft rim **shipped #151**. Peer feet `stream_anchors` **landed #83** (coordinate only). Near LOD raise **shipped #61**. First big-map **landed #81**. Texture mips / compression **shipped #60** on the same **distance rings** (near 256² / mid 64² / far 16²; far softer). In-repo grit mips stay until Lab-Rat cooks more — see Texture LOD compress. See `TERRAIN_NORTHSTAR.md` + fulcrumRust `docs/TERRAIN.md`
...
| **Stream** | Live **11×11** window (`STREAM_RINGS` 5 — **#137**; #81 opened 9×9). **#108** amortizes cook (cook=1/2 · splash-pumped load-in). **#123** deepens play (`defer=worker/paint/gpu` · skip far mask-only remesh). **#137** heading prefetch + shipped hold=2/12. Live hold **#151** 4/16 + cheap LOD-3 `visible=5/19` + desert `8`. Beabim #83 `stream_anchors` also returns remote feet (coordinate only) |
...
| **Seams** | One extract density on every LOD · 8-subdiv bridge (32→16→8→4 stays 2:1 Lengyel) · yard flatten outer **9.2 → 20 m**. Residual soft LOD pop inside a cell still parked. Far-4 / void-past-extract **later landed #151** cheap LOD-3 (`visible=5/19`) + desert floor. Stream hitch amortize **later landed #108** |
...
| **7. Distance load** | **Landed #81** — local-player stream. **#108** amortizes cook (cook=1/2 · prefetch=5 m). **#123** deepens play (`defer=worker/paint/gpu` · skip far mask-only remesh). **#137** 11×11 / r=5 · prefetch=5+heading · shipped hold=2/12. Live hold **#151** 4/16 + `visible=5/19` + desert `8`. Beabim peer feet `stream_anchors` **landed #83** (coordinate only — no Transvoxel rewrite) |

See `TERRAIN_NORTHSTAR.md` + `PEEK_FINDINGS.md` Closed by #81 / Closed by #108 / Closed by #123 / Closed by #137 / Closed by #142 / Closed by #151. Live walk extents **#142** 37×37 + **#151** horizon/desert/rim — #81 stays the prior 8× / 19×19 fact. Hypha #88 consumes this same #79/#81 column for pawn plant — mesher untouched. Lab-Rat plugs **landed later #80**.

## Stream hitch amortize (Hypha — landed #108 + #123)

Evan lock. **Shipped** [fulcrumRust #108](https://github.com/initialvisuals/fulcrumRust/pull/108) (2026-09-09, `b3be2974`) + deepen [fulcrumRust #123](https://github.com/initialvisuals/fulcrumRust/pull/123) (2026-09-09, `b4725b8c`). **Hypha** owns amortized cook — not a new map size. Hitch *fix* layer 1 is #108 (cook budget / splash-pumped load-in). Worker/paint/gpu deepen is **#123**. Live window + heading prefetch + warm hold is **#137** (11×11 / shipped hold=2/12). Live hold **#151** 4/16 + cheap LOD-3 / desert / soft rim. Hitch *visibility* stays Augury #110 / #121 (Hypha emits, does not rebuild Home). Residual soft LOD pop / live octree stay parked leftovers.
...
See `PEEK_FINDINGS.md` Closed by #108 / Closed by #123 / Closed by #137 / Closed by #151 + `TERRAIN_NORTHSTAR.md`.
...
See `PEEK_FINDINGS.md` Closed by #137 + `TERRAIN_NORTHSTAR.md` + fulcrumRust `docs/TERRAIN.md`. Live host extents **later landed #142** (STREAM held 11×11). Live hold / horizon / desert / soft rim **later landed #151** (`hold=4/16` — do **not** read #137 `hold=2/12` as the live tip).
...
See `PEEK_FINDINGS.md` Closed by #142 + `TERRAIN_NORTHSTAR.md` + fulcrumRust `docs/TERRAIN.md` / `docs/SPAWNS.md`. STREAM past extract / desert / soft rim **later landed #151**.

## STREAM past extract + desert floor + soft rim (Hypha — landed #151)

Evan lock. **Shipped** [fulcrumRust #151](https://github.com/initialvisuals/fulcrumRust/pull/151) (2026-09-10, merge `0f6171cf`). **Hypha** owns horizon / exterior / hold / rim-hook on the #142 37×37 host — **not** a fine STREAM radius bump. Hitch *fix* layers stay #108 / #123. Fine window stays #137 **11×11**. Pend coalesce stays #142 `2/2`. Hitch *visibility* stays Augury #110 / #121 / #128. Dial sheet: fulcrumRust `docs/TERRAIN.md`.
...
| **Soft rim** | World block at extract+desert (~**422 m**): damp outward vel; spring back; no clip slap. `RimHook` / `Debugger::note_rim` → `RIM APPROACH\|BLOCK\|CLEAR hook=augury` for Augury glasses glitch + radio crackle — do **not** invent Slain chrome. Augury chrome BLOCK hold is **#158** on the #156 shelf (spring-stop `approaching && t >= 0.995`; Hypha `blocked` is crossing-frame only) |
...
Out of scope (do **not** claim shipped by #151): Lab-Rat 7×7 / CHANNELS / pedon / grit crawl (#152) · Range feel / AIM TUNE / heat · Beabim net/PVP/HOST · Augury glasses chrome / Slain rewrite · rock desert / Lab-Rat COL. Residual soft LOD pop inside a cell / live octree still parked.

See `PEEK_FINDINGS.md` Closed by #151 + `TERRAIN_NORTHSTAR.md` + fulcrumRust `docs/TERRAIN.md`.
...
Do **not** claim NRM/GLOSS GPU bind · remesh · chunk resize · STREAM bump · Range / Hypha / Beabim / Augury shipped this. #80 slope/PBR plugs + #101 UV sheet + #112 consume stay the prior layers. Grit/COL LOD luma crawl stop **later landed #152**. Desert skirt vendor sand COL **later landed #157**.

See `PEEK_FINDINGS.md` Closed by #144 / Closed by #152 + fulcrumRust `docs/STAMP_PBR_DIAL_SHEET.md`.

## Grit / COL LOD luma crawl (Lab-Rat — landed #152)

Evan lock. **Shipped** [fulcrumRust #152](https://github.com/initialvisuals/fulcrumRust/pull/152) (2026-09-10, `a86c1ed6`). Seat: **Lab-Rat**. Texture + UV / LOD sample stability only — not Hypha STREAM / extract / hold / Transvoxel remesh / landmark ride / pend. Chunk metres **16** held. Extra grit still on building faces only. CREDITS + STAMP_PBR_DIAL_SHEET + CHANNELS / TERRAIN already claimed in-PR. Canonical dial sheet: fulcrumRust [`docs/STAMP_PBR_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/STAMP_PBR_DIAL_SHEET.md) (LOD luma crawl row). Overnight cooks steal these numbers from the sheet — not every `.rs`. Do **not** invent dials.

Root: grit/COL bilinear on promote reshuffled darks at the same world XZ (`sample_tiled` used `fract(x/tile) * width`; a promote/demote changed the bilinear neighborhood). `material_lod` also flipped on the exact mesh chunk face.

| Dial | Lock |
|------|------|
| `STABLE_LOCUS_RES` | **16** — color / wear / COL / building grit_modulate snap to far-mip cell centers. Same world cell → same sample on 256/64/16 |
| `MATERIAL_HOLD_M` | **2.5** — stateless finer-pack hold past the mesh ring face. Not STREAM hold / extract |
| `LUMA_GRAIN` | ±**0.08** — fine pack may nudge the far-locus luma by this much. Promote cannot jump past the cap |
| Height / DISP | bilinear (unchanged) — geo / remesh path untouched |
| `FULCRUM_UV` | same — `promote_for_uv` still keeps a finer **existing** mip (`2,2` → mid 64²). Scale does not remesh |
| `CHUNK_METERS` | **16** held |

Walk or zoom a ring face: grit/COL dark spots stay put. Geo pop-in is Hypha STREAM (separate). Buildings still read poured concrete (grit on faces only).

| Held | Lock |
|------|------|
| Chunk metres | **16** |
| Extra grit | building faces only |
| Hypha STREAM / extract / hold / remesh / landmark ride / pend | off — #108/#123/#137/#142 layers stay |
| Height / DISP | bilinear |

Do **not** claim STREAM bump · remesh · chunk resize · Range / Hypha / Beabim / Augury shipped this. #60 grit mips + #112 `promote_for_uv` + #144 `pbr=vendor` stay the prior layers.

See `PEEK_FINDINGS.md` Closed by #152 + fulcrumRust `docs/STAMP_PBR_DIAL_SHEET.md` + `docs/CHANNELS.md` + `docs/TERRAIN.md`. Desert skirt vendor sand COL **later landed #157**.

## Desert skirt vendor sand COL (Lab-Rat — landed #157)

Evan lock. **Shipped** [fulcrumRust #157](https://github.com/initialvisuals/fulcrumRust/pull/157) (2026-09-10, `f6cd2586`; merge tip `8431bc25`). Seat: **Lab-Rat**. Texture + UV / vertex paint only — not Hypha STREAM / RimHook / extract extents / Transvoxel remesh / chunk resize. Not Range powder/heat. Not Beabim KIND. Not Augury glasses/radio. CREDITS + STAMP_PBR_DIAL_SHEET + TERRAIN / STAMPS already claimed in-PR. Canonical dial sheet: fulcrumRust [`docs/STAMP_PBR_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/STAMP_PBR_DIAL_SHEET.md) (Desert skirt row). Overnight cooks steal these numbers from the sheet — not every `.rs`. Do **not** invent dials.

Hypha #151 planted a flat desert skirt past extract (height + Sand hook). Yard sand tile **4.0 m** divides the old **8 m** quads, so every skirt vert sampled one COL texel — exterior read as bare height.

| Piece | Dial | Notes |
|-------|------|-------|
| COL | in-repo `pbr_sand_col.png` (`pbr=vendor`) | Same thumb as yard sand. `FULCRUM_PBR` / `FULCRUM_ATELIER` still win |
| Tile | **2.8 m** (`DESERT_SAND_TILE_M`) | Yard sand stays **4.0**. 4.0 aliases 8 m / 4 m verts onto one texel |
| Quads | **4.0 m** | Was **8 m** (height only). Vertex paint needs verts near tile density. Not a Transvoxel remesh |
| Grit | far `lod_mips` 16² wear | `stamp_wear_scale_lod` + `edge_wear` at lod 3. **Not** `grit_modulate` (building faces, #144/#152) |
| Material | `TerrainMaterial::Sand` | No rock / no stamp lobes. Height stays `DESERT_FLOOR_Y` (flat) |
| Extent | `desert=8` / **128 m** | Held from #151. Soft rim ~422 m is Hypha / Augury — do not retune |
| Sample | `desert_skirt_color` → `pbr_shelf::albedo_desert` | Same sand map. `FULCRUM_UV` still remaps XZ. Grim clamp unchanged |
| Smoke | `pbr=vendor` `desert=8/col=2.8` | Walk past the 37×37 rim (~296 m). Skirt reads sand COL tiles, not a flat wash |
...
| Yard sand tile | **4.0** |
| Extra grit | building faces only |
| #152 locus / hold / grain | `STABLE_LOCUS_RES` **16** · `MATERIAL_HOLD_M` **2.5** · `LUMA_GRAIN` ±**0.08** |
| Hypha STREAM / RimHook / hitch / landmark ride | off — #108/#123/#137/#142 layers stay |

Do **not** claim STREAM bump · remesh · chunk resize · rock desert · RimHook chrome · Range / Hypha / Beabim / Augury shipped this. #144 `pbr=vendor` + #152 locus/hold stay the prior layers. #151 height + Sand hook stays the skirt geo.

See `PEEK_FINDINGS.md` Closed by #157 + fulcrumRust `docs/STAMP_PBR_DIAL_SHEET.md` + `docs/TERRAIN.md` + `docs/STAMPS.md`.

## HOLO_MYCELIUM hideout leftover (Lab-Rat — landed #154)

Evan lock. **Shipped** [fulcrumRust #154](https://github.com/initialvisuals/fulcrumRust/pull/154) (2026-09-10, `d6b56a7e`). Seat: **Lab-Rat**. Cheap looping board-room hologram leftover for the pre-raid hideout hub. **Not** pycelium GPU mesocosm. **Not** Hypha STREAM / extract remesh / chunk metres / #151 sand COL. **Not** Beabim KIND / PeerBody. **Not** Range powder / Music. **Not** Augury Slain / arcade cabinets. CREDITS + HOLO_MYCELIUM_DIAL_SHEET + STEAL already claimed in-PR. Canonical dial sheet: fulcrumRust [`docs/HOLO_MYCELIUM_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/HOLO_MYCELIUM_DIAL_SHEET.md). Overnight cooks steal these numbers from the sheet — not every `.rs`. Do **not** invent dials. Do **not** invent a T·H·L hideout floorplan — Evan still owns the board-room.

| Dial | Lock |
|------|------|
| **Matrix** | **12 × 8 × 12** — tight pedon. Occupancy is a fixed `u8` grid |
| **Max live** | **96** spheroids + 1 scanline slab. Fixed `[Cell; 96]`. No per-frame alloc |
| **Max tips** | **10** — agents walk / curl / die. Branch is rare |
| **Update Hz** | **10** — frame tick only accumulates. Solids retint for the scanline between steps |
| **Sphere min / max** | **0.010 / 0.022** m half-span — low-poly pixel boxes. Size rides 0..1 growth then wilt |
| **Pitch** | **0.038** m cell spacing |
| **Life** | **28** steps — body can wilt early (hash). Tips last until the agent dies |
| **Tip purple** | `[0.68, 0.20, 0.86]`. Body blue `[0.16, 0.42, 0.92]`. Scan wash toward cyan |
| **Brightness** | **0.22 → 0.96** × scan boost **0.62** — scales with size. Scanline is a Y-plane gauss, not a second mesh |
| **Scanline Hz** | **0.55** — sweeps the volume forever. Thin leftover slab + per-cell tint |
| **Cycle** | **9.0** s or starve (`<3` live / `0` tips). Soft restart. Generation increments. Seed xor gen |
| **Warm** | **22** steps on `new` — hideout spawn already reads a colony |
| **Table AABB** | min **−3.50, 0.74, −1.10** · max **−1.30, 0.82, −0.10** — existing hideout **left desk**. Placeholder only |
| **Holo volume** | table top + **0.02** m, centered XZ. No collide. No glasses |
| **Env** | `FULCRUM_HOLO=0` / `off` — kill switch. Default on |
| **Smoke** | `holo=gen=N cells=M tips=T`. Extract `stream_rev` / terrain tris / CHANNELS layers must not move |

Peek: Title → Deploy. Look **left** at the wood desk. Blue leftover pixels grow / die; tips read purple; a cyan scanline sweeps the cloud. Home **COLL**: table AABB wire (blue) + projected points. Hideout upload reuses extract pedon leftover buffers (places never overlap). Pedon rebake stays the extract leftover.

Do **not** claim STREAM remesh · sand COL · KIND / PeerBody · Range powder · Augury Slain · T·H·L board-room metres · pycelium GPU mesocosm. #130 extract pedon stays the extract leftover.

See `PEEK_FINDINGS.md` Closed by #154 + fulcrumRust `docs/HOLO_MYCELIUM_DIAL_SHEET.md`.

## Locus Voidspore yard leftover (Lab-Rat — landed #171)

Evan lock. **Shipped** [fulcrumRust #171](https://github.com/initialvisuals/fulcrumRust/pull/171) (2026-09-10, merge `ce4162a5`). Seat: **Lab-Rat**. Cheap looping growth leftover on the extract yard — dark / inked webbing mycelium with purple tips. Start of a Locus Mycelium Voidspore entity. **Not** a fourth named plot. **Not** pycelium GPU mesocosm. **Not** Hypha STREAM / extract remesh / chunk metres 16 / RimHook / landmark ride. **Not** Range powder / AIM TUNE / PreferredHand. **Not** Beabim KIND / net. **Not** Augury Slain / glasses. Quiet grit thumbs stay as-is. CREDITS + VOIDSPORE_DIAL_SHEET + STEAL already claimed in-PR. Canonical dial sheet: fulcrumRust [`docs/VOIDSPORE_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/VOIDSPORE_DIAL_SHEET.md). House pointer: [`VOIDSPORE.md`](VOIDSPORE.md). Overnight cooks steal these numbers from the sheet — not every `.rs`. Do **not** invent dials.
...
| **Max solids** | **152** — 3-lobe fruit + tip chip worst case. Growth leftover GPU **768 / 192** KiB (was 640 / 160) so yard glow + this colony fit |
...
Peek: Title → Deploy. Extract spawn faces +Z. Look **left** (~−X, a couple metres forward). Dark inked webbing sits on the dirt; tips read purple; fruiting nodes stack like rock lobes. Home **COLL**: purple pad wire + projected points (steal probe_mark). Not a glasses card. Draw path is existing extract `upload_growth` leftover slot. Hideout HOLO_MYCELIUM keeps the pedon slot.

Do **not** claim STREAM remesh · CHANNELS compile · glasses elbow · AI / combat · a fourth named plot · pycelium GPU mesocosm. #154 hideout HOLO stays the hideout leftover. Quiet grit thumbs stay.

See `PEEK_FINDINGS.md` Closed by #171 + house `VOIDSPORE.md` + fulcrumRust `docs/VOIDSPORE_DIAL_SHEET.md`.
...
Do **not** claim Hypha STREAM remesh / Range / Beabim / Augury Home chrome. STREAM stays Hypha **#123**. Pad crawl stays **#114**. Hideout hologram leftover **later landed #154** (reuses extract pedon leftover buffers in hideout — places never overlap).

See `PEEK_FINDINGS.md` Closed by #130 + `STAMP_FEEL_LOCK.md` + fulcrumRust `docs/PEDON.md`. Hideout holo: Closed by #154.
...
| **End PX** | **#138** AIM TUNE travel. `shoulder_x_max` **+0.50** · box **±0.50**. **#159** `TUNE_POS_X_ABS` **0.50** floors travel even if `shoulder_x_max` is stale/zero; End WEAPON home nudge snaps `hold_blend`; ATTACH same box. Leftover +0.226 (`−shoulder_cross_x + shoulder_x_min`) is **not** a tuner cap. `AimTuner::nudge` clamps PX only; PY/PZ/rot unclamped. H still floors at `shoulder_x_min` only |
...
| **hold_spring** | **#109** U-cycle pose ease. **7.0** — house-medium between ADS `blend_speed` **6.4** and H `shoulder_spring` **8.0**. Sprint **6.2** / inspect **10.0** unchanged. Glasses still snap; viewmodel springs. **#159** End WEAPON home-pose nudge **snaps** `hold_blend` (U-cycle ease stays). Live ADS blend is **6.4** × kit ergo (**#113**) — not folded into this dial |
...
| **PreferredHand** | **#116** Hypha onboard. **#168** Range A: **`HAND R` / `HAND L`** from **live hold X** (gun right of middle = HAND R — not inverted `Shoulder` enum). One right bank + sagittal **−X / −yaw / −roll**. Leftover H dest ~**−0.041** is **not** the live left hold. **No** `scale.x = −1` |
| **Seat** | Range Tech. Same #59 feel PR as lean flip / hop / heat look / distant hit. Tilt path **#84**. First +0.08 / left hold **#94**. Live hip / ready-Y **#98**. Live hip_low **#99**. Live ads_cant **#100**. Live hold springs **#109**. Live End PX **#138**. Live End PX hideout wall **#159**. Hypha PreferredHand **#116** seats home. **#168** live HAND chrome + sagittal mirror. Do **not** claim #167 live-save |

See `PEEK_FINDINGS.md` Closed by #59 / Closed by #84 / Closed by #94 / Closed by #98 / Closed by #99 / Closed by #100 / Closed by #109 / Closed by #113 / Closed by #116 / Closed by #138 / Closed by #159 / Closed by #168 / Closed by #165 + H shoulder-swap tilt (#84) + Patch A RH hip bias (#94) + RH body-width + ready-hip Y (#98) + low-hip shotgun stance (#99) + canted CQC ADS (#100) + U-cycle hold springs (#109) + kit handling / ergo / MOA (#113) + PreferredHand onboard (#116) + AIM TUNE PX travel (#138) + AIM TUNE PX hideout wall (#159) + PreferredHand A (#168) + Options READY HIP + two-pose U (#165).
...
Evan lock. **Shipped** [fulcrumRust #66](https://github.com/initialvisuals/fulcrumRust/pull/66) (2026-09-08, `05dd80ad`). **Hypha** owns the post path. Range Tech keeps `FeelState.barrel_energy` / heat-tune hold-**J** / heat dials on `heat-card-dial-sheet.md` — live card field is the **#129** CE tip **0.2.8** lock (#71 blend / v77 / dump stay DNA). **#155** sells visible tip warp on this same path. Same #55 fullscreen `engine/src/post.rs` stack as #68 ADS DoF — no second composer, no new Graphics sliders.
...
| **HUD / glasses** | Still composite after post. Glasses / live sheet still drive the live HeatDials (**#129** CE tip field; #71 DNA; **#155** warp range) |
| **Siblings** | #59 v77 shimmer intent (lattice crawl stays spatial input). #67 Patch A (explicitly did not fight #66). #68 ADS near on the same stack. #55 GPU post stack. **#71 dump-dial blend** (Range Tech DNA; live **#129** CE tip field; did not reopen orange cards). **#90** `pixel_warp_uv` sits after `heat_warp_uv` (heat body not edited — Options **WARP** is pixellation, not heat). **#155** lowered the shader gate to **0.001** + lattice amp at the CE floor so hold-J is visible |
...
Post strength + radius stay on the **#66** path. **#129** shipped `visual * haze` at the CE floor — that product is **0.01**, and the shader early-out `< 0.01` then × **0.010** made hold-J invisible. **#155** restores visibility: `visual × (lattice_emissive_mean × 1.35) × haze_warp_mul(haze)` · gate **0.001** · UV scale **0.010**. Haze **0** still kills. Radius scale/cap **0.40 / 0.08** held. Tip-weighted lattice energy + energy-weighted post UV (field on the can, not a haze cloud). Overlay disc lobe stays parked (`HEAT_LOBE_DISCS = 0`).
...
See `heat-card-dial-sheet.md` + `HEAT_WARP_DIAL_SHEET.md` + `PEEK_FINDINGS.md` Closed by #129 + Closed by #155.

## Visible barrel heat warp (Range Tech — landed #155)

Evan lock. **Shipped** [fulcrumRust #155](https://github.com/initialvisuals/fulcrumRust/pull/155) (2026-09-10, `d3d7848c`). **Range Tech** owns heat dials on the Hypha **#66** colorless post path. Overnight feel: heat cards / barrel lattice warp missing — even at max Options **WARP** Evan could not see colorless tip distortion. Soft hypo: `#129` `visual × 0.01` maxed `post_heat_strength` at 0.01; shader early-out `strength < 0.01` then scaled leftover by **0.010** (~0.0001 UV). Options **WARP** is Hypha pixellation (`post.warp_strength`) — it does not feed Range heat. Cards **were** authoring the field. Fog 375/520 was **not** eating the warp. After #66 the cards do not draw RGB — warp *is* the look.

| Dial | Was (#129) | **Now** | Stolen / DNA | Note |
|------|------------|---------|--------------|------|
| `haze_strength` | 0.01 | **0.01** | 0.11 | CE enable floor unchanged. **0** off / **0.01** enable / **0.11** stolen max |
| `post_heat_strength` | `visual × 0.01` | **`visual × lattice × 1.35`** at the floor | × **1.20** at 0.11 | #66 lattice amp. 0 still kills |
| Shader gate | `strength < 0.01` | **`strength < 0.001`** | | Enable floor is not a visibility cliff |
| Shader UV scale | 0.010 | **0.010** | #66 | Strength 0.81 → ~0.008 UV tip shimmer |
| Radius scale / cap | 0.40 / 0.08 | **0.40 / 0.08** | | Muzzle-adjacent — no fog blob |
| `scale_x` / `scale_y` | 0.28 / 0.86 | **unchanged** | 0.20 / 1.86 | Short thin tip cards |
| `count` / `lobe` | 20 / 0.40 | **unchanged** | 14 / 0.35 | Cards author the field |
| Fog near / far | 375 / 520 | **unchanged** | | Extract distance haze stays |
| Options **WARP** | Hypha pixellation 0.01–1.0 | **unchanged** | | Not heat |

Cards still author UV center / radius / lattice energy. RGB stays zeroed. Tight radius stays the anti-blob (the #71 wash was lattice restore on a **tall** field). #89 projectile feel / AIM TUNE / #143 wound / Lab-Rat stamps / Hypha STREAM / Beabim KIND / Augury chrome untouched.

See `HEAT_WARP_DIAL_SHEET.md` + `heat-card-dial-sheet.md` + `PEEK_FINDINGS.md` Closed by #155 + fulcrumRust `docs/HEAT_WARP_DIAL_SHEET.md`.
...
- Cheap **5-box** operator silhouette (slate) — not Mixamo / not Locus. **#91** 3-box gun stub on networked muzzle — **partial** 1P ≠ 3P (biped hip stub). **#153** held 3P clone draws authored `kit_boxes` on `GUN_HOLD_LOCAL` `(0.18, 1.05, 0.28)` via `presented_local` (lean rides torso). Hands / gear / Mixamo still open
...
- **#102 KIND_LOOT** on the #91 UDP leftover. Host relays; both sides apply drop/take by `InstanceId`. Hairline `{NAME} DROP/TAKE KIT` via `plan_name_tag` (`FULCRUM_NAME` / HOST / P{id}). Starting kits stay local. Solo `net=off` unchanged — no trail chrome. **#153** host `KIND_LOOT_WORLD` **15** snapshot (HELLO + ~4 Hz, `LOOT_WORLD_SECS` **0.25**) so late join + one-shot UDP miss recover. Throw vel on `LootEvent` (bytes 44–55). Mag / reserves / optic / can from `KitLoadout::snapshot` — **no magic refill**. `RECV_BYTES` **1024** (was 256)
...
- **#153 MP weapon honesty** — host `KIND_LOOT_WORLD` **15** so both windows see the same world drops. Held 3P clone: POSE unused **byte 3** advertises weapon / optic / can / empty hands; remote `PeerBody` draws authored `kit_boxes` on `GUN_HOLD_LOCAL` `(0.18, 1.05, 0.28)` via `presented_local` (lean/peek rides torso). Hideout starting-room remotes draw. Hub downs keep kits (no `KIND_BODY` bag); `HUB_PROTECT_SECS` **15** (`protect_ms` on KIND_PVP bytes 22–23). Raid / extract stays normal (loot + 1.20 s leftover, no hub flag). Dial sheet: fulcrumRust `docs/MP_WEAPON_DIAL_SHEET.md`. Soft names: house `HIDEOUT_HUB.md` (metres still pending)
- Honesty: two instances agree on **peer presence** (pose + silhouette) + **#102** kit loot trail + **#119** shot / Locus leftover / death bag + **#122** shared raid instance (same raid id + host seed) + **#133** PVP leftover (toggle / hide names / peer hit / KIND_BRASS / rim respawn) + **#139** HOST session board (PVP radio pre-enter · LAN invite, no 127 seed) + **#141** PVP honesty (PeerBody leftover · HP/AR Sync · unique pads) + **#147** leftover ray (**500 m**, was 80) + **#153** world-drop snapshot / held 3P kit / hideout PeerBodies / hub no-loot + 15 s protect while menus mute the local pawn only. Range feel / HoB / heat / eject dials / 1P hip / AIM TUNE / **#143** 1P screen-react (hooks Hit not Sync) stay local — **no** full world replication / land feel / H tilt (#84) / audio device (#82) / Graphics dump (#86) / HDRI sun (#87) / Locus brain tick on the joiner / stamps / COL / deform / scatter / Transvoxel rewrite / Augury elbow / hatch UX (#85) / world seed / ToD / Mixamo player body / server browser / dedicated infra / hideout floorplan metres / crisp flash chrome
...
- Hideout hub **#153**: downs / hits inside hideout do **not** drop a kit or seat a `KIND_BODY` bag. **Z** throw stays a player drop. `HUB_PROTECT_SECS` **15** (both peers agree via `protect_ms`). Extract / raid bleed still drops a bag + 1.20 s leftover. Crisp flash chrome stays Augury/Range (pending)
- Range wound feel **#143**: 1P screen-react only (suppress / armour / HP). Hooks KIND_PVP **Hit** (and local `apply_player_wound`) — **Sync** does not punch. **Death / Slain chrome stays Augury** (#36 `DOWNED` / `DEAD` / bag). Do **not** claim #143 shipped death cam / slain UI
- Soft rim leftover **#156**: glasses + glitch + FX crackle on Hypha `RimHook`. Thin hold **#158** spring-stop `approaching && t >= 0.995` stays BLOCK / `CARRIER  LOST` / glitch **0.36** + crackle (Hypha `blocked` is crossing-frame only). **Mutes under Slain** — death envelope wins (no second leftover on the plate). Do **not** invent a Slain plate / Space / death cam from the rim
- Parked / still TODO (do not claim done): death cam **later landed #146** + **#162**; teammate net stabilize; timed surface kill; full extract loot loop. Ragdoll visual flop **later landed #131** `PeerBody`. Leftover hurt volumes **later landed #141** (`PeerBody::hurtboxes()` + skin **0.06** + sweep **4**)
- Range wound feel **#143**: 1P screen-react only (suppress / armour / HP). Hooks KIND_PVP **Hit** (and local `apply_player_wound`) — **Sync** does not punch. **Death / Slain chrome later landed #146** + **#162**. Do **not** claim #143 shipped death cam / slain UI
- Parked / still TODO (do not claim done): death cam **later landed #146** + **#162**; teammate net stabilize; timed surface kill; full extract loot loop. Ragdoll visual flop **later landed #131** `PeerBody`. Leftover hurt volumes **later landed #141** (`PeerBody::hurtboxes()` + skin **0.06** + sweep **4**)
...
Atelier roughness packs are **4k 48-bit PNG** — too large for extract. Do **not** ship raw 4k 48-bit into the yard. Quiet grit greyscales **landed #58** (vendored bake-downs + `sample_channels` quiet height + `grit::rough` wear). Hypha ring-mip texture LOD **shipped #60**. Lab-Rat slope/PBR/dirt/scatter/deform plugs **landed #80**. Blender UV dials **landed #101** — tile those packs without shrinking geo. Hypha Transvoxel UV consume **landed #112** (`promote_for_uv` + wear/COL honor). Grit/COL LOD luma crawl stop **landed #152** (`STABLE_LOCUS_RES` **16** · `MATERIAL_HOLD_M` **2.5** · `LUMA_GRAIN` ±**0.08**; height/DISP stay bilinear). Desert skirt vendor sand COL **landed #157** (`pbr_sand_col` · tile **2.8 m** · quads **8→4 m** · far lod grit). Atelier **150 roughness + textures/PBR ~26 sets landed**. Evan **clean** yell 2026-09-08 ~00:00 ET — atelier plugs **open**. Whole roughness→stamp cook is **not** done (SVG / density-mask / experiment-log still open).
...
| **Hypha** | LOD-tied mips / compression **shipped #60** on Transvoxel **distance rings**. Near **256²** (Lab-Rat vendor) · mid **64²** box mip · far **16²** box mip (cheaper / softer; far drops grain hashes). **#112** `promote_for_uv` + TerrainHost wear/COL honor Lab-Rat `#101` `uv::xform` — not a chunk resize / not a remesh. Lab-Rat **#152** grit/COL luma crawl stop: color/wear/COL snap to far **16²** loci (`STABLE_LOCUS_RES` **16**) · `MATERIAL_HOLD_M` **2.5** · `LUMA_GRAIN` ±**0.08**; height/DISP stay bilinear. In-repo `assets/stamps/grit_*.png` until Lab-Rat cooks more. Smoke `grit_mips=256/64/16 n=196608 f=768` `uv=` |
...
| **RES / FOV / AA / AO / POST** | **#164** Graphics shelf. **RES** default **NATIVE** (desktop native) + 1280×720 · 1920×1080 · UW 2560×1080 · 2560×1440 · 3840×2160 · **UW 4K CLASS** 3840×1600 · **3440×1440** · Steam survey **2560×1600 / 1920×1200 / 1366×768**. **FOV** hip base **90** (ADS optic locks still win). **AA** FXAA/OFF · **AA STR** **0.5** live #55 mix (not CA 0.35) · **AO** OFF/LOW/HIGH (Low 8-tap · r **0.55**; High **24**-tap same radius) · **POST** OFF/LOW/HIGH default HIGH. Persist `resolution` / `fov_hip` / `aa_type` / `aa_strength` / `ao_quality` / `post_quality`. Old `ao` / `aa` bools still load |
| **TEXTURE / MIPMAP** | **#172** Lab-Rat. Rows under **STREAM**. **TEXTURE** default **HIGH** LOW 16² · MED 64² · HIGH 256² · ULTRA 256² (Ultra === High — tip 256² `pbr=vendor`). **MIPMAP** default **MED** LOW +1 · MED identity · HIGH −1 · ULTRA −2. Persist `texture_quality` / `mipmap_quality`. `FULCRUM_TEXTURE` / `FULCRUM_MIPMAP` override at boot. Chunk metres **16** stay |
| **Graphics hint** | `POST / FOG / CAM / WARP  ·  A/D NUDGE` |
| **Gameplay** | Glasses labels toggle + crosshair toggle (real — drop quads when off). **#168** hides the 2D plus when `ads_factor` **> 0.5**; hip + U-cycle canted keep it. Range **READY HIP** **landed #165** — **CHEST** / **LOW HIP** on this pane (Enter / click / A/D). Persist `default_hip`. Hint: `SHOOT FEEL STAYS · ENTER TOGGLE` |
| **Controls** | **#163** split: **MOUSE H** (`look_mul_h`) · **MOUSE V** (`look_mul_v`) · **HIP** (`hip_look_mul`) · **ADS** (`ads_look_mul`) — all default **1.00** on the existing `LOOK_MUL` lock min **0.25** / max **2.00** / step **0.05**. Invert Y toggle. Old single `look_mul` seeds both mouse axes when split keys are absent. Feel-lab `look_sens` **0.0022** + feel ADS **0.86** stay DNA. Binds stay README. Hint: `LOOK SITS ON FEEL-LAB SENS · BINDS IN README` |
| **Audio** | Untouched by #46 — Range Tech #21 Voice/Music/FX mixer. **#82** DEVICE row sits above the buses on that pane |
| **Persist** | `project.json` in cwd, or `FULCRUM_SETTINGS=/path/to.json` — resolved **absolute** (**#169**). #82 `output_device` · **#90** `warp_strength` · **#103** `aim_live` / `aim_tune.*` (**#167** per-kit pull — authored kits keep persisted peeks; **#169** flush absorb undirty + `project.json.lock`) · **#116** `profile_onboarded` / `preferred_hand` · **#163** `look_mul_h` / `look_mul_v` / `hip_look_mul` / `ads_look_mul` · **#164** `resolution` / `fov_hip` / `aa_type` / `aa_strength` / `ao_quality` / `post_quality` · **#165** `default_hip` · **#172** `texture_quality` / `mipmap_quality` live on the same file |
...
| LIVE persist | **landed #103** — Hypha `project.json` · `aim_live` default **true** · End sheet `LIVE` / `OFF`. **#167** per-kit pull — authored kits keep persisted peeks; apply snaps `hold_blend`. **#169** absolute path · flush absorb · `project.json.lock` |
| PX travel | **#138** `shoulder_x_max` **±0.50** — leftover +0.226 is **not** an End cap. **#159** `TUNE_POS_X_ABS` **0.50** floor · End WEAPON home snap · ATTACH same box. PY/PZ/rot unclamped |
| Glasses | `AIM TUNE` |
...
| Seat | Owns |
|------|------|
| **Range Tech** | Live End tuner DNA on existing ViewmodelDials + kit_mesh sockets (#97). LIVE persist into Hypha `project.json` (#103). Per-kit pull so authored-sheet flush cannot clobber other guns (#167). Two-exe absolute path + flush absorb + `project.json.lock` (#169). PX travel **±0.50** (#138). PX hideout wall floor + home snap (#159) |
| **Augury** | Home debugger **later landed #110**. Do **not** claim shipped by #97 / #103 / #138 / #159 |

See `PEEK_FINDINGS.md` Closed by #97 / Closed by #103 / Closed by #138 / Closed by #159 / Closed by #167 / Closed by #169.

## AIM TUNE LIVE persist (Range Tech — landed #103)

Evan lock. **Shipped** [fulcrumRust #103](https://github.com/initialvisuals/fulcrumRust/pull/103) (2026-09-09, `74da84e3` / `e0452f82`). **Range Tech** steals the Hypha Graphics/Audio `project.json` persist path so End AIM TUNE peeks survive quit/relaunch. Does **not** take Augury Home (debugger **later landed #110**). #97 End tuner / Delete JSON dump schema stay. **#138** PX travel does **not** change this schema (`aim_tune` / `aim_live` untouched). **#159** hideout wall does **not** change this schema. **#167** per-kit pull follow-up does **not** change this schema — authored-sheet flush must not clobber other guns. **#169** two-exe persist does **not** change this schema — absolute path + flush absorb + `project.json.lock`.
...
| Seat | Owns |
|------|------|
| **Range Tech** | AIM TUNE LIVE persist on Hypha `project.json` (#103). Per-kit pull (#167). Two-exe persist (#169). #97 End stays |
...
See `PEEK_FINDINGS.md` Closed by #103. Canted optic + AIM TUNE ATTACH **later landed #150** (`attachments.canted`). AIM TUNE PX hideout wall **later landed #159**. Per-kit pull **later landed #167**. Two-exe persist **later landed #169**.

## AIM TUNE PX hideout wall (Range Tech — landed #159)

Evan lock. **Shipped** [fulcrumRust #159](https://github.com/initialvisuals/fulcrumRust/pull/159) (2026-09-10, merge `5478db35`). **Range Tech.** #138 named AIM TUNE PX **±0.50** but hideout LIVE still parked at **PX +0.2258** (Evan 2026-09-10, MP9-Z HIP). Fine/Coarse +X dead; −X free. Leftover +0.226 was never a write `min()` on End — tests started at authored hip **+0.2403** (already past the wall). `hold_spring` **7.0** eased `hold_blend` so the gun lagged the panel. Steal fulcrumRust [`docs/AIM_TUNE_X_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/AIM_TUNE_X_DIAL_SHEET.md). CREDITS + STEAL already claimed in-PR — house shelf only. Do **not** invent dials. Do **not** invent A-note PRs.

| Dial | Value | Notes |
|------|-------|-------|
| Authored MP9-Z hip X | **+0.2403** | Unchanged. Ready-hip RH |
| Authored SR-25 / M24 hip X | **+0.256 / +0.261** | Unchanged |
| Hideout live-save (2026-09-10) | **+0.2258** | Under leftover wall. Fine +X must leave. Coarse → **+0.50** |
| H dest (default hip) | **~−0.041** | `hip + shoulder_cross_x`, then floor `shoulder_x_min` |
| `shoulder_cross_x` | **−0.281** | Unchanged |
| `shoulder_x_min` | **−0.055** | H live-pose left floor only. Not the tuner box |
| Leftover derived +X wall | **+0.226** | `−shoulder_cross_x + shoulder_x_min`. Documented, **≠ End box** |
| `shoulder_x_max` / `TUNE_POS_X_ABS` | **+0.50** | AIM TUNE PX travel. Symmetric **±0.50**. Floor if dial is stale/zero |
| End PX min / max | **−0.50 / +0.50** | WEAPON + ATTACH PX writes. PY/PZ/rot unclamped |
| `hold_spring` | **7.0** | U-cycle ease stays. End home-pose nudge **snaps** `hold_blend` |
| ADS `blend_speed` | **6.4** | Untouched |
| Live-save schema | #103 | `aim_tune.*` + `aim_live`. No new keys. Per-kit pull **#167** must not clobber other guns. Two-exe persist **#169** (absolute path · flush absorb · `project.json.lock`) |

| Seat | Owns |
|------|------|
| **Range Tech** | End PX floor + WEAPON home snap + ATTACH same box (#159). #138 named the ±0.50 box |
| **Hypha** | PreferredHand **#116** seats home. **#168** Range live HAND chrome + AIM TUNE HAND·kit·pose — not this cook |

Intact / out of scope: authored hip / H dest · #103 live-save · ADS blend · Beabim · Hypha STREAM · Lab-Rat stamps · Augury Home · PreferredHand live in AIM TUNE dialogue. Do **not** claim Beabim / Hypha / Lab-Rat / Augury shipped this.

See `PEEK_FINDINGS.md` Closed by #159.

## Options READY HIP + two-pose play (Range Tech — landed #165)

Evan lock. **Shipped** [fulcrumRust #165](https://github.com/initialvisuals/fulcrumRust/pull/165) (2026-09-10, merge `9b8121b1`). **Range Tech.** Steal house `OPTIONS_SHELF.md` + fulcrumRust [`docs/OPTIONS_DEFAULT_HIP_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/OPTIONS_DEFAULT_HIP_DIAL_SHEET.md). CREDITS + STEAL_MAP already claimed in-PR — house shelf only. Do **not** invent dials. Do **not** invent A-note PRs. Do **not** duplicate planted **#163** mouse / hip / ADS rows.

| Dial | Value | Notes |
|------|-------|-------|
| Options **READY HIP** | **CHEST** / **LOW HIP** | Gameplay pane. Enter / click / A/D. Same Hypha persist as glasses / crosshair |
| Chest (tip default) | `hip` Y **−0.2128** | Persist `default_hip`: `"chest"`. Walking ready-hip |
| Low hip | `hip_low` Y **−0.3528** | Persist `default_hip`: `"low"`. #99 shotgun low-ready authored pose |
| Parse aliases | `chest` / `ready` / `hip` · `low` / `low_hip` / `hip_low` | Missing key = Chest |
| In-play **U** | two poses | Chosen default hip ↔ **canted**. No Chest → LowHip → Canted ladder |
| Spawn / ready | chosen hip | Hideout ready · extract deploy · death+PVP respawn · quit-to-menu. Canted is **not** a spawn pose |
| Hold-**Mouse5** | `ads_cant` | From the default hip. Does not steal U |
| RMB from canted | CQC ADS | `#100` |
| RMB from Low | iron/optic | `#99` |
| `ready_hold(kind)` | `default_hip.resolve(kind)` | Seat and U both go through it |
| Shotgun Low-later | hook only | `WeaponKind::default_hip_override` → `Option<DefaultHip>`. Day-one MP9-Z / SR-25 / M24 return `None`. **Not shipped** |

Authored `hip` / `hip_low` / `hip_cant` + AIM TUNE WEAPON poses stay. Play does not cycle all three. PreferredHand A chrome **not** this cook — READY HIP intersects the hold/grip pose home without inventing HAND L/R Options. Powder **B later landed #166**. AIM TUNE LIVE **#167** already landed (sibling — do not restamp **#103**).

| Seat | Owns |
|------|------|
| **Range Tech** | READY HIP Chest/Low + two-pose U (#165) |
| **Hypha** | Persist path (`default_hip` on `project.json`). Res / FOV / AA / AO / post / WARP / STREAM **untouched** |
| **Augury** | Remap / Tab **untouched** |

Intact / out of scope: mouse V/H · hip/ADS look mul (**#163** planted — not restamped) · Hypha res / FOV / AA / AO / post / WARP / STREAM · Lab-Rat texture / mip **later landed #172** · Augury remap / Tab · Powder **C** · PreferredHand mirror bank · stamps · Death chrome · AIM TUNE PX · authored pose numbers. Do **not** claim shotgun Low / PreferredHand chrome / Powder C shipped. Powder **B later landed #166**. AIM TUNE LIVE **#167** already landed (sibling).

See `PEEK_FINDINGS.md` Closed by #165 + house `OPTIONS_SHELF.md` + `HOLD_POSE_SPINE.md`.
...
## 1P viewmodel ≠ 3P biped gun (MP honesty) — partial shipped #91 + #153

Evan lock (2026-09-09 InitialVisuals). **Partial shipped #91** — peer biped hip gun stub (`reconstructed_gun`). **#153** held 3P clone — authored `kit_boxes` on the posed hip-hand (not a copy of the viewmodel mesh, not PreferredHand, not End tuner sockets). Hands / gear / Mixamo still **open**. Overnight cooks steal from this shelf. Goes with HANDS down the road.
...
- **#153** remote `PeerBody` draws authored `kit_boxes(weapon, optic, can)` on `GUN_HOLD_LOCAL` `(0.18, 1.05, 0.28)` via `presented_local` so lean/peek rides the torso. POSE unused **byte 3** advertises weapon / optic / can / empty. Identity attachments — not `kit_boxes_tuned` / 1P cant
- Do **not** claim Mixamo / hands / gear / PreferredHand 3P
...
| **Beabim** | 3P sync path — peer gun stub **landed #91**; peer shot muzzle **#119** uses that hip stub; KIND_* / hitboxes / loot UI (consume Hypha metres); PVP leftover **#133**; HOST session board **#139**; PVP honesty **#141** (`PeerBody::hurtboxes()` leftover · HP/AR Sync · unique pads); leftover ray **#147** (`LEFTOVER_HIT_M` **500 m** was 80 · flat `SMG_PELLET` **14** · Locus yard keeps own 80); MP weapon honesty **#153** (`KIND_LOOT_WORLD` **15** · held 3P kit clone · hideout PeerBodies · hub no-loot / `HUB_PROTECT_SECS` **15`); hands / gear still open |
| **Lab-Rat** | Stamps stay out |

See `PEEK_FINDINGS.md` Holding / locked intent — 1P ≠ 3P + Closed by #91 + Closed by #119 + Closed by #131 + Closed by #145 + Closed by #141 + Closed by #147 + Closed by #153.
...
Keep **`heat_warp_uv` (#66 / #71 / #155)** distinct from **`pixel_warp_uv` (#90)**. Heat haze stays Range on the Hypha #66 colorless path. Heat cards stay on `heat-card-dial-sheet.md`. Warp *range* / shader gate stay on `HEAT_WARP_DIAL_SHEET.md`. Options Graphics **WARP** cannot sell barrel heat.
...
## Soft rim leftover (Augury — landed #156)

Evan lock. **Shipped** [fulcrumRust #156](https://github.com/initialvisuals/fulcrumRust/pull/156) (2026-09-10, `f7167202`). Thin hold follow-up [fulcrumRust #158](https://github.com/initialvisuals/fulcrumRust/pull/158) (2026-09-10, `f42e5f9`) — squash onto this shelf; **not** a second soft-rim landing. Seat: **The Augury**. Overlay + FX only on Hypha #151 `RimHook`. Soft-block physics stay Hypha (`apply_soft_rim` / `RIM_SOFT_M` **24** / world rim ~**422**). Ride edge-triggered Home STREAM lines (`RIM APPROACH / BLOCK / CLEAR hook=augury`) — **no** Slain plate, Space respawn, or death cam. CREDITS + SOFT_RIM_DIAL_SHEET + STEAL already claimed in-PR under #156 (Clerk classified #158 **micro** under Evan major-only — house shelf only). Canonical dial sheet: fulcrumRust [`docs/SOFT_RIM_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/SOFT_RIM_DIAL_SHEET.md). Overnight cooks steal these numbers from the sheet — not every `.rs`. Do **not** invent dials. Do **not** re-credit the whole soft-rim system as a new landing.

Hypha `apply_soft_rim` sets `RimHook.blocked` **only on the crossing frame**. After the spring stop, `hook.t` holds at **1.0** with `approaching` (`blocked=false`). Crossing-frame `blocked` alone is not enough — without a chrome hold, Augury flipped back to APPROACH / `SIGNAL  THINS`. **#158:** Augury treats `hook.blocked || (hook.approaching && hook.t >= 0.995)` as **BLOCK** voice (`CARRIER  LOST` + glitch **0.36** + crackle). Does **not** rewrite Hypha physics / STREAM / sand COL / ballistics / AIM TUNE.

| Dial | Value | Notes |
|------|-------|-------|
| Soft band (Hypha, read-only) | `RIM_SOFT_M` **24** m | `hook.t` 0 at band enter → 1 at spring stop. **Do not rewrite** |
| World rim (Hypha, read-only) | extract + desert − `RIM_STOP_M` (~**422** m) | Damp outward vel + spring back |
| Chrome BLOCK gate | `hook.blocked \|\| (hook.approaching && hook.t >= 0.995)` | **#158** hold. Crossing-frame `blocked` alone is not enough after spring stop |
| Approach glitch | floor **0.08** → peak **0.28** | Scales with `hook.t`. Steal `death::glitch_quads` / band shift / chroma |
| Block glitch | **0.36** hold | Below death red (~0.55) / Slain (~0.95). Holds at spring-stop `t >= 0.995` even when `blocked` is false |
| Chroma | **0.012** | Same scale as death prompt split |
| Message | `SIGNAL  THINS` / `CARRIER  LOST` | Mid-glasses `y = 0.22`. Fade **0.55** s on CLEAR. Spring-stop hold stays `CARRIER  LOST`. Not an error toast |
| Crackle | FX `Slot::Crackle` · **0.42** s | Procedural radio breakup. Retrigger on APPROACH/BLOCK. Holds with #158 BLOCK. Stop on CLEAR / Slain. Not Music loop |

| Held | Lock |
|------|------|
| Hypha STREAM / desert skirt / soft-block math | **#151** `RimHook` / `apply_soft_rim` — Augury rides, does not retune |
| Slain plate / Space / death cam | **out** — mute rim chrome if Slain is live |
| Range heat/wound/AIM · Beabim KIND · Lab-Rat COL · PreferredHand · LANDMARK_RIDE | **untouched** |

Do **not** claim Hypha / Range / Beabim / Lab-Rat shipped this. Do **not** invent a Slain plate from the rim.

See `PEEK_FINDINGS.md` Closed by #156 (hold #158 squashed onto that row) + fulcrumRust `docs/SOFT_RIM_DIAL_SHEET.md`.
...
| **Silhouette** | Cheap **5-box** operator (slate) — not Mixamo / not Locus. **#91** 3-box gun stub — **partial** 1P ≠ 3P. **#153** held 3P `kit_boxes` clone on PeerBody lean; hands / gear still open |
...
| **Gun stub** | Remote 5-box slate operator gets a 3-box gun stub on the networked muzzle (`reconstructed_gun`). **Later landed #153** authored `kit_boxes` clone on `GUN_HOLD_LOCAL` |
...
| **Pose / gun stub** | #91 hip stub. **Later landed #153** held 3P `kit_boxes` clone + `KIND_LOOT_WORLD` snapshot (**1P ≠ 3P** partial) |
...
See `PEEK_FINDINGS.md` Closed by #102. World/sim leftover: Closed by #119. No-pause + KIND_RAID: Closed by #122. MP weapon honesty: Closed by #153.
...
**Still local / other seat (deliberately):** Range 1P feel / AIM TUNE / HoB / heat / SFX / eject dials · wound screen-react **later landed #143** (1P Hit punch, not Sync — do **not** claim #141 shipped feel) · Lab-Rat stamps / pedon / CHANNELS · Hypha STREAM / mesher (#137 11×11 stay; live hold **later landed #151** 4/16 — consume height / PeerBody metres only — do **not** claim Hypha shipped this leftover) · Augury Home binds / hatch glasses · Death/Slain **later landed #146** + **#162** · party name exception · Range **2.5× head** · Mixamo / full 3P kit honesty / hands / gear · server browser / dedicated infra. Do **not** claim Range / Hypha / Lab-Rat / Augury shipped this.
...
**Still local / other seat (deliberately):** Range 1P feel / AIM TUNE / HoB / heat / zero / falloff · Lab-Rat stamps / pedon / CHANNELS · Hypha STREAM / mesher / 4× · Augury Home / hatch / death · Mixamo / hands / gear / server browser. Hideout hub leftover **later landed #153**. Do **not** claim Range / Hypha / Lab-Rat / Augury shipped this.

See `PEEK_FINDINGS.md` Closed by #147 + fulcrumRust `docs/PVP_DIAL_SHEET.md`. MP weapon honesty + hideout hub: Closed by #153. Ballistics A leftover: Closed by #160. Range loft DNA + AIM TUNE stash: Closed by #161. Powder B: Closed by #166.

## Ballistics A leftover / KIND_SHOT loft (Beabim — landed #160)

Evan lock. **Shipped** [fulcrumRust #160](https://github.com/initialvisuals/fulcrumRust/pull/160) (2026-09-10, merge `e0803dc1967418fef7bdd7e04a31c2a3b2f681ed`). Seat: **Beabim** owns leftover/net only. House `BALLISTICS_SPINE` cook **A**. Hitscan stashed, not deleted. Extends #119 KIND_SHOT + #147 leftover ray. Range Tech loft DNA + AIM TUNE leftover stash **later landed #161**. CREDITS + STEAL_MAP + `BALLISTICS_A_DIAL_SHEET` already claimed in-PR. Canonical dial sheet: fulcrumRust [`docs/BALLISTICS_A_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/BALLISTICS_A_DIAL_SHEET.md). Overnight cooks steal these numbers from the sheet — not every `.rs`. Do **not** invent dials.
...
| **#147 leftover** | Max / eye / pellet / Locus yard **80 m** / PeerBody hurt skin-sweep **held** |

**Still local / other seat (deliberately):** powder A/B/C **later landed Powder B #166** (A restoreable · C parked) · Hypha STREAM · Augury · hub protect · `KIND_LOOT_WORLD` · Locus yard **80 m**. Range loft DNA + AIM TUNE stash **later landed #161**. Do **not** claim Hypha / Lab-Rat / Augury shipped this.

See `PEEK_FINDINGS.md` Closed by #160 + Closed by #161 + Closed by #166 + fulcrumRust `docs/BALLISTICS_A_DIAL_SHEET.md` + house `BALLISTICS_SPINE.md`.

## Ballistics A loft DNA + AIM TUNE leftover stash (Range Tech — landed #161)

Evan lock. **Shipped** [fulcrumRust #161](https://github.com/initialvisuals/fulcrumRust/pull/161) (2026-09-10, merge `4f47875376683f922c99adc7d7eb45a0736dc8a1`). Seat: **Range Tech** owns 1P loft DNA + tracer streak + AIM TUNE leftover stash chrome. Consumes Beabim leftover/net (**#160**). Does **not** fork KIND_SHOT / leftover collide / HOST shared solve. House `BALLISTICS_SPINE` cook **A**. Tip order **#160 → #161** complete. CREDITS + STEAL_MAP + `BALLISTICS_A_DIAL_SHEET` Range section already claimed in-PR. Canonical dial sheet: fulcrumRust [`docs/BALLISTICS_A_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/BALLISTICS_A_DIAL_SHEET.md) Range half. Overnight cooks steal these numbers from the sheet — not every `.rs`. Do **not** invent dials. Do **not** invent powder numbers.
...
| **#160 leftover** | Default `ballistic_A` · hitscan stash · eye **1.60** · **500 m** / `SMG_PELLET` **14** · arc chord **0.17 m** **held** |

**Still local / other seat (deliberately):** powder A/B/C **later landed Powder B #166** (A restoreable · C parked) · KIND_SHOT encode / leftover collide / HOST shared solve (Beabim #160) · Hypha STREAM / PeerBody mesh · Lab-Rat stamps · Augury chrome · #147 500 m / eye 1.60 / hub protect / `KIND_LOOT_WORLD` · no-pause / `fulcrum://`. Do **not** claim Beabim / Hypha / Lab-Rat / Augury shipped this Range half.

See `PEEK_FINDINGS.md` Closed by #161 + Closed by #166 + fulcrumRust `docs/BALLISTICS_A_DIAL_SHEET.md` + house `BALLISTICS_SPINE.md`.

## Powder B slightly hot service (Range Tech — landed #166)

Evan lock. **Shipped** [fulcrumRust #166](https://github.com/initialvisuals/fulcrumRust/pull/166) (2026-09-10, merge `ca11b14349af448ec0ae782f97eccdbbbbc346c8`). Seat: **Range Tech** owns rifle powder tip. Consumes leftover/net (**#160**) + loft DNA (**#161**). Does **not** fork KIND_SHOT / leftover collide / HOST shared solve / AIM TUNE `leftover_hit`. House `BALLISTICS_SPINE` leftover cook **A** held. Rifle powder tip is **B**. CREDITS + STEAL_MAP + `BALLISTICS_A_DIAL_SHEET` Powder B table already claimed in-PR. Canonical dial sheet: fulcrumRust [`docs/BALLISTICS_A_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/BALLISTICS_A_DIAL_SHEET.md) Powder B tip. Overnight cooks steal these numbers from the sheet — not every `.rs`. Do **not** invent dials. Do **not** invent Powder C numbers.
...
| **C** | Friend-peek arcade | — | Parked. No kit numbers. `rifle_muzzle_speed() = None` (no-op apply) |
...
| **M24 muzzle / g / zero** | 810 / 9.8 / 100 m | held (already B) |
| **M24 vacuum drop @ 420 / 500 m** | 1.317 / 1.867 m | held |
| **MP9-Z muzzle / g** | **300** / **14.0** | held (not rifle powder) |

Drop is the loft vacuum `0.5 · g · (range/speed)²` (`feel::vacuum_drop_m`). No drag / Cd / sample dt. No persist key. Not `leftover_hit`.

**Still local / other seat (deliberately):** Powder **C** numbers · KIND_SHOT encode / leftover collide / HOST shared solve (Beabim #160) · AIM TUNE leftover_hit (`hitscan` / `ballistic_A`) · Hypha STREAM / PeerBody mesh / PreferredHand · Lab-Rat stamps · Augury chrome · #147 500 m / eye 1.60 / hub protect / `KIND_LOOT_WORLD` · no-pause / `fulcrum://`. READY HIP **#165** / Graphics **#164** / PreferredHand **#168** already landed — do not restamp those rows here. Do **not** claim Beabim / Hypha / Lab-Rat / Augury shipped this Range powder.

See `PEEK_FINDINGS.md` Closed by #166 + fulcrumRust `docs/BALLISTICS_A_DIAL_SHEET.md` + house `BALLISTICS_SPINE.md`.

## Options mouse V/H + hip/ADS (Range Tech — landed #163)

Evan lock. **Shipped** [fulcrumRust #163](https://github.com/initialvisuals/fulcrumRust/pull/163) (2026-09-10, merge `96e4410e`). Seat: **Range Tech** owns Controls mouse V/H + hip/ADS on the existing Augury/Hypha Options pane. House `OPTIONS_SHELF` Camera / feel seat. Feel-sheet ADS weight **0.86** + blend **6.4** stay DNA — Options ADS is a user mul on top, default **1.00**. CREDITS + STEAL_MAP + `OPTIONS_MOUSE_SENS_DIAL_SHEET` already claimed in-PR. Canonical dial sheet: fulcrumRust [`docs/OPTIONS_MOUSE_SENS_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/OPTIONS_MOUSE_SENS_DIAL_SHEET.md). House pointer: [`OPTIONS_MOUSE_SENS.md`](OPTIONS_MOUSE_SENS.md). Overnight cooks steal these numbers from the sheet — not every `.rs`. Do **not** invent dials.

| Dial | Lock |
|------|------|
| **MOUSE H** | `look_mul_h` **1.00** · `LOOK_MUL` **0.25–2.00** / step **0.05**. Yaw |
| **MOUSE V** | `look_mul_v` **1.00** · same lock. Pitch. Invert Y stays a toggle. #51 dizzy-play invert X stays LAST_PASS |
| **HIP** | `hip_look_mul` **1.00** · same lock. Non-ADS look |
| **ADS** | `ads_look_mul` **1.00** · same lock. **Not** feel `AdsHipDials.ads_look_mul` **0.86** |
| **Feel ADS / blend** | **0.86** / **6.4** held. Per-kit ergo still scales blend |
| **Feel look_sens** | `MoveDials.look_sens` **0.0022** held. Options muls sit on top |
| **Legacy** | Old `project.json` `"look_mul"` seeds both mouse axes when `look_mul_h` / `look_mul_v` are absent. Split keys win when present |
| **Live scale** | `yaw = look_dx × H × lerp(hip, ads, ads_factor) × feel_look_sens × fov_ads_scale` · same for pitch with V. Default all-1.00 ⇒ old single LOOK **1.00** |

**Still local / other seat (deliberately):** Hypha res / FOV / AA / AO / POST **later landed #164** · WARP / STREAM · Lab-Rat texture / mip **later landed #172** · Augury remap / Tab inventory · PreferredHand A chrome **later landed #168** · AIM TUNE PX box · stamps · death chrome. Options READY HIP **later landed #165**. Powder **B later landed #166**. AIM TUNE LIVE **#167** already landed (sibling). Do **not** invent implementation. Do **not** claim Hypha / Lab-Rat / Augury / Beabim shipped this.

See `PEEK_FINDINGS.md` Closed by #163 + house `OPTIONS_SHELF.md` + fulcrumRust `docs/OPTIONS_MOUSE_SENS_DIAL_SHEET.md`.

## AIM TUNE LIVE per-kit persist (Range Tech — landed #167 + #169)

Evan lock. **Shipped** [fulcrumRust #167](https://github.com/initialvisuals/fulcrumRust/pull/167) (2026-09-10, merge `aef44571`) + two-exe persist [fulcrumRust #169](https://github.com/initialvisuals/fulcrumRust/pull/169) (2026-09-10, merge `ea342444`). Seat: **Range Tech** owns End LIVE persist follow-up so three-gun peeks survive relaunch, then HOST/peer path + flush. #103 schema stays (`aim_live` + `aim_tune.*` — no new keys). Hideout and raid share one Hypha `project.json` (`FULCRUM_SETTINGS` if set, else cwd; resolved **absolute**). CREDITS + STEAL_MAP + `AIM_TUNE_LIVE_SAVE` already claimed in-PR. Canonical dial sheet: fulcrumRust [`docs/AIM_TUNE_LIVE_SAVE.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/AIM_TUNE_LIVE_SAVE.md) (merge tip `ea342444`). House pointer: [`AIM_TUNE_LIVE_SAVE.md`](AIM_TUNE_LIVE_SAVE.md). Overnight cooks steal from the sheet — not every `.rs`. Do **not** invent dials.
...
**Still local / other seat (deliberately):** End PX ±0.50 (#159) · Options `look_mul` (#163) · Beabim leftover stash · PreferredHand · powder **B later landed #166** (C parked) · Options READY HIP **later landed #165** · Hypha Graphics guts · Lab-Rat stamps · Augury Home. **Two-instance (HOST + peer) — landed #169.** Path split: default persist is **cwd** `project.json` (absolute-resolved). `build-and-run.bat` cds to the repo, so two bats share one file. Double-click `target\release\app.exe` writes a **different** file. Same-machine share: set **`FULCRUM_SETTINGS`** to one absolute path on both, or launch both from the same folder. Flush: AIM TUNE is local Range, not KIND_POSE. LIVE / Options / debugger save absorbs undirty kits (`aim_epoch`). Same-kit last writer wins. `project.json.lock` serializes the read-merge-write. Instances do **not** hot-reload the other window. Two-instance is **not** a soft follow.

**Northstar (Evan 2026-09-10).** AIM TUNE LIVE = **authoring only**. Bake peeks into kit DNA for all players when banks feel right — **not** a per-player profile. Do **not** invent a player-profile persist path. Do **not** claim the bake-into-kit-DNA cook shipped. #103 / #167 / #169 stay atelier LIVE persist.

**Still local / other seat (deliberately):** End PX ±0.50 (#159) · Options `look_mul` (#163) · Beabim leftover stash · PreferredHand · powder · Options hip Chest/Low · Hypha Graphics guts · Lab-Rat stamps · Augury Home. Do **not** claim Hypha / Lab-Rat / Augury / Beabim shipped this.

See `PEEK_FINDINGS.md` Closed by #167 + Closed by #169 + house `AIM_TUNE_LIVE_SAVE.md` + fulcrumRust `docs/AIM_TUNE_LIVE_SAVE.md`.

## Options Graphics RES / FOV / AA / AO / POST (Hypha — landed #164)

Evan lock. **Shipped** [fulcrumRust #164](https://github.com/initialvisuals/fulcrumRust/pull/164) (2026-09-10, merge `159b3d11`). Seat: **Hypha** owns Resolution, FOV hip base, AA type/strength, AO quality, post-processing quality, Graphics pane wiring on the existing Augury #45 overlay. House `OPTIONS_SHELF` Resolution / Graphics / FOV seat. CREDITS + STEAL_MAP + `OPTIONS_GRAPHICS_DIAL_SHEET` already claimed in-PR. Canonical dial sheet: fulcrumRust [`docs/OPTIONS_GRAPHICS_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/OPTIONS_GRAPHICS_DIAL_SHEET.md). House pointer: [`OPTIONS_GRAPHICS.md`](OPTIONS_GRAPHICS.md). Overnight cooks steal these numbers from the sheet — not every `.rs`. Do **not** invent dials.

| Dial | Lock |
|------|------|
| **RES** | Default **NATIVE** (desktop native — no invented pair). Presets: 1280×720 · 1920×1080 · UW 2560×1080 · 2560×1440 · 3840×2160 4K · **UW 4K CLASS** (3840×1600 class) · **3440×1440** (Evan) · Steam survey fill **2560×1600 / 1920×1200 / 1366×768** (July 2026 primary display). Borderless stays desktop-native. Persist `resolution` |
| **FOV** | Hip **base** default **90**. ADS still uses optic locks (iron/holo **60** · acog **25** · canted **60** CQC · scope **10**). Feel-sheet FOVs are not overwritten. Pane 70–110 / step 1 is Hypha chrome — **not** tip-locked. Persist `fov_hip` |
| **AA** | Type **FXAA / OFF**. Default FXAA. TAA later. Persist `aa_type` |
| **AA STR** | **0–1** on the live #55 mix. Default **0.5** = pass knob. **Not** CA **0.35**. Persist `aa_strength` |
| **AO** | **OFF / LOW / HIGH**. Default Off. Low = live #55 8-tap · r **0.55**. High = Mycelium **24**-tap, same 0.55 radius. No invented radius. Persist `ao_quality` |
| **POST** | **OFF / LOW / HIGH**. Default **HIGH**. Off skips Options post. Low = AA only. Heat #66 + wound #143 stay. Persist `post_quality` |
| **Compat** | Old `"ao"` / `"aa"` bools still load (true → AO Low / AA FXAA). New keys win when present |

**Still local / other seat (deliberately):** Lab-Rat texture / mipmap **later landed #172** · Range mouse V/H + hip/ADS (**#163**) · Augury remaps + Tab · STREAM `StreamPolicy` · Powder **B later landed #166** · PreferredHand / ballistics leftover / AIM TUNE · Beabim net · Bloom / godRays / brightness / gamma (**#86** no path). READY HIP **later landed #165**. AIM TUNE LIVE **#167** is Range parallel. Do **not** claim Range / Lab-Rat / Augury / Beabim shipped this.

See `PEEK_FINDINGS.md` Closed by #164 + house `OPTIONS_SHELF.md` + fulcrumRust `docs/OPTIONS_GRAPHICS_DIAL_SHEET.md`.

## Options TEXTURE + MIPMAP (Lab-Rat — landed #172)

Evan lock. **Shipped** [fulcrumRust #172](https://github.com/initialvisuals/fulcrumRust/pull/172) (2026-09-10, merge `3ca49f27`). Seat: **Lab-Rat** owns Graphics **TEXTURE** / **MIPMAP** (#114 seat) on the existing Hypha pane. House `OPTIONS_SHELF` Graphics seat. CREDITS + STEAL_MAP + `OPTIONS_TEXTURE_DIAL_SHEET` already claimed in-PR. Canonical dial sheet: fulcrumRust [`docs/OPTIONS_TEXTURE_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/OPTIONS_TEXTURE_DIAL_SHEET.md). House pointer: [`OPTIONS_TEXTURE.md`](OPTIONS_TEXTURE.md). Overnight cooks steal these numbers from the sheet — not every `.rs`. Do **not** invent dials.

| Dial | Lock |
|------|------|
| **TEXTURE** | Persist `texture_quality`. Default **HIGH**. LOW 16² · MED 64² · HIGH 256² · ULTRA 256². Vendor COL / PBR shelf downsample of the existing thumb. Ultra === High — tip already ships 256² (`pbr=vendor`). Not atelier 3K |
| **MIPMAP** | Persist `mipmap_quality`. Default **MED**. LOW +1 ring · MED identity · HIGH −1 · ULTRA −2. Bias sits **after** `promote_for_uv` / `MATERIAL_HOLD_M`. High ≈ `FULCRUM_UV` scale 2; Ultra ≈ scale 4 (lod-2 bridge → near 256²; lod-3 horizon → mid 64²). Height / mesh lod / chunk metres **16** stay |
| **#152 held** | `STABLE_LOCUS_RES` **16** · `MATERIAL_HOLD_M` **2.5** · `LUMA_GRAIN` ±**0.08**. Height / DISP stay bilinear |
| **Apply** | Building solids sample COL live (`grade_solid`). Terrain verts + desert skirt **regrade** on change (recolor in place — no remesh, no STREAM radius) |
| **Env** | `FULCRUM_TEXTURE` / `FULCRUM_MIPMAP` override Options at boot (same pattern as `FULCRUM_STREAM`) |
| **Smoke** | `tex=high/256` on the `pbr=` tag · `mip=med` on `grit_mips=` |
| **Where** | Options → GRAPHICS (title or HOLD). Rows sit under **STREAM**. A/D or Enter cycles |

**Still local / other seat (deliberately):** Hypha res / FOV / AA / AO / post / fog / cam / WARP / STREAM · Range mouse V/H · hip/ADS · READY HIP · powder · AIM TUNE · Augury remap / Tab · Beabim net · Soft-rim / death chrome · 4k atlases / new STREAM rings. Stamp pad **7×7**. Do **not** claim Hypha / Range / Augury / Beabim shipped this.

See `PEEK_FINDINGS.md` Closed by #172 + house `OPTIONS_SHELF.md` + fulcrumRust `docs/OPTIONS_TEXTURE_DIAL_SHEET.md`.

## PreferredHand A hold-bone mirror + HAND L/R chrome (Range Tech — landed #168)

Evan lock. **Shipped** [fulcrumRust #168](https://github.com/initialvisuals/fulcrumRust/pull/168) (2026-09-10, merge `1849ce6b`). Seat: **Range Tech** owns one authored (Right) pose bank on the hold bone + sagittal HAND L/R mirror. House `HOLD_POSE_SPINE`. CREDITS + STEAL_MAP + `PREFERRED_HAND_A_DIAL_SHEET` already claimed in-PR. Canonical dial sheet: fulcrumRust [`docs/PREFERRED_HAND_A_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/PREFERRED_HAND_A_DIAL_SHEET.md). House pointer: [`PREFERRED_HAND_A.md`](PREFERRED_HAND_A.md). Overnight cooks steal these numbers from the sheet — not every `.rs`. Do **not** invent dials.
...
| **Mirror A** | One right bank. Left = **−X / −yaw / −roll** (cant **0.785** → **−0.785**). Pitch / Y / Z stay. **No** `scale.x = −1` |
| **H** | Live HAND flip. PreferredHand onboard (#116) seats the home |
| **AIM TUNE** | Shows **HAND · kit · pose** (`HAND R  MP9-Z  HIP`). Left numbers are the mirror. WEAPON +X while HAND L writes authored −X so the gun still walks camera-right. End PX **±0.50** held |
...
| **Leftover H dest** | `shoulder_cross_*` / `shoulder_x_min` stay DNA for End leftover-wall arith (~**+0.226**) — **not** applied to the live left hold |

**Still local / other seat (deliberately):** Powder **B later landed #166** (A restoreable · C parked) · READY HIP persist rows (do **not** restamp) · AIM TUNE LIVE **#167** already landed (sibling persist) · Options mouse V/H + hip/ADS · Hypha 3P lean boxes · Lab-Rat stamps · Augury Home / glasses elbows / death chrome. Play U stays two poses (chosen hip ↔ canted) without adding READY HIP dials. Do **not** claim Hypha / Lab-Rat / Augury / Beabim shipped this.

See `PEEK_FINDINGS.md` Closed by #168 + house `HOLD_POSE_SPINE.md` + fulcrumRust `docs/PREFERRED_HAND_A_DIAL_SHEET.md`.

## MP weapon honesty + hideout hub PeerBodies (Beabim — landed #153)

Evan lock. **Shipped** [fulcrumRust #153](https://github.com/initialvisuals/fulcrumRust/pull/153) (2026-09-10, merge `ecd9db82`). Seat: **Beabim** owns two-instance listen-server weapon honesty + hideout-hub leftover. Extends #102 loot trail + #91 hip stub + #141/#147 PVP leftover. Soft names from house `HIDEOUT_HUB.md` (Clerk [#107](https://github.com/initialvisuals/house-docs/pull/107)). Floorplan metres **still pending** — do **not** invent hall / wing / ceiling. House lock **1P≠3P** held. No-pause held. Server browser parked. CREDITS + MP_WEAPON_DIAL_SHEET + STEAL already claimed in-PR. In-repo dial sheet: fulcrumRust [`docs/MP_WEAPON_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/MP_WEAPON_DIAL_SHEET.md) — do **not** invent dials.

| Dial | Lock |
|------|------|
| **`KIND_LOOT`** | **8** — Drop / Take / Sync verb. Trail crumbs on Drop/Take only |
| **`KIND_LOOT_WORLD`** | **15** — packed host leftover. `n` + up to **8** events (`WORLD_DROP_CAP`). HELLO + ~4 Hz (`LOOT_WORLD_SECS` **0.25**) |
| **Throw vel** | `LootEvent.vel` bytes 44–55. Both peers simulate the same toss. Old 48-byte leftovers decode vel **0** (rest immediately) |
| **Mag / reserves** | `in_mag` / `reserve_mags` / `mag_size` from `KitLoadout::snapshot` at drop. **No magic refill** |
| **Attachments** | `optic` + `suppressor` + fire-mode bit. Pickup applies that snapshot |
| **`RECV_BYTES`** | **1024** (was 256 — packed 8-drop snapshot is ~489 bytes) |
| **Held kit advert** | POSE unused **byte 3** — `weapon` 0–1 · `optic` 2–3 · can bit 4 · empty bit 5. Old packets = MP9 iron, hands full |
| **`GUN_HOLD_LOCAL`** | **(0.18, 1.05, 0.28)** — same metres as `PeerPose::reconstructed_gun`. Rides lean via `presented_local` |
| **3P mesh** | `kit_boxes(weapon, optic, can)` identity attachments. Not `kit_boxes_tuned` / 1P cant |
| **Hideout PeerBody** | `locus_solids` / `draws_actor_solids` — starting-room remotes draw the same boxes + held clone as extract |
| **Hub no-loot** | Hideout downs / hits do **not** drop gear or a `KIND_BODY` bag. **Z** throw stays a player drop |
| **`HUB_PROTECT_SECS`** | **15**. KIND_PVP unused bytes **22–23** (`protect_ms`). Leftover ray + Hit apply gated until the window ends. Not `PVP_RESPAWN_SECS` **1.20**. Raid / extract stays normal |
| **1P ≠ 3P** | Held. 3P is a readable authored clone on the posed hip-hand — not a copy of the viewmodel |
| **No-pause / KIND_RAID** | #122 unchanged |
| **HOST board / leftover ray** | #139 / #147 unchanged |

**Still local / other seat (deliberately):** Range 1P viewmodel / AIM TUNE / HoB / heat / Music / powder A/B/C · Hypha STREAM / Transvoxel / PeerBody box table (consume `presented_local` / `GUN_HOLD_LOCAL` only) · Lab-Rat stamps / volume metres · Augury death / Slain / crisp overlay / hatch / Home · hideout floorplan metres · Mixamo / hands / gear · server browser. Do **not** claim Range / Hypha / Lab-Rat / Augury shipped this.

See `PEEK_FINDINGS.md` Closed by #153 + fulcrumRust `docs/MP_WEAPON_DIAL_SHEET.md` + house `HIDEOUT_HUB.md`.
...
| **Augury** | **Death / Slain landed #146 + #162**. Do **not** claim #143 shipped slain chrome |
...
See `PEEK_FINDINGS.md` Closed by #143 + fulcrumRust `docs/WOUND_FEEL_DIAL_SHEET.md`. Death / Slain: Closed by #146 + #162.

## Death / Slain chrome (Augury — landed #146 + #162)

Evan lock. **Shipped** [fulcrumRust #146](https://github.com/initialvisuals/fulcrumRust/pull/146) (2026-09-10, `60a88648`) + overlay polish [fulcrumRust #162](https://github.com/initialvisuals/fulcrumRust/pull/162) (2026-09-10, `46fb2968`). Seat: **The Augury** owns glasses + death overlay only. PvE bleed-out envelope → Slain plate → PRESS SPACE after `SLAIN_INTERRUPT_SECS` **5.0**. Does **not** change Beabim HOST radio / hitbox math / 1.20s PVP leftover / #153 hub protect / no-loot, or Range live-world jostle (#143). Soft-rim #156/#158 left alone (no Slain plate). CREDITS + DEATH_DIAL_SHEET + STEAL already claimed in-PR. Canonical dial sheet: fulcrumRust [`docs/DEATH_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/DEATH_DIAL_SHEET.md). Overnight cooks steal these numbers from the sheet — not every `.rs`. Do **not** invent dials.
...
| **Prompt** | `PROMPT_FADE_SECS` **0.60** | White mono **PRESS SPACE TO RESPAWN**. Space after `SLAIN_INTERRUPT_SECS` **5.0** → existing `claim_place_spawn` / hideout / rim (#141 pads) |

Hit flash: `HIT_FLASH_SECS` **0.18**, wash **0.20**. Hit leftover `HIT_GLITCH` **0.18** (same band/chunk/chroma DNA as Slain; decays with `HIT_FLASH_SECS`; Range #143 shake + red wash held). Downed wash **0.48** (stim still readable). Downed HP/AR `DOWN_BAR` **[0, 0, 0, 1]** full-width black. Play HUD strip while downed or Slain live (inventory / crosshair / heat / tuner / extras off; overlay keeps wash + black bars + down-loop glasses). Space / continue gate `SLAIN_INTERRUPT_SECS` **5.0** — illegal before 5.0 s, legal after; full red→black→beat→slain.jpg→PRESS SPACE still plays.
...
PVP down does **not** play Slain. Beabim 1.20s leftover auto-respawn is unchanged. #153 hub protect / no-loot untouched.

| Seat | Owns |
|------|------|
| **Augury** | Overlay wash, Slain plate, glasses glitch, bar shake/flash/trail, downed HUD strip. Space after `SLAIN_INTERRUPT_SECS` **5.0** → existing respawn plant |
| **Range Tech** | Live hit-react (camera jostle / world blur) — **#143**. Untouched |
| **Beabim** | 1.20s PVP leftover / KIND_PVP / hitbox / #153 hub protect / no-loot — **untouched** |

**Still local / other seat (deliberately):** Range live jostle · Beabim HOST / KIND_PVP leftover / #153 hub protect · Lab-Rat stamps · Hypha 4× / landmark · soft-rim #156/#158 (no Slain plate). Do **not** claim Range / Hypha / Lab-Rat / Beabim shipped this.

See `PEEK_FINDINGS.md` Closed by #146 + #162 + fulcrumRust `docs/DEATH_DIAL_SHEET.md`.
...
- Augury glasses EXTRACT chrome **landed #85** (elbow card, no popup). Death/Slain chrome **landed #146** + **#162** (`HIT_GLITCH` **0.18** · `DOWN_BAR` full black · play HUD strip · `SLAIN_INTERRUPT_SECS` **5.0**). Soft rim leftover **landed #156** (`SIGNAL  THINS` / `CARRIER  LOST` + FX `Slot::Crackle`; Hypha `RimHook` physics read-only; thin hold **#158** spring-stop `approaching && t >= 0.995` stays BLOCK). CE Home debugger **landed #110**. Home LOGS COPY + tracker toggles **landed #121**. Home occlusion + 3D probes + COLL/PERF/SPWN **landed #125**. Hatch toggle + shaft ride **landed #115**. Door / extract cancel chrome **landed #120**. Timed surface kill still **open**. Hypha stream hitch *fix* **landed #108**. Worker/paint/gpu deepen **landed #123**. Residual soft LOD pop / live octree still parked. CE PERF/PHYS/RENDER/heartbeat/combat-log flags — house-docs steal later- Growth PoCs after window exists
...
Beabim PVP leftover ray (Evan sniper peek: peers visible ~700 m decal while wound died at **80 m**; `LEFTOVER_HIT_M` / `first_leftover_hit` **500 m** was 80; eye-centered `PeerBody` + #141 hurt skin / sweep / inclusive slab held; flat `SMG_PELLET` **14** — no invented falloff; Locus yard `apply_shot` keeps its own **80 m** helper; Range feel / zero / HoB / falloff / Hypha STREAM / Augury death / Lab-Rat stamps untouched; house lock **1P≠3P** held; no-pause held; hideout hub leftover **later landed #153**; ballistic leftover / `KIND_SHOT` loft **later landed #160**): fulcrumRust PR #147 (2026-09-10) — **landed**. `9a9853d4`; merge tip `0b90b2ef`. See `PEEK_FINDINGS.md` Closed by #147 + fulcrumRust `docs/PVP_DIAL_SHEET.md`.
Beabim KIND_SHOT loft + ballistic_A leftover (hitscan stash; `KIND_SHOT.dir` = HOST launch loft from `muzzle_and_launch` / `hip_honest` / MOA — not look-dir; default leftover `ballistic_A` via `feel::step_ballistic` same `vel.y -= g·dt` as peer streak; stash restore `FULCRUM_LEFTOVER` / `leftover_hit` / `Session::set_leftover_hit_model`; leftover eye **1.60** re-solves loft via `solve_ballistic_launch` + kit zero — do not reuse 1P muzzle HoB pitch; **500 m** / flat `SMG_PELLET` **14** held; arc chord `HEAD_HALF_H + HURT_SKIN` **0.17 m** — not a powder dt; shared helpers `feel::solve_ballistic_launch` · `feel::step_ballistic` · `biped::leftover_launch_dir`; no KIND bump; unused byte 3 stays; powder A/B/C / Hypha STREAM / Augury / hub protect / `KIND_LOOT_WORLD` / Locus yard 80 m left alone; Range loft DNA + AIM TUNE stash **later landed #161**): fulcrumRust PR #160 (2026-09-10) — **landed**. Merge `e0803dc1967418fef7bdd7e04a31c2a3b2f681ed`. See `PEEK_FINDINGS.md` Closed by #160 + fulcrumRust `docs/BALLISTICS_A_DIAL_SHEET.md` + house `BALLISTICS_SPINE.md`.
Range Tech loft DNA + AIM TUNE leftover stash (consume Beabim #160; 1P `muzzle_and_launch` + `hip_honest_dir` + MOA is `KIND_SHOT.dir`; tracer streak steals `feel::step_ballistic` so local/peer follow the lofted `ShotEvent`; hitscan leftover stash does **not** flatten the streak; End **MODEL** writes `leftover_hit` (`hitscan` / `ballistic_A`); TELE/PERF `LEFTOVER` reads the same enum; no `NetShot.loft` / no 48-byte KIND / no `ballistic_model`; powder A/B/C parked at ship — Powder **B later landed #166**; KIND_SHOT encode / leftover collide / HOST shared solve stay #160): fulcrumRust PR #161 (2026-09-10) — **landed**. Merge `4f47875376683f922c99adc7d7eb45a0736dc8a1`. See `PEEK_FINDINGS.md` Closed by #161 + Closed by #166 + fulcrumRust `docs/BALLISTICS_A_DIAL_SHEET.md` + house `BALLISTICS_SPINE.md`.
Range Tech Powder B slightly hot service (SR-25 muzzle **785 → 810** steal M24; ½v² 616225 → 656100 **+6.47%**; gravity **9.8** / zero **100 m** / flat pellet **14** held; vacuum drop @ 420 m **1.403 → 1.317** · @ 500 m **1.988 → 1.867**; M24 already 810 / 9.8 / 100; MP9-Z 300 / 14.0 held; `PowderKind` **B** tip default; **A** restoreable via `apply_rifle_powder`; **C** arcade parked `rifle_muzzle_speed() = None`; drop = loft vacuum `0.5 · g · (range/speed)²`; no drag / Cd / persist key; leftover_hit stash untouched; KIND_SHOT encode / leftover collide / HOST shared solve stay #160): fulcrumRust PR #166 (2026-09-10) — **landed**. Merge `ca11b14349af448ec0ae782f97eccdbbbbc346c8`. See `PEEK_FINDINGS.md` Closed by #166 + fulcrumRust `docs/BALLISTICS_A_DIAL_SHEET.md` + house `BALLISTICS_SPINE.md`.
Range Tech Options mouse V/H + hip/ADS (Controls **MOUSE H** / **MOUSE V** / **HIP** / **ADS** on the existing #46 `LOOK_MUL` lock **1.00** / **0.25–2.00** / step **0.05**; persist `look_mul_h` / `look_mul_v` / `hip_look_mul` / `ads_look_mul`; old `look_mul` seeds both mouse axes when split keys are absent; feel-lab `look_sens` **0.0022** + feel ADS **0.86** / blend **6.4** stay DNA; Invert Y stays; Options READY HIP **#165** already landed (sibling). Powder **B later landed #166**; Hypha res/FOV/AA/AO/post/WARP/STREAM · Lab-Rat texture/mip · Augury remap/Tab · PreferredHand · AIM TUNE PX left alone): fulcrumRust PR #163 (2026-09-10) — **landed**. Merge `96e4410e`. See `PEEK_FINDINGS.md` Closed by #163 + house `OPTIONS_SHELF.md` + fulcrumRust `docs/OPTIONS_MOUSE_SENS_DIAL_SHEET.md`.
Range Tech AIM TUNE LIVE per-kit persist (Evan 2026-09-10 three-gun End peeks reset to authored/old on relaunch; same hideout+raid `project.json`; keys stay `aim_tune.example_*`; `pull_aim` is per-kit so an un-applied authored sheet cannot clobber the other guns; apply snaps `hold_blend`; schema `aim_live` + `aim_tune.*` unchanged; End PX ±0.50 / Options look_mul / leftover stash stay; two-instance path + flush **later landed #169**): fulcrumRust PR #167 (2026-09-10) — **landed**. Merge `aef44571`. See `PEEK_FINDINGS.md` Closed by #167 + house `AIM_TUNE_LIVE_SAVE.md` + fulcrumRust `docs/AIM_TUNE_LIVE_SAVE.md`.
Range Tech two-exe AIM TUNE persist (HOST/peer peeks split when the second instance wrote another cwd `project.json`, or when a LIVE/Options flush last-wrote the whole `aim_tune` blob; resolve `FULCRUM_SETTINGS` / cwd to an **absolute** path; flush absorb undirty kits from disk so one exe cannot wipe the other's per-kit peeks; `project.json.lock` serializes the read-merge-write; same-kit last writer wins; instances do **not** hot-reload the other window; schema `aim_live` + `aim_tune.*` unchanged; two-instance is **not** a soft follow; Evan 2026-09-10 northstar: LIVE = **authoring only** — bake peeks into kit DNA for all players when banks feel right, **not** a per-player profile; bake-into-DNA is **not** shipped): fulcrumRust PR #169 (2026-09-10) — **landed**. Merge `ea342444`. See `PEEK_FINDINGS.md` Closed by #169 + house `AIM_TUNE_LIVE_SAVE.md` + fulcrumRust `docs/AIM_TUNE_LIVE_SAVE.md`.
Hypha Options Graphics shelf (RES default **NATIVE** + Evan **3440×1440** + Steam survey **2560×1600 / 1920×1200 / 1366×768** · FOV hip base **90** · AA **FXAA/OFF** · AA STR live #55 mix **0.5** · AO **OFF/LOW/HIGH** Low 8-tap · r **0.55** / High **24**-tap same radius · POST **OFF/LOW/HIGH** default HIGH; persist `resolution` / `fov_hip` / `aa_type` / `aa_strength` / `ao_quality` / `post_quality`; old `ao`/`aa` bools still load; pane FOV 70–110 / step 1 is chrome — **not** tip-locked; WINDOW / CA / GRAIN / DOF / FOG / CAM / WARP / STREAM unchanged; Lab-Rat texture/mip · Range #163 · Augury remap/Tab · STREAM policy · Powder **B later landed #166**; AIM TUNE LIVE **#167** already landed): fulcrumRust PR #164 (2026-09-10) — **landed**. Merge `159b3d11`. See `PEEK_FINDINGS.md` Closed by #164 + house `OPTIONS_SHELF.md` + fulcrumRust `docs/OPTIONS_GRAPHICS_DIAL_SHEET.md`.
Range Tech PreferredHand A hold-bone mirror + HAND L/R chrome (**`HAND R` / `HAND L`** from live hold X — gun right of camera middle = HAND R, not inverted `Shoulder` enum; one right bank + sagittal **−X / −yaw / −roll** cant **0.785** → **−0.785**; pitch/Y/Z stay; **no** `scale.x = −1`; H is live HAND flip; #116 onboard seats home; AIM TUNE shows **HAND · kit · pose**; WEAPON +X while HAND L writes authored −X; crosshair hides `ads_factor` **> 0.5**; hip + U-cycle canted keep it; End PX **±0.50** held; leftover H dest ~**−0.041** not the live left hold; Powder **B later landed #166**; READY HIP **#165** already landed (sibling); AIM TUNE LIVE **#167** already landed): fulcrumRust PR #168 (2026-09-10) — **landed**. Merge `1849ce6b`. See `PEEK_FINDINGS.md` Closed by #168 + house `HOLD_POSE_SPINE.md` + fulcrumRust `docs/PREFERRED_HAND_A_DIAL_SHEET.md`.
Beabim MP weapon honesty + hideout hub PeerBodies (`KIND_LOOT_WORLD` **15** host snapshot HELLO + ~4 Hz so late join + one-shot UDP miss recover; throw vel on `LootEvent`; mag / reserves / optic / can from `KitLoadout::snapshot` — **no magic**; held 3P `kit_boxes` clone on `GUN_HOLD_LOCAL` `(0.18, 1.05, 0.28)` via `presented_local` — lean rides torso; hideout starting-room PeerBodies; hub no-loot downs; `HUB_PROTECT_SECS` **15** / `protect_ms` KIND_PVP bytes 22–23; raid leftover stays extract; house lock **1P≠3P** held; metres still pending; crisp flash chrome pending): fulcrumRust PR #153 (2026-09-10) — **landed**. Merge `ecd9db82`. See `PEEK_FINDINGS.md` Closed by #153 + fulcrumRust `docs/MP_WEAPON_DIAL_SHEET.md` + house `HIDEOUT_HUB.md`.
Range Tech 1P screen-react / wound feel (suppress-near soft short blur only; armour jostle + soft blur, **no red**; HP stronger jostle + blur + red fade; envelope `--===--------` ramp→front peak→tail; never full-strength blur; 1P local only; Hypha consumes `post.wound = [blur, red, 0, 0]`; hooks Beabim `KIND_PVP` Hit not Sync; Death/Slain **later landed #146** + **#162**; #141 leftover / AIM TUNE / heat lattice / stamps untouched): fulcrumRust PR #143 (2026-09-09) — **landed**. `ae09f5f8`. See `PEEK_FINDINGS.md` Closed by #143 + fulcrumRust `docs/WOUND_FEEL_DIAL_SHEET.md`. Death / Slain: Closed by #146 + #162.
...
Lab-Rat stamp / building / terrain PBR polish (rocks offset 3-lobe + shade/face/chip `Solid.grade = Rock`; buildings `Solid.grade = Concrete` + face UVs walls=vertical/lids=XZ; default `pbr=vendor` 256² COL thumbs for dirt/sand/rock/concrete/organic + concrete NRM; tiles concrete **3.2** · rock **3.6** · dirt **3.8** · sand **4.0** · organic **3.2**; grit **4.6 / 2.6 / 3.4**; COL mix `0.46 + 1.08·sample`; extra grit on building faces only; texture+UV only — chunk **16 m** held; Hypha STREAM / geo / remesh / 37×37 / leftover ray untouched; GPU NRM on `fs_world` parked): fulcrumRust PR #144 (2026-09-10) — **landed**. `de448bc`. See `PEEK_FINDINGS.md` Closed by #144 + fulcrumRust `docs/STAMP_PBR_DIAL_SHEET.md`. Grit/COL LOD luma crawl stop **later landed #152**.
Lab-Rat grit/COL LOD luma crawl stop (walking/zooming no longer reshuffles grit/COL dark spots at the same world XZ; `STABLE_LOCUS_RES` **16** · `MATERIAL_HOLD_M` **2.5** · `LUMA_GRAIN` ±**0.08**; height/DISP stay bilinear; chunk **16** held; extra grit still on building faces only; texture+UV / LOD sample stability only — off STREAM / extract / hold / remesh / landmark ride / pend): fulcrumRust PR #152 (2026-09-10) — **landed**. `a86c1ed6`. See `PEEK_FINDINGS.md` Closed by #152 + fulcrumRust `docs/STAMP_PBR_DIAL_SHEET.md` (LOD luma crawl row) + `docs/CHANNELS.md` + `docs/TERRAIN.md`. Desert skirt vendor sand COL **later landed #157**.
Lab-Rat desert skirt vendor sand COL (Hypha #151 height-only skirt read as bare wash — yard sand **4.0** aliased old **8 m** quads onto one COL texel; steal existing `pbr_sand_col` thumb `pbr=vendor`; desert tile **2.8 m**; skirt quads **8→4 m**; wear = far `lod_mips` 16² grit `stamp_wear_scale_lod` + `edge_wear`; material stays `Sand`; height stays `DESERT_FLOOR_Y`; no rock lobes; no `grit_modulate` on Transvoxel verts; smoke `pbr=vendor` `desert=8/col=2.8`; off STREAM / RimHook / remesh / chunk **16** / Range powder / Beabim KIND / Augury glasses): fulcrumRust PR #157 (2026-09-10) — **landed**. `f6cd2586`; merge tip `8431bc25`. See `PEEK_FINDINGS.md` Closed by #157 + fulcrumRust `docs/STAMP_PBR_DIAL_SHEET.md` / `docs/TERRAIN.md` / `docs/STAMPS.md`.
Augury CE Slain death flow + glasses chrome (PvE bleed-out envelope Red `DEATH_RED_SECS` **0.55** wash 0.78 · FadeBlack **0.70** · Beat **1.00** · Slain `SLAIN_FADE_SECS` **0.78** plate `assets/images/ui/deathscreens/slain.jpg` · Prompt **0.60** PRESS SPACE after `SLAIN_INTERRUPT_SECS` **5.0** → existing `claim_place_spawn` / #141 pads; hit leftover `HIT_GLITCH` **0.18** · `DOWN_BAR` full black · play HUD strip; hit flash 0.18/0.20 · downed wash 0.48; delayed HP/AR trail default on `BAR_CATCH` **6.5**; `FULCRUM_BAR_DELAY=0/off` snaps; glitch rides envelope `GLITCH_PROMPT_FLOOR` **0.12**; PVP down does **not** play Slain; Beabim 1.20s leftover / #153 hub protect untouched; Range live jostle / HOST / hitbox / Hypha 4× / landmark / Lab-Rat stamps / soft-rim #156/#158 left alone): fulcrumRust PR #146 (2026-09-10) — **landed**. `60a88648`. Overlay polish **later landed #162**. See `PEEK_FINDINGS.md` Closed by #146 + #162 + fulcrumRust `docs/DEATH_DIAL_SHEET.md`.
Augury death/down overlay chrome (hit leftover `HIT_GLITCH` **0.18** same band/chunk/chroma DNA as Slain, decays with `HIT_FLASH_SECS`; Range #143 shake + red wash held; downed HP/AR `DOWN_BAR` **[0, 0, 0, 1]** full-width black; play HUD strip while downed or Slain live — inventory / crosshair / heat / tuner / extras off, overlay keeps wash + black bars + down-loop glasses; `SLAIN_INTERRUPT_SECS` **5.0** Space / continue illegal before 5.0 s, legal after; full red→black→beat→slain.jpg→PRESS SPACE still plays; #146 envelope held; Beabim #153 hub protect / no-loot / PVP 1.20s leftover untouched; soft-rim #156/#158 left alone): fulcrumRust PR #162 (2026-09-10) — **landed**. Merge `46fb2968`. See `PEEK_FINDINGS.md` Closed by #146 + #162 + fulcrumRust `docs/DEATH_DIAL_SHEET.md`.
...
Lab-Rat HOLO_MYCELIUM hideout leftover (cheap looping board-room hologram on hideout left desk; matrix **12×8×12** · max live **96** + 1 scanline slab · max tips **10** · update **10** Hz · sphere **0.010 / 0.022** m · tip purple `[0.68, 0.20, 0.86]` · body blue `[0.16, 0.42, 0.92]` · brightness **0.22 → 0.96** × scan **0.62** · scanline **0.55** Hz · cycle **9.0** s or starve · warm **22** · table AABB placeholder left desk; `FULCRUM_HOLO=0` kills; smoke `holo=gen=`; reuses extract pedon leftover buffers in hideout; **not** STREAM / remesh / #151 sand COL / KIND / PeerBody / Range powder / Augury Slain; no T·H·L invent): fulcrumRust PR #154 (2026-09-10) — **landed**. `d6b56a7e`. See `PEEK_FINDINGS.md` Closed by #154 + fulcrumRust `docs/HOLO_MYCELIUM_DIAL_SHEET.md`.
Lab-Rat Locus Voidspore yard leftover (cheap looping growth leftover left of extract spawn; dark / inked webbing + purple tips; matrix **10×4×10** · max live **48** · max tips **8** · max solids **152** · update **10** Hz · cell **0.016 / 0.034** m · pitch **0.085** m · life **36** · density gate **0.12** · lobe **0.42** · tip purple `[0.68, 0.20, 0.86]` · body ink `[0.10, 0.04, 0.12]` / void `[0.16, 0.05, 0.14]` · brightness **0.16 → 0.78** × grit rough · cycle **11.0** s or starve · warm **18** · origin XZ **−3.55, 2.35** · floor `terrain.height_at`; growth leftover GPU **768 / 192** KiB was 640 / 160; `FULCRUM_VOIDSPORE=0` kills; smoke `voidspore=gen=`; existing `upload_growth` leftover path; **not** a fourth plot / STREAM remesh / glasses / AI): fulcrumRust PR #171 (2026-09-10) — **landed**. Merge `ce4162a5`. See `PEEK_FINDINGS.md` Closed by #171 + house `VOIDSPORE.md` + fulcrumRust `docs/VOIDSPORE_DIAL_SHEET.md`.
Augury soft rim leftover (glasses `SIGNAL  THINS` / `CARRIER  LOST` + death-DNA glitch leftover `death::glitch_quads` / band shift / chroma + FX `Slot::Crackle` radio breakup; Hypha #151 `RimHook` / `apply_soft_rim` physics read-only `RIM_SOFT_M` **24** / world rim ~**422**; approach floor **0.08** → peak **0.28** · block **0.36** · chroma **0.012**; fade **0.55** s · crackle **0.42** s; mute under Slain; no Slain plate / Space / death cam; STREAM / desert / Lab-Rat COL / Range heat/wound/AIM / Beabim KIND / PreferredHand / LANDMARK_RIDE untouched): fulcrumRust PR #156 (2026-09-10) — **landed**. Merge `f7167202`. Thin hold follow-up #158 (2026-09-10, `f42e5f9`) — spring-stop `approaching && t >= 0.995` stays BLOCK / `CARRIER  LOST` / glitch **0.36** + crackle (Hypha `blocked` is crossing-frame only); squash onto the #156 shelf, not a second soft-rim landing. See `PEEK_FINDINGS.md` Closed by #156 + fulcrumRust `docs/SOFT_RIM_DIAL_SHEET.md`.
...
Hypha colorless muzzle heat (sample-only `heat_warp_uv`; lattice = post input only; no world-pipeline orange card): fulcrumRust PR #66 (2026-09-08) — **landed**. Range Tech keeps `barrel_energy` / heat dials / hold-J. Live card field is the **#129** CE tip **0.2.8** lock (#71 blend DNA). Visible hold-J warp **later landed #155**. See `PEEK_FINDINGS.md` Closed by #66 / Closed by #129 / Closed by #155.
...
Range Tech heat dials CE tip 0.2.8 lock (live HeatDials past #71 blend; haze **0.01** / size **0.99** / scaleX **0.28** / lobe **0.40**; post `visual * haze` at ship — **later sold #155**; radius **0.40 / 0.08**; tip/energy-weighted post UV; no fog blob; #66 colorless path stays; #89 untouched): fulcrumRust PR #129 (2026-09-09) — **landed**. `54c558a`. See `PEEK_FINDINGS.md` Closed by #129 + Closed by #155 + `heat-card-dial-sheet.md`.
Range Tech visible barrel heat warp at hold-J (`haze_strength` **0** off / **0.01** CE enable floor / **0.11** stolen max; post `visual × lattice × 1.35` at the floor · × **1.20** at 0.11; shader gate **0.001** was 0.01; UV scale **0.010**; radius **0.40 / 0.08**; tip cards `scale_x/y` **0.28/0.86** unchanged; Options **WARP** stays Hypha pixellation; #129 CE field / #66 colorless path / #89 projectile / AIM TUNE / #143 wound untouched): fulcrumRust PR #155 (2026-09-10) — **landed**. `d3d7848c`. See `PEEK_FINDINGS.md` Closed by #155 + `HEAT_WARP_DIAL_SHEET.md` + fulcrumRust `docs/HEAT_WARP_DIAL_SHEET.md`.
...
Range Tech AIM TUNE PX travel (`shoulder_x_max` **±0.50**; leftover +0.226 from `−shoulder_cross_x + shoulder_x_min` is **not** an End +X cap; `AimTuner::nudge` clamps PX only; PY/PZ/rot unclamped; H still floors at `shoulder_x_min` **−0.055**; authored hip **+0.2403** / H dest **~−0.041** / #103 live-save / `hold_spring` / ADS blend untouched): fulcrumRust PR #138 (2026-09-09) — **landed**. Merge `aa489abd`. Hideout wall follow-up **later landed #159**. See `PEEK_FINDINGS.md` Closed by #138 + Closed by #159 + fulcrumRust `docs/AIM_TUNE_X_DIAL_SHEET.md`.
Range Tech AIM TUNE PX hideout wall (`TUNE_POS_X_ABS` **0.50** floor even if `shoulder_x_max` is stale/zero; End WEAPON home-pose nudge snaps `hold_blend`; ATTACH same ±0.50 box; leftover +0.226 ≠ End box; hideout LIVE **+0.2258** Fine +X leaves / Coarse → **+0.50**; H `shoulder_x_min` floor stays; authored hip **+0.2403** / #103 schema / `hold_spring` U-cycle / ADS blend untouched; PreferredHand A chrome **later landed #168**; AIM TUNE LIVE **#167** already landed): fulcrumRust PR #159 (2026-09-10) — **landed**. Merge `5478db35`. See `PEEK_FINDINGS.md` Closed by #159 + fulcrumRust `docs/AIM_TUNE_X_DIAL_SHEET.md`.
...
Hypha play STREAM 11×11 + warm resident hold (radius + warm-return on #108/#123 hitch — not a new map size; `STREAM_RINGS` **5** / **11×11** was 4 / 9×9; leading edge 80 m; prefetch **5 m** face + walk heading reverse included + one cheap lod-3 look-ahead; GPU halo `HOLD_RINGS` **2** + CPU TTL `RESIDENT_TTL_SECS` **12** `hold=2/12`; return walk promotes/republishes no extract; flush/smoke still drop; cook=1/2 · cook_ms 8/16 · `defer=worker/paint/gpu` held; hole-fill heading-side first; halo-exit `hold_publish` keeps extract0 / gpu on separate ticks; TELE MAX ~30 ms; smoke `prefetch=5+heading` `hold=2/12` `defer=worker/paint/gpu`; Home LOGS `STREAM` still `resident=` / `cold=` / `pend=`; Lab-Rat 7×7 / Range / Beabim / Augury Home / #131 / #136 untouched; 4× world + pend coalesce **later landed #142** — STREAM held; live hold / horizon / desert / soft rim **later landed #151**): fulcrumRust PR #137 (2026-09-09) — **landed**. `4ea0c1e4`; follow-up `a26628a`. See `PEEK_FINDINGS.md` Closed by #137 / Closed by #151 + `TERRAIN_NORTHSTAR.md` + fulcrumRust `docs/TERRAIN.md`.
Hypha 4× world scale + pend coalesce (next ~4× area on the #81 host — not a STREAM radius bump; GRID **19×19 / 304 m / 92 416 m² → 37×37 / 592 m / 350 464 m²**; rings **9 → 18**; spawn stays origin; STREAM held 11×11 / r=5; prefetch 5+heading same; hold 2/12 same at ship (live **later landed #151** 4/16); cook/hitch 1/2 · 8/16 · defer=worker/paint/gpu same; **NEW** pend coalesce admit 2 / cap 2 `coalesce=2/2`; Home SPIKE `pendΔ ≥ 2`; walk-forward GPU concat appends hole-fills; rim **144 → 288 m** via `probes::rim_radius_m()` / `GRID_ORIGIN`; inside `playable_half_m` 295.25; stamp pad stays 7×7; Lab-Rat CHANNELS / pedon left alone; Range / Beabim net/PVP/HOST / Augury Home chrome left alone; smoke `extract_m2=350464` `rings=18` `prefetch=5+heading` `hold=2/12` `defer=worker/paint/gpu` `coalesce=2/2` `spawns=rim=8`): fulcrumRust PR #142 (2026-09-10) — **landed**. `d1eb09df`. See `PEEK_FINDINGS.md` Closed by #142 / Closed by #151 + `TERRAIN_NORTHSTAR.md` + fulcrumRust `docs/TERRAIN.md` / `docs/SPAWNS.md`.
Hypha STREAM past extract + desert floor + soft rim (horizon / exterior / hold / rim-hook on the #142 37×37 host — not a fine STREAM radius bump; fine window stays #137 **11×11** / r=5; cheap LOD-3 `visible=5/19` void→coarse floor; flat desert `desert=8` / 128 m heightfield + skirt — sand stamp hook / height only, no rock lobes, Lab-Rat COL later; hold **2/12 → 4/16**; `StreamPolicy` window/wide/resident via `FULCRUM_STREAM` or Options Graphics **STREAM** — wide = r=18 hold=8/30, resident = whole-extract fine + hold=18/inf RTX 3060 peek; soft rim extract+desert ~422 m damp outward vel / spring back / no clip slap; `RimHook` / `Debugger::note_rim` → `RIM APPROACH|BLOCK|CLEAR hook=augury` glasses glitch + radio crackle — do **not** invent Slain chrome; prefetch 5+heading · cook/hitch 1/2 · 8/16 · defer=worker/paint/gpu · coalesce=2/2 held; extract still 37×37 / 592 m / 350464 m²; stamp pad 7×7; smoke `hold=4/16` `stream=window` `visible=5/19` `desert=8` `coalesce=2/2` `defer=worker/paint/gpu` `extract_m2=350464`): fulcrumRust PR #151 (2026-09-10) — **landed**. Merge `0f6171cf`. See `PEEK_FINDINGS.md` Closed by #151 + `TERRAIN_NORTHSTAR.md` + fulcrumRust `docs/TERRAIN.md`.
...
LOD-tied grit / material mips (near 256² / mid 64² / far 16² BC4-style; far drops grain hashes; atelier read-only): fulcrumRust PR #60 (2026-09-07) — **landed**. Hypha. See `PEEK_FINDINGS.md` Closed by #60. Grit/COL LOD luma crawl stop **later landed #152**.
...
First big-map (drop walls · ~8× extend · chunked Transvoxel · slope COL · local-player stream): Evan dump (2026-09-08) / Hypha **#81 landed**. Stamp pad still 7×7. Stream hitch amortize **later landed #108**. Worker STREAM extract+paint **later landed #123**. Play STREAM 11×11 + warm hold **later landed #137**. 4× world + pend coalesce **later landed #142**. STREAM past extract / desert / soft rim **later landed #151**. Lab-Rat slope/PBR/dirt/scatter/deform plugs **landed #80**. NRM/GLOSS parked. Beabim peer feet `stream_anchors` **landed #83** (coordinate only). Range Tech kits+FX draw + `dBXpg` still **open**; kit PBR stub + music playlist **landed #64** / Augury FoW brand+menu video **when cut ready**. Atelier PBR batch **in** (150 roughness + textures/PBR ~26 sets). See `TERRAIN_NORTHSTAR.md` + `PEEK_FINDINGS.md` Closed by #81 / #108 / #123 / #137 / #142 / #151 / #80.
Hypha 19×19 open extract + chunk stream + slope COL hooks: fulcrumRust PR #81 (2026-09-09) — **landed**. `73dc8fe4`. Underfoot **32/16/8/4**. Smoke `subdivs=32/16/8/4` `extract_m2=92416` `resident=` `stream_cold=` `pbr=`. Stream hitch amortize **later landed #108**. Worker STREAM extract+paint **later landed #123**. Play STREAM 11×11 + warm hold **later landed #137**. 4× world + pend coalesce **later landed #142**. STREAM past extract / desert / soft rim **later landed #151**. Do **not** claim NRM-GLOSS GPU / world replication / Range heat as this PR. Lab-Rat plugs **landed later #80**. Peer feet `stream_anchors` **landed later #83**. See `PEEK_FINDINGS.md` Closed by #81.
...
Lab-Rat sandbox pedon (off-stream leftover CHANNELS slab; stick XZ `3.90, -0.55` · slab XZ `5.20, -0.55` · `TABLE_H` 1.02 m · reach 1.35 m · slab 1.90 m half-span / 0.38 m thick / 10×6×10 @ 0.18 m; tap **F** rebake; glasses `PEDON  F  REBAKE  GEN n`; seed = extract xor gen; `pedon` GPU overlay; `StampField::layers` / `stream_rev` stay cold; smoke `pedon=gen=1`; not Home chrome / not #127 / not #114 pad crawl; STREAM stays Hypha #123; hideout hologram leftover **later landed #154** — reuses extract pedon leftover buffers in hideout, places never overlap): fulcrumRust PR #130 (2026-09-09) — **landed**. `10a4c7b3`; tip `ef6dea41`. See `PEEK_FINDINGS.md` Closed by #130 + `STAMP_FEEL_LOCK.md` + fulcrumRust `docs/PEDON.md`. Hideout holo: Closed by #154.
...
Range Tech Options READY HIP + two-pose play (Gameplay **CHEST** tip default / **LOW HIP**; persist `default_hip` `chest` / `low`; aliases `chest`/`ready`/`hip` · `low`/`low_hip`/`hip_low`; missing = Chest; authored `hip` Y **−0.2128** / `hip_low` Y **−0.3528**; in-play **U** is chosen hip ↔ canted only; spawn/ready seats the pick; Hold-Mouse5 still `ads_cant`; shotgun `default_hip_override` hook only — day-one kits `None`; Controls mouse V/H + hip/ADS stay planted **#163** — do not restamp; PreferredHand chrome not invented; Powder **B later landed #166**; AIM TUNE LIVE **#167** already landed): fulcrumRust PR #165 (2026-09-10) — **landed**. Merge `9b8121b1`. See `PEEK_FINDINGS.md` Closed by #165 + house `OPTIONS_SHELF.md` + fulcrumRust `docs/OPTIONS_DEFAULT_HIP_DIAL_SHEET.md`.
