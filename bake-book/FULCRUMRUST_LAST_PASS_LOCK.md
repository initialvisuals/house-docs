...
- **O** (hold) = raid extract-check intent (`Session::extract_checking`; fulcrumRust #78). **Not** zero. Hideout is a no-op. Augury EXTRACT elbow card **landed #85** (no popup). Hatch toggle + shaft ride **landed #115**. Door / extract cancel chrome **landed #120** (`GATE_SECS` **2.20**; hold-O still intent only — countdown wins while armed). Timed surface kill still **~**. **P** = drop look-at probe (**landed #125**; CE **T** remapped — T is bandage). **#128** SPWN+P (or TELE Insert→SPAWN→P) writes `kind:spawn`. **#132** Lab-Rat bake consumes into `World.player_spawns` (Beabim owns pool — still open). Surface `name` is `ground` / … — not mesh id `extract`. Arcade↔sim stay dead (#76)
- **X** = prone
...
Evan lock. **Shipped** [fulcrumRust #132](https://github.com/initialvisuals/fulcrumRust/pull/132) (2026-09-09, `67a5a2bb`; merge tip `71c467ed`). **Lab-Rat** owns bake-time extract player spawn loci. Beabim owns the runtime pool (one-person-per-spawn, knock off until reset) — **not** this cook. #128 stays the Augury Home writer of `kind:spawn`. #127 stays CHANNELS dirt sit on the same sheet. **Not** PVP / hitboxes / respawn UI. **Not** biped eye / cam. **Not** a second MP KIND. Dial sheet: fulcrumRust `docs/SPAWNS.md`.
...
Do **not** claim Beabim knock-off / PVP / respawn UI / biped eye shipped. Hideout / empty hatch stubs keep an empty pool.

See `PEEK_FINDINGS.md` Closed by #132 + fulcrumRust `docs/SPAWNS.md`.
...
- **P** — unused after #76 for arcade↔sim (no new HoB bind). `FeelSheet::toggle_hob_zero` + session **P** apply gone. Then #33 arcade (aim-dir) ↔ sim (HoB + ballistic zero). **Later remapped #125** — drop look-at probe (CE **T** remapped; T is bandage). **#128** SPWN+P / Insert **SPAWN** writes `kind:spawn`; **#132** Lab-Rat bake consumes into `World.player_spawns` (Beabim owns pool — still open); surface `name` is `ground` / … — not mesh id `extract`
