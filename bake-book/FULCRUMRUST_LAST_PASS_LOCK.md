...
- **O** (hold) = raid extract-check intent (`Session::extract_checking`; fulcrumRust #78). **Not** zero. Hideout is a no-op. Augury EXTRACT elbow card **landed #85** (no popup). Hatch toggle + shaft ride **landed #115**; timed surface kill still **~**. **P** unused after #76 (no arcade↔sim; no new bind)
- **X** = prone
...
See `PEEK_FINDINGS.md` Closed by #59 / Closed by #84 / Closed by #94 / Closed by #98 / Closed by #99 / Closed by #100 / Closed by #109 / Closed by #113 + H shoulder-swap tilt (#84) + Patch A RH hip bias (#94) + RH body-width + ready-hip Y (#98) + low-hip shotgun stance (#99) + canted CQC ADS (#100) + U-cycle hold springs (#109) + kit handling / ergo / MOA (#113).
...
- **O** — then cycled those presets. **#78:** hold extract-check intent (`Session::extract_checking`). **Not** zero. Hideout is a no-op. Augury EXTRACT elbow card **landed #85** (no popup). Hatch toggle + shaft ride **landed #115**; timed surface kill still **~**
...
Evan lock. **Shipped** [fulcrumRust #78](https://github.com/initialvisuals/fulcrumRust/pull/78) (2026-09-09, `e86bfa79`). **Range Tech** owns hold-O raid intent, **−/=** zero, grounded slide gate. **The Augury** EXTRACT elbow card **landed #85** (no popup — not landed by #78). Hatch toggle + shaft ride **later landed #115**. Timed surface kill / extract loot loop still **~**. No heat / ballistics rewrite / Lab-Rat terrain. Ledger: hold-O check wired; glasses EXTRACT chrome is #85; ride/toggle is **X/#115**; timed kill stays `~`.

| Dial | Was | Now |
|------|-----|-----|
| **O** | cycle live zero 50 → 100 → 200 m (#33; stayed after #76) | **hold** raid extract-check intent (`Session::extract_checking`). **Not** zero. Hideout is a no-op. Augury EXTRACT elbow card **landed #85** (no popup). **#115** reads `OPEN` / `CLOSED` / `SHAFT` |
...
**#78** hold-O / −/= zero / grounded slide stays. **#76** SIM-only stays. Augury EXTRACT elbow card **landed #85** (no popup). Hatch toggle + shaft ride **later landed #115**. Do **not** claim a second land system or timed surface kill. First big-map host **landed #81** (this PR did not ship it). Lab-Rat slope/PBR/dirt/scatter/deform plugs **landed later #80**. NRM/GLOSS / further roughness→stamp still parked.
...
| **Hold-O** | Paints `EXTRACT` on the nearest in-front hatch/shaft — elbow card, **no popup**. Release clears. **#115** reads `OPEN` / `CLOSED` / `SHAFT` |
...
Hatch toggle + shaft ride **landed #115**. Do **not** claim timed surface kill / extract loot loop shipped. Do **not** invent bloom / god-rays or Range / Hypha / Lab-Rat work as this PR.
...
Do **not** claim timed surface kill / extract loot loop shipped. Do **not** invent bloom / god-rays or Range / Hypha / Lab-Rat work as this PR.
...
- PreferredHand + new-profile onboard — **parked** house/Hypha. **#84** / **#91** / **#94** / **#97** / **#98** / **#99** / **#100** / **#102** / **#103** / **#109** / **#110** / **#113** / **#115** did **not** ship it
- Augury glasses EXTRACT chrome **landed #85** (elbow card, no popup). CE Home debugger **landed #110**. Hatch toggle + shaft ride **landed #115**. Timed surface kill still **open**. Hypha stream hitch *fix* **landed #108**. Residual soft LOD pop / live octree still parked. CE PERF/PHYS/RENDER/heartbeat/combat-log flags — house-docs steal later
