...
- **O** (hold) = raid extract-check intent (`Session::extract_checking`; fulcrumRust #78). **Not** zero. Hideout is a no-op. Augury EXTRACT elbow card **landed #85** (no popup). Hatch toggle + shaft ride **landed #115**. Door / extract cancel chrome **landed #120** (`GATE_SECS` **2.20**; hold-O still intent only — countdown wins while armed). Timed surface kill still **~**. **P** = drop look-at probe (**landed #125**; CE **T** remapped — T is bandage). Arcade↔sim stay dead (#76)
- **X** = prone
...
- **Home** = CE debugger toggle (fulcrumRust #110). Tabs **LOGS** · **TELE** · **COLL** · **PERF** · **SPWN** · **CHEAT** (←/→ cycle; two-row strip **#125**). Hitch WARN >33 ms / HITCH >100 ms. Glasses `DEBUG`. **End** stays AIM TUNE. Persist `project.json` `debugger_tab` (shares file with Range `aim_live` / `aim_tune` — one `persist_settings()` flush; #125 adds `collision` / `perf` / `spawner`). LOGS COPY + tracker toggles **landed #121** — **Enter** COPY full boot→now ring (clipboard + cwd `fulcrum.logs`; `FULCRUM_LOGS` override) · ring **800** · `[sssss.mmm]` · Insert/Delete **STREAM** · **BAKE** · **GATE** · **PLAY**/GAMEPLAY · **INT**/INTERNAL · `log_*` in `project.json` default **on** · medium edge events · TELE max ms stays when tracks off. Home occlusion + 3D probes **landed #125** — **P** drop look-at probe (cap **32**) · TELE/COLL/PERF **Enter** copies `fulcrum.probes` (`FULCRUM_PROBES` override) · soft Enter debounce **0.45s** · fills under glyphs
...
| **Emit** | Augury Home `STREAM` / `BAKE` (`r=` `cold=` `pend=`) into #110 logger (#121 tracks can quiet STREAM/BAKE). **#123** STREAM lines add `extract0` / `paint` / `gpu` + hitch-thread ms + r↑/cold↓ (Hypha emits only) |
...
See `PEEK_FINDINGS.md` Closed by #127 + fulcrumRust `docs/PROBES.md`.
...
- **P** — unused after #76 for arcade↔sim (no new HoB bind). `FeelSheet::toggle_hob_zero` + session **P** apply gone. Then #33 arcade (aim-dir) ↔ sim (HoB + ballistic zero). **Later remapped #125** — drop look-at probe (CE **T** remapped; T is bandage)
...
| Seat | Owns |
|------|------|
| **Augury** | Native Home debugger overlay (#110). Glasses `DEBUG`. Analysis-core chrome. LOGS COPY / tracks **later landed #121**. Occlusion + probes **later landed #125** |
...
See `PEEK_FINDINGS.md` Closed by #110. Hitch *fix* shelf: Closed by #108 / Closed by #123. LOGS COPY / tracks: Closed by #121. Occlusion + probes: Closed by #125.
...
Out of scope (do **not** claim shipped by #121): Hypha #108 hitch amortize rewrite · End AIM TUNE · door/extract cancel · Beabim pause/MP · Range feel · stamps. Home occlusion + 3D probes **later landed #125**.

See `PEEK_FINDINGS.md` Closed by #121. Foundation: Closed by #110. Hitch *fix* shelf: Closed by #108 / Closed by #123. Occlusion + probes: Closed by #125.
...
| Dial | Value |
|------|--------|
| Bind | **Home** toggle (CE). **End** AIM TUNE untouched |
...
Out of scope (do **not** claim shipped by #125): Hypha hitch *fix* deepen (**later landed #123**) / STREAM refill · End AIM TUNE · Lab-Rat stamps/crawl consume (**later landed #127**) · Range tip/optic consume · Beabim MP · timed surface kill.

See `PEEK_FINDINGS.md` Closed by #125. Foundation: Closed by #110. LOGS COPY / tracks: Closed by #121. Hitch *fix* shelf: Closed by #108 / Closed by #123.
...
Hypha worker STREAM extract+paint (deepen of #108 amortize on the same #81 9×9; play worker extract+paint — hitch thread never `sample_channels` on play Transvoxel extract; worker owns extract+paint; tick applies one finished mesh then leftover paint/GPU; `defer` **`worker/paint/gpu`**; cook=1/2 · cook_ms 8/16 · prefetch 5 m stay; skip far mask-only remesh when R climbs; STREAM `extract0`/`paint`/`gpu` + hitch-thread ms + r↑/cold↓ into #121 Home LOGS; Hypha emits only — no debugger rewrite; load splash stays sync; CHANNELS stay bake-time; smoke Lab-Rat `probes=off` **and** Hypha `cook_ms`/`defer`; Lab-Rat stamps / Range / Beabim / GATE / Augury Home chrome untouched): fulcrumRust PR #123 (2026-09-09) — **landed**. `b4725b8c`. See `PEEK_FINDINGS.md` Closed by #123 + `TERRAIN_NORTHSTAR.md`.
