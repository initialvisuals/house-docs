...
- Soft rim leftover **#156**: glasses + glitch + FX crackle on Hypha `RimHook`. **Mutes under Slain** — death envelope wins (no second leftover on the plate). Do **not** invent a Slain plate / Space / death cam from the rim
...
| Block glitch | **0.36** hold | Below death red (~0.55) / Slain (~0.95) |
| Chroma | **0.012** | Same scale as death prompt split |
| Message | `SIGNAL  THINS` / `CARRIER  LOST` | Mid-glasses `y = 0.22`. Fade **0.55** s on CLEAR. Not an error toast |
| Crackle | FX `Slot::Crackle` · **0.42** s | Procedural radio breakup. Retrigger on APPROACH/BLOCK. Stop on CLEAR / Slain. Not Music loop |
...
See `PEEK_FINDINGS.md` Closed by #156 + fulcrumRust `docs/SOFT_RIM_DIAL_SHEET.md`.
