...
- **#133 shoot peers** — KIND_SHOT + leftover **eye ray** (**1.60 m**, centered) can hit remote volumes. Host-auth Hit relay. Head center **1.62 m** half **0.11** (consume Hypha `EYE_Y` / `HEAD_H` / `HEAD_HALF_H` — do **not** claim Hypha #131 shipped this). XZ **0** — left-offset killed. Pellet Locus `SMG_PELLET` **14**. Kind **KIND_PVP** (13)
...
- **#133 ragdoll** — hooks only (`apply_peer_hit_react` / `peer_corpse_anchor` / KIND_BODY Drop). Visual flop waits on Hypha #131 `PeerBody`
...
- Parked / still TODO (do not claim done): death cam; teammate net stabilize; timed surface kill; full extract loot loop. Ragdoll visual flop waits on #131 `PeerBody`
...
**Still local / other seat (deliberately):** Range 1P feel / AIM TUNE / HoB / heat / SFX / eject dials (#138 PX **±0.50** consumed, not restacked) · Lab-Rat stamps / pedon / CHANNELS · Hypha STREAM / mesher (consume height only — do **not** claim #131 shipped) · Augury Home binds / hatch glasses (SPWN+P still writes the bake sheet) · party name exception · Range **2.5× head** · Mixamo / full 3P kit honesty / hands / gear · server browser / dedicated infra. Do **not** claim Range / Hypha / Lab-Rat / Augury shipped this.
...
**Still local / other seat (deliberately):** Range 1P feel / AIM TUNE / HoB / heat / SFX · Lab-Rat stamps / pedon / CHANNELS · Hypha STREAM / mesher / #131 `PeerBody` · Augury Home binds / hatch glasses (HOST sheet steals leftover HOLD frames only) · Mixamo / full 3P kit honesty / hands / gear · server browser / dedicated infra. Do **not** claim Range / Hypha / Lab-Rat / Augury shipped this.
...
Do **not** claim Range 1P / AIM TUNE / Beabim KIND_* / leftover PVP / Lab-Rat stamps / CHANNELS / pedon / Augury Home / STREAM / #131 biped shipped this.
...
- Biped foot plant **landed #88**. FOLLOW **8.5** / DEADZONE **0.04** / RISE **2.2** / SINK **6.5** / SNAP_ERR **1.15** / LIFT_MAX **0.14** / BOOT_HALF_H center **0.11**. STEAL_MAP biped **partial** (plant + host hooks). Mixamo clips / player body / 2-bone IK / GPU skin still parked. Mesher untouched. Landmark AABB ride **landed #136** (`RIDE_STEP` **0.50** · `RIDE_SKIN` **0.06** · `SUPPORT_STEP` **0.25** unchanged; #88 FOLLOW/DEADZONE stay)
...
Beabim PVP leftover (KIND_PVP default **off**; `FULCRUM_PVP=1` / `--pvp` / INVITE left-right; joiner cannot flip; names **hidden when PVP on** — co-op / PVP-off keeps #91; KIND_BRASS host-relay 3P hip + pawn-right, always on listen stub, not a PVP gate; Range #79/#134 eject dials stay local; leftover eye **1.60 m** centered / head **1.62 m** half **0.11** consume Hypha `HEAD_H` / `HEAD_HALF_H` — do **not** claim #131 shipped; respawn **1.20 s** from Lab-Rat #132 `World.player_spawns` 8 rim @ **144 m**, one-pad knock-off; solo yard `World.spawn`; ragdoll hook-only until #131 PeerBody; house lock **1P≠3P** held; no-pause held; server browser parked; AIM TUNE / Range #138 PX **±0.50** untouched; HOST session board / no-127 invite **later landed #139**): fulcrumRust PR #133 (2026-09-09) — **landed**. `a950ba92`; merge tip `0d73c780`. See `PEEK_FINDINGS.md` Closed by #133 + fulcrumRust `docs/PVP_DIAL_SHEET.md`.
...
Hypha landmark AABB ride (local pawn `ground_y` = `ride_surface_y` = max(heightfield, AABB top under footprint) in a **0.50 m** ride band; `RIDE_STEP` **0.50** · `RIDE_SKIN` **0.06** · `SUPPORT_STEP` **0.25** unchanged; walls-as-floor only in the ride band; Locus hurtboxes stay walls; peers `plant_simple_root_on` same column; `slide_xz` / `resolve` skip XZ when `is_ride_top`; #88 FOLLOW/DEADZONE stay; STREAM / Lab-Rat stamps / Range 1P / AIM TUNE / Beabim KIND / leftover PVP / Augury Home / #131 biped untouched): fulcrumRust PR #136 (2026-09-09) — **landed**. `67bfc1ec`; merge tip `07513e40`. See `PEEK_FINDINGS.md` Closed by #136 + fulcrumRust `docs/LANDMARK_RIDE.md`.
