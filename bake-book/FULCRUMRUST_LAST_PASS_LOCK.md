...
| **Seat** | Range Tech. Same #59 feel PR as lean flip / hop / heat look / distant hit. Tilt path **#84**. First +0.08 / left hold **#94**. Live hip / ready-Y **#98**. Live hip_low **#99**. Live ads_cant **#100**. Live hold springs **#109**. Live End PX **#138**. Live End PX hideout wall **#159**. Hypha PreferredHand **#116** seats H. PreferredHand live in AIM TUNE dialogue is follow-up — not #159 |

See `PEEK_FINDINGS.md` Closed by #59 / Closed by #84 / Closed by #94 / Closed by #98 / Closed by #99 / Closed by #100 / Closed by #109 / Closed by #113 / Closed by #116 / Closed by #138 / Closed by #159 + H shoulder-swap tilt (#84) + Patch A RH hip bias (#94) + RH body-width + ready-hip Y (#98) + low-hip shotgun stance (#99) + canted CQC ADS (#100) + U-cycle hold springs (#109) + kit handling / ergo / MOA (#113) + PreferredHand onboard (#116) + AIM TUNE PX travel (#138) + AIM TUNE PX hideout wall (#159).
...
See `AESTHETIC_DIEGETIC_LOCK.md`. Hypha window / tab guts / persist shipped #46. GPU post stack live #55 (toggles change the image; not full HDR bloom / god-ray). **#66** colorless heat warp and **#68** ADS near sit on that same pass / same Options **DOF**. **#86** Graphics dump fills thin FOG / CAM rows on that Graphics pane — does **not** invent bloom / god-ray / brightness / gamma paths. **#87:** dump **sunSize 0.62** rides the sky disc (no longer parked). **#90:** Options Graphics **WARP** after CAM FAR (`warp_strength` default **0.01**). **#164:** Graphics/RES/FOV shelf cook — RES / FOV / AA / AO / POST persist on that same pane.

## Menus / settings ownership (Evan dump 2026-09-07; Augury shell #45; Hypha guts #46; GPU post #55; colorless heat #66; ADS near #68; Graphics dump #86; HDRI sun #87; pixellation / WARP #90; wound consume #143; Graphics/RES/FOV #164)

Augury shell polish shipped #45 (title + HOLD chrome + Options list shell + logo seat). Hypha Graphics/Gameplay/Controls guts + window mode + persist shipped #46. GPU post stack live **#55** — toggles change the image (AO/AA/CA/grain/DoF). **#66** colorless muzzle heat and **#68** ADS near DoF sit on that same pass. **#86** Graphics dump locks fog / cam / sky defaults + thin Options FOG / CAM rows. **#87** completed the HDRI sun disc (dump **sunSize 0.62** rides the disc). **#90** pixellation / floor-warp adds Options Graphics **WARP** after CAM FAR (`warp_strength` default **0.01**, step **0.01**, 0 = off; mix toward CE PIXEL SCALE **2**). **#164** Graphics/RES/FOV shelf cook fills RES / FOV / AA type / AA STR / AO quality / POST master on that same pane (persist `resolution` / `fov_hip` / `aa_type` / `aa_strength` / `ao_quality` / `post_quality`). Honest: not the full Mycelium HDR bloom / god-ray / contact-shadow chain — do **not** invent bloom / god-ray / brightness / gamma paths. Logo/title mark #41 still stands.
...
| Dial | Lock |
|------|------|
| **Window** | Live via winit. **Borderless** = default launch. **Windowed** = decorated 1280×720. **Exclusive** = exclusive video mode when OS/GPU expose one, else borderless fallback. Also `--windowed` / `FULCRUM_WINDOW` (`borderless` / `windowed` / `exclusive`). **#164** RES dropdown beside WINDOW. Default **NATIVE**. Borderless stays desktop-native |
...
| **Persist** | `project.json` in cwd, or `FULCRUM_SETTINGS=/path/to.json`. #82 `output_device` · **#90** `warp_strength` · **#103** `aim_live` / `aim_tune.*` · **#116** `profile_onboarded` / `preferred_hand` · **#163** `look_mul_h` / `look_mul_v` / `hip_look_mul` / `ads_look_mul` · **#164** `resolution` / `fov_hip` / `aa_type` / `aa_strength` / `ao_quality` / `post_quality` (old `ao` / `aa` bools still load) live on the same file |
...
See `PEEK_FINDINGS.md` Closed by #103. Canted optic + AIM TUNE ATTACH **later landed #150** (`attachments.canted`). AIM TUNE PX hideout wall **later landed #159**.
...
| Live-save schema | #103 | `aim_tune.*` + `aim_live`. No new keys |

| Seat | Owns |
|------|------|
| **Range Tech** | End PX floor + WEAPON home snap + ATTACH same box (#159). #138 named the ±0.50 box |
| **Hypha** | PreferredHand **#116** seats H. **PreferredHand live in AIM TUNE dialogue is follow-up — not this cook** |
...
**Still local / other seat (deliberately):** Hypha res / FOV slider / AA / AO / post **later landed #164** · WARP / STREAM · Lab-Rat texture / mip / `lod_mips` · Augury remap / Tab inventory · PreferredHand onboard · AIM TUNE PX box · stamps · death chrome. Options default hip **Chest (default)** vs **Low** toggle + Powder B — Range still cooking; do **not** claim landed. Do **not** invent implementation. Do **not** claim Hypha / Lab-Rat / Augury / Beabim shipped this.

See `PEEK_FINDINGS.md` Closed by #163 + house `OPTIONS_SHELF.md` + fulcrumRust `docs/OPTIONS_MOUSE_SENS_DIAL_SHEET.md`.

## Options Graphics / RES / FOV (Hypha — landed #164)

Evan lock. **Shipped** [fulcrumRust #164](https://github.com/initialvisuals/fulcrumRust/pull/164) (2026-09-10, merge `159b3d11`). Seat: **Hypha** owns Options Graphics pane wiring on Augury **#45** chrome. House `OPTIONS_SHELF` Resolution / Graphics / FOV seat. Persist `project.json`. CREDITS + STEAL_MAP + `OPTIONS_GRAPHICS_DIAL_SHEET` already claimed in-PR. Canonical dial sheet: fulcrumRust [`docs/OPTIONS_GRAPHICS_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/OPTIONS_GRAPHICS_DIAL_SHEET.md). House pointer: [`OPTIONS_GRAPHICS.md`](OPTIONS_GRAPHICS.md). Overnight cooks steal these numbers from the sheet — not every `.rs`. Do **not** invent dials.

| Dial | Lock |
|------|------|
| **RES** | `resolution` default **NATIVE** (desktop native — no invented pair). Dropdown beside WINDOW. Shelf presets + UW 4K CLASS (3840×1600 class) + Evan 3440×1440 + Steam survey fill **2560×1600 / 1920×1200 / 1366×768**. Borderless stays desktop-native |
...
| **Legacy** | Old `project.json` `"ao"` / `"aa"` bools still load. New keys win when present |

**Still local / other seat (deliberately):** Lab-Rat texture / mip / `lod_mips` · Range mouse V/H + hip/ADS (**#163** already shelved) · Range READY HIP Chest/Low · Augury remap / Tab inventory · STREAM `StreamPolicy` · Powder / PreferredHand / ballistics / AIM TUNE · Beabim net · TAA · bloom / godRays / brightness / gamma. Do **not** invent implementation. Do **not** claim Range / Lab-Rat / Augury / Beabim shipped this.
...
Range Tech Options mouse V/H + hip/ADS (Controls **MOUSE H** / **MOUSE V** / **HIP** / **ADS** on the existing #46 `LOOK_MUL` lock **1.00** / **0.25–2.00** / step **0.05**; persist `look_mul_h` / `look_mul_v` / `hip_look_mul` / `ads_look_mul`; old `look_mul` seeds both mouse axes when split keys are absent; feel-lab `look_sens` **0.0022** + feel ADS **0.86** / blend **6.4** stay DNA; Invert Y stays; Options default hip Chest vs Low + Powder B still cooking — do **not** claim landed; Hypha res/FOV/AA/AO/post **later landed #164** · WARP/STREAM · Lab-Rat texture/mip · Augury remap/Tab · PreferredHand · AIM TUNE PX left alone): fulcrumRust PR #163 (2026-09-10) — **landed**. Merge `96e4410e`. See `PEEK_FINDINGS.md` Closed by #163 + house `OPTIONS_SHELF.md` + fulcrumRust `docs/OPTIONS_MOUSE_SENS_DIAL_SHEET.md`.
Hypha Options Graphics/RES/FOV (RES default **NATIVE** beside WINDOW · FOV hip **90** / optic ADS locks still win · AA **FXAA/OFF** · AA STR **0.5** live #55 mix · AO **OFF/LOW/HIGH** Low 8-tap r **0.55** · High Mycelium 24-tap · POST **OFF/LOW/HIGH** default **HIGH**; persist `resolution` / `fov_hip` / `aa_type` / `aa_strength` / `ao_quality` / `post_quality`; old `ao` / `aa` bools still load; Augury #45 chrome; Lab-Rat texture/mip · remaps/Tab · TAA · bloom left alone): fulcrumRust PR #164 (2026-09-10) — **landed**. Merge `159b3d11`. See `PEEK_FINDINGS.md` Closed by #164 + house `OPTIONS_SHELF.md` + fulcrumRust `docs/OPTIONS_GRAPHICS_DIAL_SHEET.md`.
