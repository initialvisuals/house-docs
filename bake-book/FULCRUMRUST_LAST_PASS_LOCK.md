...
## Crossover shoulder / left-corner peek (fulcrumRust #59; tilt #84; RH hip #94)

Evan lock. **Travel landed** [fulcrumRust #59](https://github.com/initialvisuals/fulcrumRust/pull/59). **Tilt path landed** [fulcrumRust #84](https://github.com/initialvisuals/fulcrumRust/pull/84). **RH hip bias + left straighten landed** [fulcrumRust #94](https://github.com/initialvisuals/fulcrumRust/pull/94). Range Tech. Quiet influence — house words: **crossover shoulder / left-corner peek** (no franchise name-drop in shelf / READMEs / public copy).

| Dial | Lock |
|------|------|
| **Authored hip** | +X ~**0.18** (right; live MP9-Z **0.1843** — was ~0.10) |
| **H hold** | Springs the **viewmodel** across the chest to a partial left (~**−0.041**, cap `shoulder_x_min` **−0.055**) |
| **Travel deepen** | `shoulder_cross_x` **−0.225** (**#94**; was −0.145) — keeps dest ~−0.041 |
| **Tilt** | **#94** slight straighten pitch/yaw/roll **0.04 / 0.10 / 0.08** (supersedes #84 chest-cross **0.08 / 0.32 / 0.39**) on existing ViewmodelDials / ADS cant DNA |
| **Not** | A capsule / eye slide. Mesh mirror / `scale.x = −1`. Infinite travel. Inward yaw to fake aim |
| **Left probe** | `shoulder_viewmodel` **0.12** — helps left-corner leans |
| **ADS** | Keeps **0.32** of the crossover (travel and tilt). ADS X stays feel-lab bore-center |
| **H** | Existing FoW shoulder habit — viewmodel crossover, not a new key |
| **Seat** | Range Tech. Same #59 feel PR as lean flip / hop / heat look / distant hit. Tilt path **#84**. Live hip / left hold **#94** |

See `PEEK_FINDINGS.md` Closed by #59 / Closed by #84 / Closed by #94 + H shoulder-swap tilt (#84) + Patch A RH hip bias (#94).
...
| Cue | Lock |
|-----|------|
| **H crossover** | Authored hip +X ~**0.18** (right; live **0.1843**). Viewmodel springs to partial left (~**−0.041**, cap `shoulder_x_min` **−0.055**). `shoulder_cross_x` **−0.225**. Extra left probe `shoulder_viewmodel` **0.12**. ADS keeps **0.32**. Arms-limited — not a capsule/eye slide, not a mesh mirror, not infinite travel. Existing H bind, not a new key. **#94** slight straighten pitch/yaw/roll **0.04 / 0.10 / 0.08** (supersedes #84 chest-cross 0.08/0.32/0.39) — no `scale.x = −1` |
...
| Seat | Owns |
|------|------|
| **Range Tech** | H travel (#59) + tilt path (#84). Live hip / left hold **#94** |
| **house / Hypha** | PreferredHand + new-profile onboard — **parked**. Do **not** claim shipped |

Live hip X / `shoulder_cross_x` / left tilt **superseded #94** — hip **+X ~0.18** / **0.1843**; `shoulder_cross_x` **−0.225**; pitch/yaw/roll **0.04 / 0.10 / 0.08** (slight straighten, not chest-cross cant). See Patch A RH hip bias (#94).
...
#94 RH hip authored defaults stay until Evan nudges live (MP9-Z hip **0.1843 / −0.1688 / −0.1953**; ADS iron X **0.0084**; SR-25 / M24 hip X **0.20 / 0.205**; H travel / slight straighten stay). Kit boxes + muzzle tip follow live can/optic offsets. World drops stay authored identity.
...
- PreferredHand + new-profile onboard — **parked** house/Hypha. **#84** / **#94** / **#97** did **not** ship it
...
Evan peek feel (lean flip + deepen, CE hop + air hop, heat v77 look, H crossover, tracers-until-impact + FX `hit`): fulcrumRust PR #59 (2026-09-07) — **landed**. H tilt path **superseded #84**. Live hip / left hold **superseded #94**. See `PEEK_FINDINGS.md` Closed by #59.
