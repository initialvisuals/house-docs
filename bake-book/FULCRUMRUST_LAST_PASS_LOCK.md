...
- **O** (hold) = raid extract-check intent (`Session::extract_checking`; fulcrumRust #78). **Not** zero. Hideout is a no-op. Augury EXTRACT elbow card **landed #85** (no popup). Hatch toggle + shaft ride **landed #115**. Door / extract cancel chrome **landed #120** (`GATE_SECS` **2.20**; hold-O still intent only — countdown wins while armed). Timed surface kill still **~**. **P** = drop look-at probe (**landed #125**; CE **T** remapped — T is bandage). **#128** SPWN+P (or TELE Insert→SPAWN→P) writes `kind:spawn`. Surface `name` is `ground` / … — not mesh id `extract`. Arcade↔sim stay dead (#76)
- **X** = prone
...
- **P** — unused after #76 for arcade↔sim (no new HoB bind). `FeelSheet::toggle_hob_zero` + session **P** apply gone. Then #33 arcade (aim-dir) ↔ sim (HoB + ballistic zero). **Later remapped #125** — drop look-at probe (CE **T** remapped; T is bandage). **#128** SPWN+P / Insert **SPAWN** writes `kind:spawn`; surface `name` is `ground` / … — not mesh id `extract`
...
| **Lab-Rat** | Probe consume already landed #127 (dirt sit at `kind:spawn`) — not rewritten |

Out of scope (do **not** claim shipped by #128): Hypha hitch *fix* (#108 / **#123** already shelved) · End AIM TUNE · Lab-Rat spawn bake consume (already landed #127 — not this cook) · Range tip/optic consume · PVP hitboxes · Beabim MP · timed surface kill.

See `PEEK_FINDINGS.md` Closed by #128. Foundation: Closed by #110. LOGS COPY / tracks: Closed by #121. Occlusion + probes: Closed by #125. Hitch *fix* shelf: Closed by #108 / Closed by #123.
