...
See `PEEK_FINDINGS.md` Closed by #147 + fulcrumRust `docs/PVP_DIAL_SHEET.md`. MP weapon honesty + hideout hub: Closed by #153.
...
Beabim PVP leftover ray (Evan sniper peek: peers visible ~700 m decal while wound died at **80 m**; `LEFTOVER_HIT_M` / `first_leftover_hit` **500 m** was 80; eye-centered `PeerBody` + #141 hurt skin / sweep / inclusive slab held; flat `SMG_PELLET` **14** — no invented falloff; Locus yard `apply_shot` keeps its own **80 m** helper; Range feel / zero / HoB / falloff / Hypha STREAM / Augury death / Lab-Rat stamps untouched; house lock **1P≠3P** held; no-pause held; hideout hub leftover **later landed #153**): fulcrumRust PR #147 (2026-09-10) — **landed**. `9a9853d4`; merge tip `0b90b2ef`. See `PEEK_FINDINGS.md` Closed by #147 + fulcrumRust `docs/PVP_DIAL_SHEET.md`.
