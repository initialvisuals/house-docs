...
## Listen-server pose presence + HOLD JOIN (fulcrumRust #34 + Beabim #83; invite leftover #91; loot trail #102; world/sim leftover #119; no-pause + KIND_RAID #122; PVP leftover #133)

Seat: **Beabim** owns this MP slice (listen-server / two-instance sync / join panel / live-profile loot trail / world-sim leftover / no-pause / shared instance / PVP leftover). Hypha #34 was the handshake-only stub. [PR #83](https://github.com/initialvisuals/fulcrumRust/pull/83) (`24eaca4b`). Invite leftover + peer names + gun pose **landed #91** (`c4d75c11`). Live-profile loot trail **landed #102** (`6fc0d5d6`). World/sim leftover **landed #119** (`6562070e726c0ae69719598bdc294a1f51d2e37e`). No-pause + KIND_RAID shared instance **landed #122** (`cbb26d8796faa9e476e8695fb4b118486d1759b3`). PVP leftover **landed #133** (`a950ba92`; merge tip `0d73c780`) — #83 stays the pose / HOLD JOIN foundation. Augury #120 still owns glasses HOLD % / `GATE_SECS` **2.20**. Range #138 AIM TUNE PX **±0.50** stays Range.
...
- **#133 KIND_PVP** — host leftover. Default **off**. `FULCRUM_PVP=1` / `--pvp` seeds the host. INVITE sheet left/right flips. Joiner cannot flip. WELCOME seeds a late joiner. Dial sheet: fulcrumRust `docs/PVP_DIAL_SHEET.md`
...
- Honesty: two instances agree on **peer presence** (pose + silhouette) + **#102** kit loot trail + **#119** shot / Locus leftover / death bag + **#122** shared raid instance (same raid id + host seed) + **#133** PVP leftover (toggle / hide names / peer hit / KIND_BRASS / rim respawn) while menus mute the local pawn only. Range feel / HoB / heat / eject dials / 1P hip / AIM TUNE stay local — **no** full world replication / land feel / H tilt (#84) / audio device (#82) / Graphics dump (#86) / HDRI sun (#87) / Locus brain tick on the joiner / stamps / COL / deform / scatter / Transvoxel rewrite / Augury elbow / hatch UX (#85) / world seed / ToD / Mixamo player body / server browser / dedicated infra
...
| Dial | Lock |
|------|------|
| **Default** | **off**. `FULCRUM_PVP=1` / `--pvp` seeds the host. INVITE sheet left/right toggles. Joiner cannot flip |
...
See `PEEK_FINDINGS.md` Closed by #133 + fulcrumRust `docs/PVP_DIAL_SHEET.md`.
...
Listen-server + invite stub: fulcrumRust PR #34 (2026-09-07). Handshake-only + no in-game join field **superseded #83**. Leftover INVITE + names + gun stub **landed #91**. Loot trail **landed #102**. World/sim leftover **landed #119**. No-pause + KIND_RAID **landed #122**. PVP leftover **landed #133**.
...
Beabim PVP leftover (KIND_PVP default **off**; `FULCRUM_PVP=1` / `--pvp` / INVITE left-right; joiner cannot flip; names **hidden when PVP on** — co-op / PVP-off keeps #91; KIND_BRASS host-relay 3P hip + pawn-right, always on listen stub, not a PVP gate; Range #79/#134 eject dials stay local; leftover eye **1.60 m** centered / head **1.62 m** half **0.11** consume Hypha `HEAD_H` / `HEAD_HALF_H` — do **not** claim #131 shipped; respawn **1.20 s** from Lab-Rat #132 `World.player_spawns` 8 rim @ **144 m**, one-pad knock-off; solo yard `World.spawn`; ragdoll hook-only until #131 PeerBody; house lock **1P≠3P** held; no-pause held; server browser parked; AIM TUNE / Range #138 PX **±0.50** untouched): fulcrumRust PR #133 (2026-09-09) — **landed**. `a950ba92`; merge tip `0d73c780`. See `PEEK_FINDINGS.md` Closed by #133 + fulcrumRust `docs/PVP_DIAL_SHEET.md`.
