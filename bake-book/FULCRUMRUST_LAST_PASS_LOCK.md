...
| **Persist** | `project.json` in cwd, or `FULCRUM_SETTINGS=/path/to.json`. #82 `output_device` · **#90** `warp_strength` · **#103** `aim_live` / `aim_tune.*` · **#116** `profile_onboarded` / `preferred_hand` · **#163** `look_mul_h` / `look_mul_v` / `hip_look_mul` / `ads_look_mul` live on the same file |
