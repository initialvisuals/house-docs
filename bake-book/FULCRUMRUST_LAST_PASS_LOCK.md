...
- **Landmark AABB ride** (Hypha #136): **landed**. Local pawn `ground_y` = `ride_surface_y` = max(heightfield, AABB top under footprint) in a **0.50 m** ride band so buildings / rocks / props are walkable floors — not XZ shove. Walls-as-floor **only** in the ride band. Locus hurtboxes stay walls. Peers `plant_simple_root_on` same column. Canonical dial sheet: fulcrumRust `docs/LANDMARK_RIDE.md`. #88 plant / STREAM / Lab-Rat stamps / Range `SUPPORT_STEP` **0.25** untouched. Hop vertical-before-XZ **later landed #149**. Do **not** claim Range / Beabim / Lab-Rat / Augury shipped this. See `PEEK_FINDINGS.md` Closed by #136 + Closed by #149
- **Landmark hop ride** (Hypha #149): **landed**. Thin follow-up to #136. Local pawn applies **vertical before XZ** so a Space hop (~0.20 m on the first frame) enters `RIDE_STEP` and hop-onto / land on a 0.44–0.7 m rock/prop lid does not shove off. `RIDE_STEP` **0.50** · `RIDE_SKIN` **0.06** · `SUPPORT_STEP` **0.25** held. No new hop dial — #136 numbers cover jump. Camera `push_out_of_walls` skips a box the hop-eye is standing above. Canonical dial sheet: fulcrumRust `docs/LANDMARK_RIDE.md`. Do **not** claim Range / Beabim / Lab-Rat / Augury shipped this. See `PEEK_FINDINGS.md` Closed by #149
...
## Downed / revive
- HP→0 **downs** (prone crawl + thin bleed) — not menu death. Bleed-out ~`BLEED_SECS` **22.0**; extra hits while downed shave `BLEED_HIT_SECS` **6.0**. Clock expiry → `DEAD` + dark bag. Shipped Augury #36; Death/Slain chrome **later landed #146**; teammate net / full loot loop still parked.
...
- Range wound feel **#143**: 1P screen-react only (suppress / armour / HP). Hooks KIND_PVP **Hit** (and local `apply_player_wound`) — **Sync** does not punch. **Death / Slain chrome later landed #146** (do **not** claim #143 shipped slain UI)
- Death / Slain **#146**: PvE bleed-out envelope Red `DEATH_RED_SECS` **0.55** wash 0.78 · FadeBlack **0.70** · Beat **1.00** · Slain `SLAIN_FADE_SECS` **0.78** plate `assets/images/ui/deathscreens/slain.jpg` · Prompt **0.60** PRESS SPACE → existing `claim_place_spawn` / #141 pads. Hit flash 0.18/0.20 · downed wash 0.48. `BAR_CATCH` **6.5** (default on; `FULCRUM_BAR_DELAY=0/off` snaps). Glitch rides envelope; `GLITCH_PROMPT_FLOOR` **0.12**. PVP down does **not** play Slain; Beabim 1.20s leftover untouched. Canonical dial sheet: fulcrumRust `docs/DEATH_DIAL_SHEET.md`
- Parked / still TODO (do not claim done): teammate net stabilize; timed surface kill; full extract loot loop. Ragdoll visual flop **later landed #131** `PeerBody`. Leftover hurt volumes **later landed #141** (`PeerBody::hurtboxes()` + skin **0.06** + sweep **4**). Death cam / Slain **later landed #146**
...
| **Hop** | same band — **later landed #149**. Local pawn applies **vertical before XZ**. A Space hop (~0.20 m on the first frame) enters `RIDE_STEP` so hop-onto / land on a 0.44–0.7 m lid does not shove. No extra hop dial — #136 numbers cover jump |

Do **not** claim Range 1P / AIM TUNE / Beabim KIND_* / leftover PVP / Lab-Rat stamps / CHANNELS / pedon / Augury Home / STREAM / #131 biped shipped this. 3P biped **later landed #131**. Hop vertical-before-XZ **later landed #149**.
...
- Biped foot plant **landed #88**. FOLLOW **8.5** / DEADZONE **0.04** / RISE **2.2** / SINK **6.5** / SNAP_ERR **1.15** / LIFT_MAX **0.14** / BOOT_HALF_H center **0.11**. STEAL_MAP biped **partial** (plant + host hooks). Mixamo clips / player body / 2-bone IK / GPU skin still parked. Mesher untouched. Landmark AABB ride **landed #136** (`RIDE_STEP` **0.50** · `RIDE_SKIN` **0.06** · `SUPPORT_STEP` **0.25** unchanged; #88 FOLLOW/DEADZONE stay). Hop vertical-before-XZ **later landed #149** (no new hop dial — #136 numbers cover jump). 3P biped / PeerBody **landed #131** (`EYE_Y` **1.60** · `HEAD_H` **1.62** · XZ **0** left-offset killed; crouch squat **0.62** · ragdoll flop **0.55 s**). Lean match **later landed #145** (torso peek **0.5 m** · hinge **0.55** · `LEAN_ANGLE` **0.52** · `LEAN_SMOOTH` **9.5** · pelvis/spine/chest; feet planted)
...
Hypha landmark AABB ride (local pawn `ground_y` = `ride_surface_y` = max(heightfield, AABB top under footprint) in a **0.50 m** ride band; `RIDE_STEP` **0.50** · `RIDE_SKIN` **0.06** · `SUPPORT_STEP` **0.25** unchanged; walls-as-floor only in the ride band; Locus hurtboxes stay walls; peers `plant_simple_root_on` same column; `slide_xz` / `resolve` skip XZ when `is_ride_top`; #88 FOLLOW/DEADZONE stay; STREAM / Lab-Rat stamps / Range 1P / AIM TUNE / Beabim KIND / leftover PVP / Augury Home / #131 biped untouched — 3P biped **later landed #131**; hop vertical-before-XZ **later landed #149**): fulcrumRust PR #136 (2026-09-09) — **landed**. `67bfc1ec`; merge tip `07513e40`. See `PEEK_FINDINGS.md` Closed by #136 + Closed by #149 + fulcrumRust `docs/LANDMARK_RIDE.md`.
Hypha landmark hop ride (thin follow-up to #136; local pawn applies **vertical before XZ** so a Space hop (~0.20 m first frame) enters `RIDE_STEP` and hop-onto / land on a 0.44–0.7 m rock/prop lid does not shove off; `RIDE_STEP` **0.50** · `RIDE_SKIN` **0.06** · `SUPPORT_STEP` **0.25** held; no new hop dial — #136 numbers cover jump; camera `push_out_of_walls` skips a box the hop-eye is standing above; STREAM / Lab-Rat stamps / Range 1P / AIM TUNE / Beabim KIND / Augury Home / Slain untouched): fulcrumRust PR #149 (2026-09-10) — **landed**. `ff540315`. See `PEEK_FINDINGS.md` Closed by #149 + fulcrumRust `docs/LANDMARK_RIDE.md`.
