...
- **Sandbox pedon** (Lab-Rat #130): **landed**. Stick + button on the extract yard. Tap **F** rebakes stamp / growth peek / Subtract crawl on a **separate smaller leftover slab** on a concrete table on the real pad — Patch A CHANNELS without Hypha’s 19×19 STREAM remesh. Stick XZ `3.90, -0.55` · slab XZ `5.20, -0.55` (`TABLE_H` 1.02 m) · reach 1.35 m · slab 1.90 m half-span, 0.38 m thick, 10×6×10 @ 0.18 m. Glasses `PEDON  F  REBAKE  GEN n`. Seed = extract seed xor generation. Leftover voxels = Lab-Rat `pedon` GPU overlay. `StampField::layers` / `TerrainHost::stream_rev` stay cold. Smoke `pedon=gen=1 prims=N voxels=M`. Not Home chrome / not #127 consume / not #114 pad crawl. See `PEEK_FINDINGS.md` Closed by #130 + fulcrumRust `docs/PEDON.md`
- **Extract player spawn loci** (Lab-Rat #132): **landed**. Lab-Rat supplies bake-time loci. Beabim runtime pool / one-pad knock-off **later landed #133**. Default rim: 8 pads on 19×19 outer-chunk centers, Chebyshev **144 m**, facing extract origin (`atan2(-x,-z)`; yaw 0 = +Z). Y from `terrain.height_at`. Inside `playable_half_m` (151.25). No CHANNELS writes at the far rim. Home **P** + SPWN (`kind:spawn`, typically `name:ground`) override nearest rim pad within **48 m** XZ, else append. Same `fulcrum.probes.v1` — no new fields. Dirt sit CHANNELS leftover when bake-warm still applies (#127). `World.player_spawns: Vec<SpawnLocus>` (`pos`, `yaw`, `source` Rim/Probe) for Beabim. Solo still uses yard `World.spawn` / `spawn_yaw`. Smoke `spawns=rim=8` or `spawns=rim=N+probe=M`. `probes=off` stays the CHANNELS sheet tag. **Not** this cook's PVP / hitboxes / respawn leftover (those **later landed #133**). See `PEEK_FINDINGS.md` Closed by #132 + Closed by #133 + fulcrumRust `docs/SPAWNS.md`
...
- **Landmark AABB ride** (Hypha #136): **landed**. Local pawn `ground_y` = `ride_surface_y` = max(heightfield, AABB top under footprint) in a **0.50 m** ride band so buildings / rocks / props are walkable floors — not XZ shove. Walls-as-floor **only** in the ride band. Locus hurtboxes stay walls. Peers `plant_simple_root_on` same column. Canonical dial sheet: fulcrumRust `docs/LANDMARK_RIDE.md`. #88 plant / STREAM / Lab-Rat stamps / Range `SUPPORT_STEP` **0.25** untouched. Do **not** claim Range / Beabim / Lab-Rat / Augury shipped this. See `PEEK_FINDINGS.md` Closed by #136
- **3P biped / PeerBody** (Hypha #131): **landed**. Visual / state feedback for peer bodies. `EYE_Y` **1.60** (local cam `MoveDials.eye_stand` + 3P `Socket::Eye`). `HEAD_H` **1.62** visual head center (surrounds the eye — not a crown float). `HEAD_HALF_H` **0.11**. Head / eye XZ **0** — left-offset killed. Old 3P stub 1.22 dead. Crouch squat **0.62**. Ragdoll flop **0.55 s** (`KIND_BODY` Drop → PeerBody). Pose lean unused byte 2 (`i8/127`; + = peek left / E). Lean tilt visual roll + 0.14 m lateral. Walk/run/jump primitives. Hit-react Flinch→Chest / Stagger→Spine. Foot plant #88 + #136 ride kept. Canonical dial sheet: fulcrumRust `docs/BIPED_3P_DIAL_SHEET.md`. Mixamo clips / player body / 2-bone IK / GPU skin parked. Not Range 1P AIM TUNE / HoB / heat. Not Beabim KIND_* / hitboxes / loot UI. See `PEEK_FINDINGS.md` Closed by #131
...
- Stream cook **#108** + **#123**: `COOK_BUDGET_PLAY` **1** / `COOK_BUDGET_LOAD` **2** · cook_ms **8/16** · prefetch **5 m** · splash-pumped load-in · `defer` **`worker/paint/gpu`**. Same 9×9 — not a second window. Play hitch thread never `sample_channels`; skip far mask-only remesh when R climbs
...
- Parked: live octree / unconstrained Sync dump · residual soft LOD pop inside a cell · tunnels · runtime carve · Transvoxel rewrite / world sync · NRM/GLOSS GPU. **#114** is a shallow enterable pad network, not those parked cutouts. Amortized recook under budget **shipped #108**. Worker extract+paint deepen **shipped #123**. Peer feet `stream_anchors` **landed #83** (coordinate only). Near LOD raise **shipped #61**. First big-map **landed #81**. Texture mips / compression **shipped #60** on the same **distance rings** (near 256² / mid 64² / far 16²; far softer). In-repo grit mips stay until Lab-Rat cooks more — see Texture LOD compress. See `TERRAIN_NORTHSTAR.md` + fulcrumRust `docs/TERRAIN.md`
...
| **Stream** | **9×9** window (`STREAM_RINGS` 4) by player eyes. **#108** amortizes cook on this same window (cook=1/2 · prefetch=5 m · splash-pumped load-in) — not a second 9×9. **#123** deepens play (`defer=worker/paint/gpu` · skip far mask-only remesh). Beabim #83 `stream_anchors` also returns remote feet (coordinate only) |
...
| **7. Distance load** | **Landed #81** — local-player stream. **#108** amortizes that same 9×9 (cook=1/2 · prefetch=5 m). **#123** deepens play (`defer=worker/paint/gpu` · skip far mask-only remesh). Beabim peer feet `stream_anchors` **landed #83** (coordinate only — no Transvoxel rewrite) |

See `TERRAIN_NORTHSTAR.md` + `PEEK_FINDINGS.md` Closed by #81 / Closed by #108 / Closed by #123. Hypha #88 consumes this same #79/#81 column for pawn plant — mesher untouched. Lab-Rat plugs **landed later #80**.

## Stream hitch amortize (Hypha — landed #108 + #123)

Evan lock. **Shipped** [fulcrumRust #108](https://github.com/initialvisuals/fulcrumRust/pull/108) (2026-09-09, `b3be2974`) + deepen [fulcrumRust #123](https://github.com/initialvisuals/fulcrumRust/pull/123) (2026-09-09, `b4725b8c`). **Hypha** owns amortized cook on the existing #81 9×9 stream — not a new map size. Hitch *fix* layer 1 is #108 (cook budget / prefetch / splash-pumped load-in). Worker/paint/gpu deepen is **#123**. Hitch *visibility* stays Augury #110 / #121 (Hypha emits, does not rebuild Home). Residual soft LOD pop / live octree stay parked leftovers.

| Dial | Lock |
|------|------|
| **Load-in** | Skeleton (density + stamps + props) on frame 0; splash keeps presenting; **2** extracts / frame (`COOK_BUDGET_LOAD`) until idle; GPU upload only when the window is ready. No Sync whole **9×9** dump on one frame. Splash stays sync |
...
| **Cook / prefetch** | cook=1/2 · cook_ms **8/16** · prefetch **5 m**. Does **not** open a second 9×9 |
...
| **Smoke** | `cook=1/2 prefetch=5` next to `subdivs=32/16/8/4`. **#123** also prints `cook_ms` / `defer=worker/paint/gpu` next to Lab-Rat `probes=off` |
| **Parked** | Live octree · residual soft LOD pop inside a cell · unconstrained Sync dump |

| Seat | Owns |
|------|------|
| **Hypha** | Stream hitch *fix* (#108 amortize + #123 worker/paint/gpu). Mesher stolen (`TerrainHost` / `extract_one` / activation) — no greenfield mesher |
...
See `PEEK_FINDINGS.md` Closed by #108 / Closed by #123 + `TERRAIN_NORTHSTAR.md`.
...
| Dial | Lock |
|------|------|
| Default rim | 8 pads on 19×19 outer-chunk centers, Chebyshev **144 m**, facing extract origin |
| Yaw | 0 = **+Z**. Face center `atan2(-x, -z)` |
| Y | `terrain.height_at` at bake. Inside `playable_half_m` (151.25) |
...
Do **not** claim Beabim knock-off / PVP / respawn UI / biped eye shipped by #132. Runtime pool / PVP leftover **later landed #133**. Hideout / empty hatch stubs keep an empty pool.

See `PEEK_FINDINGS.md` Closed by #132 + Closed by #133 + fulcrumRust `docs/SPAWNS.md`.
...
## Listen-server pose presence + HOLD JOIN (fulcrumRust #34 + Beabim #83; invite leftover #91; loot trail #102; world/sim leftover #119; no-pause + KIND_RAID #122; PVP leftover #133; HOST session board #139)

Seat: **Beabim** owns this MP slice (listen-server / two-instance sync / join panel / live-profile loot trail / world-sim leftover / no-pause / shared instance / PVP leftover / HOST-page session board / invite LAN surface). Hypha #34 was the handshake-only stub. [PR #83](https://github.com/initialvisuals/fulcrumRust/pull/83) (`24eaca4b`). Invite leftover + peer names + gun pose **landed #91** (`c4d75c11`). Live-profile loot trail **landed #102** (`6fc0d5d6`). World/sim leftover **landed #119** (`6562070e726c0ae69719598bdc294a1f51d2e37e`). No-pause + KIND_RAID shared instance **landed #122** (`cbb26d8796faa9e476e8695fb4b118486d1759b3`). PVP leftover **landed #133** (`a950ba92`; merge tip `0d73c780`). HOST session board + no-127 invite **landed #139** (`0f8ee594`; merge tip `f6f8d509`) — #83 stays the pose / HOLD JOIN foundation. Augury #120 still owns glasses HOLD % / `GATE_SECS` **2.20**. Range #138 AIM TUNE PX **±0.50** stays Range.
...
- Glasses labels only: `HOST  fulcrum://ip:port` (#91; was `HOST  ip:port`), then `JOIN` / `PEER` after HELLO/WELCOME — never a second ammo HUD. **#133** adds `  PVP` suffix on that same HOST/PEER slot when on (not a second HUD; not Augury Home)
...
- **#133 shoot peers** — KIND_SHOT + leftover **eye ray** (**1.60 m**, centered) can hit remote volumes. Host-auth Hit relay. Head center **1.62 m** half **0.11** (consume Hypha `EYE_Y` / `HEAD_H` / `HEAD_HALF_H` — **later landed #131**). XZ **0** — left-offset killed. Pellet Locus `SMG_PELLET` **14**. Kind **KIND_PVP** (13)
...
- **#133 respawn / join** — after down, **1.20 s** local+relay `KIND_PVP RESPAWN`. Extract net plants on Lab-Rat #132 `World.player_spawns` (8 rim @ **144 m**, Home SPWN+P override/append). One occupant per pad; knock-off until raid reset. Solo `net=off` stays on the yard `World.spawn`. Hideout stays hideout spawn. Not the 22 s bleed bag
- **#133 ragdoll** — hooks only (`apply_peer_hit_react` / `peer_corpse_anchor` / KIND_BODY Drop). Visual flop **later landed #131** Hypha `PeerBody`
- Honesty: two instances agree on **peer presence** (pose + silhouette) + **#102** kit loot trail + **#119** shot / Locus leftover / death bag + **#122** shared raid instance (same raid id + host seed) + **#133** PVP leftover (toggle / hide names / peer hit / KIND_BRASS / rim respawn) + **#139** HOST session board (PVP radio pre-enter · LAN invite, no 127 seed) while menus mute the local pawn only. Range feel / HoB / heat / eject dials / 1P hip / AIM TUNE stay local — **no** full world replication / land feel / H tilt (#84) / audio device (#82) / Graphics dump (#86) / HDRI sun (#87) / Locus brain tick on the joiner / stamps / COL / deform / scatter / Transvoxel rewrite / Augury elbow / hatch UX (#85) / world seed / ToD / Mixamo player body / server browser / dedicated infra
...
- PVP leftover **#133**: after down, **1.20 s** local+relay respawn on a rim pad (not the 22 s bleed bag). Extract net uses `World.player_spawns`. Solo yard stays `World.spawn`
- Parked / still TODO (do not claim done): death cam; teammate net stabilize; timed surface kill; full extract loot loop. Ragdoll visual flop **later landed #131** `PeerBody`
...
- **#131** landed 3P peer presentation / PeerBody (`EYE_Y` **1.60** · `HEAD_H` **1.62** centered · left-offset killed). Hypha owns the visual body. Beabim leftover rays consume those metres
- Do **not** claim Mixamo / full 3P kit honesty

| Seat | Owns |
|------|------|
| **Range Tech** | 1P dials / AIM TUNE (#97 End sheet); #94/#98/#99 hip stay 1P-only |
| **Hypha** | 3P peer presentation / PeerBody **landed #131** |
| **Beabim** | 3P sync path — peer gun stub **landed #91**; peer shot muzzle **#119** uses that hip stub; KIND_* / hitboxes / loot UI (consume Hypha metres); full kit honesty still open |
| **Lab-Rat** | Stamps stay out |

See `PEEK_FINDINGS.md` Holding / locked intent — 1P ≠ 3P + Closed by #91 + Closed by #119 + Closed by #131.
...
| **Glasses** | existing HOST/PEER slot. `  PVP` suffix when on. Not a second HUD. Not Augury Home |
...
| **Torso / limbs** | leftover volumes. Same leftover stubs. Range **2.5× head** later. Mesher stays Hypha |
...
| **KIND** | **KIND_PVP** (13) · **KIND_BRASS** (14). PVP: On / Off / Hit / Respawn. WELCOME seeds a late joiner |
| **Respawn** | **1.20 s** after down. Local + relay `KIND_PVP RESPAWN`. Extract net uses Lab-Rat #132 `World.player_spawns` (8 rim @ **144 m**, Home SPWN+P override/append). One pad per occupant; knock-off until raid reset. Solo stays on the yard `World.spawn`. Hideout stays hideout spawn. Not the 22 s bleed bag |
| **Ragdoll** | hook only — `apply_peer_hit_react` / `peer_corpse_anchor` / KIND_BODY Drop. Visual flop **later landed #131** `PeerBody` |
...
See `PEEK_FINDINGS.md` Closed by #133 + fulcrumRust `docs/PVP_DIAL_SHEET.md`. HOST session board / no-127 invite: Closed by #139.
...
See `PEEK_FINDINGS.md` Closed by #139 + fulcrumRust `docs/PVP_DIAL_SHEET.md`.
...
| **RIDE_SKIN** | **0.06** m — planted-sole hysteresis so a top does not re-enter resolve. Camera `push_out_of_walls` skips a box the eye is standing above |
...
See `PEEK_FINDINGS.md` Closed by #136 + fulcrumRust `docs/LANDMARK_RIDE.md`.

## 3P biped / PeerBody (Hypha — landed #131)

Evan lock. **Shipped** [fulcrumRust #131](https://github.com/initialvisuals/fulcrumRust/pull/131) (2026-09-09, `2fa1d13f`; merge tip `96507be3`). Seat: **Hypha** owns 3P peer presentation / PeerBody. Visual / state feedback for peer bodies — not Range 1P AIM TUNE / HoB / heat, not Beabim KIND_* / hitboxes / loot UI (Beabim consumes death/ragdoll hooks only). Canonical dial sheet: fulcrumRust [`docs/BIPED_3P_DIAL_SHEET.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/BIPED_3P_DIAL_SHEET.md). Overnight cooks steal these numbers from the sheet — not every `.rs`. CREDITS + STEAL_MAP already claimed in-PR.
...
| **Lean tilt** | visual roll + 0.14 m lateral. Not a 1P cam lean rewrite |
...
Do **not** claim Range 1P / AIM TUNE / HoB / heat / Beabim KIND_PVP / KIND_BRASS / HOST / hit tunneling / HP UI / Lab-Rat stamps / pedon / probes / spawns / Augury Home / STREAM amortize / 4× world expand / Mixamo clips / player body / 2-bone IK / GPU skin shipped this.

See `PEEK_FINDINGS.md` Closed by #131 + fulcrumRust `docs/BIPED_3P_DIAL_SHEET.md`.
...
Listen-server + invite stub: fulcrumRust PR #34 (2026-09-07). Handshake-only + no in-game join field **superseded #83**. Leftover INVITE + names + gun stub **landed #91**. Loot trail **landed #102**. World/sim leftover **landed #119**. No-pause + KIND_RAID **landed #122**. PVP leftover **landed #133**. HOST session board / no-127 invite **landed #139**.
...
Hypha landmark AABB ride (local pawn `ground_y` = `ride_surface_y` = max(heightfield, AABB top under footprint) in a **0.50 m** ride band; `RIDE_STEP` **0.50** · `RIDE_SKIN` **0.06** · `SUPPORT_STEP` **0.25** unchanged; walls-as-floor only in the ride band; Locus hurtboxes stay walls; peers `plant_simple_root_on` same column; `slide_xz` / `resolve` skip XZ when `is_ride_top`; #88 FOLLOW/DEADZONE stay; STREAM / Lab-Rat stamps / Range 1P / AIM TUNE / Beabim KIND / leftover PVP / Augury Home / #131 biped untouched — 3P biped **later landed #131**): fulcrumRust PR #136 (2026-09-09) — **landed**. `67bfc1ec`; merge tip `07513e40`. See `PEEK_FINDINGS.md` Closed by #136 + fulcrumRust `docs/LANDMARK_RIDE.md`.
Hypha 3P biped / PeerBody (`EYE_Y` **1.60** · `HEAD_H` **1.62** · `HEAD_HALF_H` **0.11** · XZ **0** left-offset killed; old 3P stub 1.22 dead; crouch squat **0.62** · ragdoll flop **0.55 s** · pose lean unused byte 2 (`i8/127`; + = peek left / E); lean tilt visual roll + 0.14 m lateral; walk/run/jump primitives; hit-react Flinch→Chest / Stagger→Spine; foot plant #88 + #136 ride kept; Mixamo clips / player body / 2-bone IK / GPU skin parked; not Range 1P AIM TUNE / HoB / heat; not Beabim KIND_* / hitboxes / loot UI): fulcrumRust PR #131 (2026-09-09) — **landed**. `2fa1d13f`; merge tip `96507be3`. See `PEEK_FINDINGS.md` Closed by #131 + fulcrumRust `docs/BIPED_3P_DIAL_SHEET.md`.
...
Hypha stream hitch amortize (cook=1/2 · prefetch=5 m · splash-pumped load-in on the existing #81 9×9; `COOK_BUDGET_PLAY` **1** / `COOK_BUDGET_LOAD` **2**; 5 m face prefetch does **not** open a second 9×9; skeleton + splash-pumped load-in; no Sync whole 9×9 dump; emit `STREAM` / `BAKE` `r=` `cold=` `pend=` into #110 logger; walk inside a cell with focus+LOD unchanged stays a no-op; mesher stolen — no greenfield mesher; live octree / residual soft LOD pop inside a cell parked; worker/paint/gpu deepen **later landed #123**; Range poses / AIM TUNE / loot · Lab-Rat UV · Beabim net · Augury Home tabs untouched): fulcrumRust PR #108 (2026-09-09) — **landed**. `b3be2974`. See `PEEK_FINDINGS.md` Closed by #108 + `TERRAIN_NORTHSTAR.md`.
Hypha worker STREAM extract+paint (deepen of #108 amortize on the same #81 9×9; play worker extract+paint — hitch thread never `sample_channels` on play Transvoxel extract; worker owns extract+paint; tick applies one finished mesh then leftover paint/GPU; `defer` **`worker/paint/gpu`**; cook=1/2 · cook_ms 8/16 · prefetch 5 m stay; skip far mask-only remesh when R climbs; STREAM `extract0`/`paint`/`gpu` + hitch-thread ms + r↑/cold↓ into #121 Home LOGS; Hypha emits only — no debugger rewrite; denser WALK/STREAM hang tags **later landed #128** (visibility — not this hitch *fix*); load splash stays sync; CHANNELS stay bake-time; smoke Lab-Rat `probes=off` **and** Hypha `cook_ms`/`defer`; Lab-Rat stamps / Range / Beabim / GATE / Augury Home chrome untouched): fulcrumRust PR #123 (2026-09-09) — **landed**. `b4725b8c`. See `PEEK_FINDINGS.md` Closed by #123 + `TERRAIN_NORTHSTAR.md`.
...
First big-map (drop walls · ~8× extend · chunked Transvoxel · slope COL · local-player stream): Evan dump (2026-09-08) / Hypha **#81 landed**. Stamp pad still 7×7. Stream hitch amortize **later landed #108** on that same 9×9. Worker STREAM extract+paint **later landed #123**. Lab-Rat slope/PBR/dirt/scatter/deform plugs **landed #80**. NRM/GLOSS parked. Beabim peer feet `stream_anchors` **landed #83** (coordinate only). Range Tech kits+FX draw + `dBXpg` still **open**; kit PBR stub + music playlist **landed #64** / Augury FoW brand+menu video **when cut ready**. Atelier PBR batch **in** (150 roughness + textures/PBR ~26 sets). See `TERRAIN_NORTHSTAR.md` + `PEEK_FINDINGS.md` Closed by #81 / #108 / #123 / #80.
Hypha 19×19 open extract + chunk stream + slope COL hooks: fulcrumRust PR #81 (2026-09-09) — **landed**. `73dc8fe4`. Underfoot **32/16/8/4**. Smoke `subdivs=32/16/8/4` `extract_m2=92416` `resident=` `stream_cold=` `pbr=`. Stream hitch amortize **later landed #108**. Worker STREAM extract+paint **later landed #123**. Do **not** claim NRM-GLOSS GPU / world replication / Range heat as this PR. Lab-Rat plugs **landed later #80**. Peer feet `stream_anchors` **landed later #83**. See `PEEK_FINDINGS.md` Closed by #81.
...
Lab-Rat extract player spawn loci (8 rim pads Chebyshev **144 m** facing extract origin; yaw 0 = +Z; `atan2(-x,-z)`; Y from `terrain.height_at`; inside `playable_half_m` 151.25; no far-rim CHANNELS; Home **P** + SPWN `kind:spawn` override within **48 m** XZ else append; same `fulcrum.probes.v1`; `World.player_spawns: Vec<SpawnLocus>`; solo yard `World.spawn` / `spawn_yaw` stays; smoke `spawns=rim=8` or `spawns=rim=N+probe=M`; `probes=off` stays CHANNELS tag; #128 writer of `kind:spawn`; #127 dirt sit leftover; Beabim pool / knock-off **later landed #133** — not this cook's PVP / respawn leftover / biped eye / second MP KIND): fulcrumRust PR #132 (2026-09-09) — **landed**. `67a5a2bb`; merge tip `71c467ed`. See `PEEK_FINDINGS.md` Closed by #132 + Closed by #133 + fulcrumRust `docs/SPAWNS.md`.
