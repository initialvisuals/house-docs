...
| **Persist** | `project.json` in cwd, or `FULCRUM_SETTINGS=/path/to.json`. #82 `output_device` · **#90** `warp_strength` · **#103** `aim_live` / `aim_tune.*` (**#167** per-kit pull — authored kits keep persisted peeks) · **#116** `profile_onboarded` / `preferred_hand` · **#163** `look_mul_h` / `look_mul_v` / `hip_look_mul` / `ads_look_mul` · **#164** `resolution` / `fov_hip` / `aa_type` / `aa_strength` / `ao_quality` / `post_quality` · **#165** `default_hip` live on the same file |
...
| LIVE persist | **landed #103** — Hypha `project.json` · `aim_live` default **true** · End sheet `LIVE` / `OFF`. **#167** per-kit pull — authored kits keep persisted peeks; apply snaps `hold_blend` |
...
| Seat | Owns |
|------|------|
| **Range Tech** | Live End tuner DNA on existing ViewmodelDials + kit_mesh sockets (#97). LIVE persist into Hypha `project.json` (#103). Per-kit pull so authored-sheet flush cannot clobber other guns (#167). PX travel **±0.50** (#138). PX hideout wall floor + home snap (#159) |
| **Augury** | Home debugger **later landed #110**. Do **not** claim shipped by #97 / #103 / #138 / #159 |

See `PEEK_FINDINGS.md` Closed by #97 / Closed by #103 / Closed by #138 / Closed by #159 / Closed by #167.
...
| Seat | Owns |
|------|------|
| **Range Tech** | AIM TUNE LIVE persist on Hypha `project.json` (#103). Per-kit pull (#167). #97 End stays |
...
See `PEEK_FINDINGS.md` Closed by #103. Canted optic + AIM TUNE ATTACH **later landed #150** (`attachments.canted`). AIM TUNE PX hideout wall **later landed #159**. Per-kit pull **later landed #167**.
...
| Live-save schema | #103 | `aim_tune.*` + `aim_live`. No new keys. Per-kit pull **#167** must not clobber other guns |
...
## AIM TUNE LIVE per-kit persist (Range Tech — landed #167)

Evan lock. **Shipped** [fulcrumRust #167](https://github.com/initialvisuals/fulcrumRust/pull/167) (2026-09-10, merge `aef44571`). Seat: **Range Tech** owns End LIVE persist follow-up so three-gun peeks survive relaunch. #103 schema stays (`aim_live` + `aim_tune.*` — no new keys). Hideout and raid share one Hypha `project.json` (`FULCRUM_SETTINGS` or cwd). CREDITS + STEAL_MAP + `AIM_TUNE_LIVE_SAVE` already claimed in-PR. Canonical dial sheet: fulcrumRust [`docs/AIM_TUNE_LIVE_SAVE.md`](https://github.com/initialvisuals/fulcrumRust/blob/main/docs/AIM_TUNE_LIVE_SAVE.md). House pointer: [`AIM_TUNE_LIVE_SAVE.md`](AIM_TUNE_LIVE_SAVE.md). Overnight cooks steal from the sheet — not every `.rs`. Do **not** invent dials.
...
**Still local / other seat (deliberately):** End PX ±0.50 (#159) · Options `look_mul` (#163) · Beabim leftover stash · PreferredHand · powder **B later landed #166** (C parked) · Options READY HIP **later landed #165** · Hypha Graphics guts · Lab-Rat stamps · Augury Home. Two-instance shared `project.json` / flush race — **soft follow**; not in the #167 dial sheet. Do **not** invent a race fix. Do **not** claim Hypha / Lab-Rat / Augury / Beabim shipped this.

See `PEEK_FINDINGS.md` Closed by #167 + house `AIM_TUNE_LIVE_SAVE.md` + fulcrumRust `docs/AIM_TUNE_LIVE_SAVE.md`.
