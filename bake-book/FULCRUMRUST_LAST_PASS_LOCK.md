...
- First big-map host **landed #81** (prior 8× **19×19 / 304 m / 92 416 m²**). Live walk is **#142 37×37 / 592 m / 350 464 m²** + **#137 11×11** stream (`STREAM_RINGS` 5 · hold=2/12 · coalesce=2/2). Stream hitch amortize **landed #108**. Worker STREAM extract+paint **landed #123** (`defer=worker/paint/gpu`). Stamp pad still **7×7**. Slope COL tint default; NRM/GLOSS parked. Lab-Rat slope/PBR/dirt/scatter/deform plugs **landed #80** (DISP bake-down + deform/scatter filled). Blender UV dials **landed #101** (texture tiles only — never geo). Hypha Transvoxel UV consume **landed #112** (`promote_for_uv` + wear/COL honor). Beabim peer feet follow remotes **#83**. Continues on fulcrumRust
