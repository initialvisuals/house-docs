...
| **Hypha** | Stream/voxel hitch *fix* — still open. Persist path shared (`debugger_tab` + `aim_live` / `aim_tune`) |

Out of scope (do **not** claim shipped by #110): Hypha stream/voxel hitch *fix* · shoot/feel · stamps/UV · MP sync · hatch elevator · CE WEAPON/ATTACHMENT tuner (already **End**) · CE PERF/PHYS/RENDER/heartbeat/combat-log flags (house-docs steal later) · full CE debugger tab contract beyond this native shell.

See `PEEK_FINDINGS.md` Closed by #110.
...
- Augury glasses EXTRACT chrome **landed #85** (elbow card, no popup). CE Home debugger **landed #110**. Hatch elevator / toggle / timed surface kill still **open**. Hypha stream hitch *fix* still open. CE PERF/PHYS/RENDER/heartbeat/combat-log flags — house-docs steal later
