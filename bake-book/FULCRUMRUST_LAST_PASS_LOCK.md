...
| **Persist** | `project.json` in cwd, or `FULCRUM_SETTINGS=/path/to.json`. #82 `output_device` · **#90** `warp_strength` · **#103** `aim_live` / `aim_tune.*` (**#167** per-kit pull — authored kits keep persisted peeks) · **#116** `profile_onboarded` / `preferred_hand` · **#163** `look_mul_h` / `look_mul_v` / `hip_look_mul` / `ads_look_mul` live on the same file |
...
**Still local / other seat (deliberately):** Hypha res / FOV slider / AA / AO / post / WARP / STREAM · Lab-Rat texture / mip / `lod_mips` · Augury remap / Tab inventory · PreferredHand onboard · AIM TUNE PX box · stamps · death chrome. Options default hip **Chest (default)** vs **Low** toggle + Powder B — Range still cooking; do **not** claim landed. Do **not** invent implementation. Do **not** claim Hypha / Lab-Rat / Augury / Beabim shipped this.
