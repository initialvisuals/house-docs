...
- **Range Tech**: kits + FX draw-distance on the wider yard; kit metal/grit PBR stub **landed #64**; store `dBXpg` still **open**; Music playlist beds **landed #64**; ADS viewmodel DoF **landed #68**
...
Range Tech Evan peek feel **landed #59**. Hypha ring-mip texture LOD **landed #60**. Hypha near LOD raise **landed #61**. Range Tech handmade atelier SFX vendor **landed #62**. Range Tech music playlist + kit metal/grit PBR stub **landed #64**. Range Tech Patch A muzzle **landed #67** (kit-tip spawn + `hip_honest_dir` + tip→impact streak clamp — hip-fire no longer behind the handguard / upper-right of the reticle). Range Tech ADS viewmodel DoF **landed #68** (ADS near + far on the same #55 pass / Options **DOF**). Further Lab-Rat roughness→stamp still open (atelier PBR batch **in**; Evan **clean** yelled — plugs **open**). Do **not** claim the whole roughness→stamp cook. Do **not** claim Lab-Rat grit/slope/PBR plugs or `dBXpg` shipped.
...
- **Honesty** — changing zero preset changes muzzle **launch dir** only (not muzzle position); arcade vs sim launch dirs differ; sim aims up to meet sight zero; arcade launches along aim. **#67** sits on top — hip fire uses `hip_honest_dir` (ads=0 on aim; ads=1 keeps this solve). **O**/**P** still change the zero; they bite when aimed
...
- **Hip launch** — was SIM 100 m HoB from a right-low hip muzzle → close-range **up + right** of the reticle. Now `hip_honest_dir`: ads=0 stays on **aim**; ads=1 keeps the arcade/sim solve. Uses existing ADS↔hip weight + **P** arcade dir. Not a new cone. **O**/**P** still change the zero; they bite when aimed
...
- Intact: **O**/**P** HoB zero + arcade↔sim (#33); tracers-until-impact + FX `hit` (#59). Heat-card color / orange (Evan / #66) and Lab-Rat terrain untouched. Profile/stash/MP/hands/inventory still open
...
- Intact: #55 GPU stack / far DoF / Options persist. No second composer. No new Graphics sliders. Heat-card color / Lab-Rat / profile / stash / MP / hands / inventory untouched
