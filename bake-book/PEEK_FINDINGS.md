...
- **Range Tech**: kits + FX draw-distance on the wider yard; kit metal/grit PBR stub **landed #64**; store `dBXpg` still **open**; Music playlist beds **landed #64**; ADS viewmodel DoF **landed #68**; heat dial blend **landed #71**; land sway + heightfield FX **landed #79**; Options Audio DEVICE **landed #82**; H shoulder-swap tilt **landed #84**. Patch A RH hip bias **landed #94** (hip **+X 0.1843**; `shoulder_cross_x` **−0.225**; left tilt **0.04 / 0.10 / 0.08**). Aim-offset tuner + attachment sockets **landed #97** (live **End** sheet / glasses `AIM TUNE`; Home stays Augury). HDRI sun disc **landed #87** (shared Hypha; dump **sunSize 0.62** rides the disc). Projectile feel **landed #89** (rect slab + debris + slug/wake + punch/scuff flash + fire-pulse glyphs — Vector mag dump live steal; artistic frame still `VECTOR_MAG_DUMP.md`). Heat cards / ballistics / binds **not touched** by #81. Heat / binds / ballistics / Audio DEVICE **not touched** by #86 / #87. #89 did **not** retune HeatDials / SIM / AXIS_LOCK +Z / #79 snap
...
- **Crossover shoulder / left-corner peek (H)** — authored hip +X ~**0.18** (right; live **#94** **0.1843**). **H** springs the **viewmodel** across the chest to a partial left (~**−0.041**, cap `shoulder_x_min` **−0.055**) — arms-limited, not a capsule/eye slide, not a full mirror, not infinite travel. Extra left probe (`shoulder_viewmodel` **0.12**) helps left-corner leans. ADS keeps **0.32** of the crossover. Viewmodel crossover on the existing H bind (FoW shoulder habit), not a new key. **#94** sits on #84 tilt path: `shoulder_cross_x` **−0.225**; left hold slight straighten pitch/yaw/roll **0.04 / 0.10 / 0.08** (supersedes #84 chest-cross 0.08/0.32/0.39) — not a mesh mirror / no `scale.x = −1`
...
- Live hip X / `shoulder_cross_x` / left tilt **superseded #94** — hip **+X 0.1843**; `shoulder_cross_x` **−0.225**; pitch/yaw/roll **0.04 / 0.10 / 0.08** (slight straighten, not chest-cross cant). See Closed by #94
...
- Intact: #59 travel DNA · #67 tip spawn · #84 tilt path · #89 projectiles. Live **End** tuner on these same dials **later landed #97**. Do **not** claim kit mesh rewrite, heat, audio, Beabim, PreferredHand onboard, or Augury Home tabs/logger as #94
...
- **Authored defaults stay #94** — MP9-Z hip **0.1843 / −0.1688 / −0.1953** · ADS iron X **0.0084** (bore-center) · SR-25 / M24 hip X **0.20 / 0.205**. H travel / slight straighten from #94 stay. Tuner writes the same dials — numbers unchanged until Evan nudges live
- **Follow** — kit boxes + muzzle tip follow live can/optic offsets. World drops stay authored identity. Glasses `AIM TUNE`
- Intact: #94 RH hip / H dials · #67 tip spawn · #89 tracers. Do **not** claim Augury Home tabs/logger · EffectComposer · Beabim · Heat rewrite · Terrain · mesh mirror / `scale.x = −1` · PreferredHand onboard · #94 RH hip retune itself · ballistics / SIM / #76 · #89 tracers · Audio DEVICE
...
- **H** — viewmodel **crossover shoulder / left-corner peek** on the existing FoW H bind (travel landed #59; tilt path **landed #84**; RH hip bias + slight straighten **landed #94**). Not a capsule/eye slide, not a mesh mirror
- **End** — live aim-offset / attachment tuner (landed #97). Feel-lab Home remapped like X→Z. **Home stays unbound / Augury**. Insert WEAPON↔ATTACH · PageDown pose/attachment · PageUp step · ↑↓ axis · ←→ / num± nudge · Delete JSON. Glasses `AIM TUNE`
