...
- **Texture compression** — atelier roughness packs are **4k 48-bit PNG** (too large). Do **not** ship raw 4k 48-bit into the yard. Lab-Rat **#58 landed** the vendored near packs (256² bake-downs under loud scars). Hypha LOD-tied mips **landed #60** on Transvoxel **distance rings** (near 256² / mid 64² / far 16²; far drops grain hashes). Blender UV dials **landed #101** — tile those packs without shrinking geo. Hypha Transvoxel UV consume **landed #112** (`promote_for_uv` + wear/COL honor). Grit/COL LOD luma crawl stop **landed #152** (`STABLE_LOCUS_RES` **16** · `MATERIAL_HOLD_M` **2.5** · `LUMA_GRAIN` ±**0.08**; height/DISP stay bilinear). In-repo grit mips stay #60 until Lab-Rat cooks more. Do **not** claim the whole roughness→stamp cook
...
- Atelier: plugs **open** (Evan **clean** yell 2026-09-08 ~00:00 ET). PBR batch **in** (150 roughness + textures/PBR ~26 sets). #58 / #80 optional `FULCRUM_GRIT=` / `FULCRUM_ATELIER=` stay read-only **load** paths (downsample on load). Lab-Rat slope/PBR/dirt/scatter/deform plugs **landed #80**. Stamp / building / terrain PBR polish **landed #144** (default `pbr=vendor` 256² COL). Grit/COL LOD luma crawl stop **landed #152** (`STABLE_LOCUS_RES` **16** · `MATERIAL_HOLD_M` **2.5** · `LUMA_GRAIN` ±**0.08**). Further roughness → stamp stays on fulcrumRust only — bake-down first; SVG / density-mask / experiment-log still open; do **not** ship raw 4k 48-bit PNG into the yard. Evan has **this week** (from 2026-09-09) to model + texture missing asks — clone CE / aim-offset tables first; primitives stay scaffolding
