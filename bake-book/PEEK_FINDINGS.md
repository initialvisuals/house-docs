...
Parked from Evan’s first full `main` peek (2026-09-07). Growth yard + curl read OK. Later same-day feel dump is **Open** below — Range Tech cooking; do **not** claim shipped.

## Open — Evan peek 2026-09-07 (Range Tech cooking; do not claim shipped)

Cooking on Range Tech after the #57 medium pass. Steal closer. Do **not** mark these as landed.

**Locks 2026-09-07** — double-jump **unlocked** (Evan override of FPS-first "no double-jump"); **crossover shoulder / left-corner peek** (Range Tech feel PR); texture LOD compress (Lab-Rat bake-down / Hypha ring mips — **#58 landed** the first quiet grit greyscale wiring, vendored bake-downs; whole roughness→stamp / ring-mip cook is **not** done); atelier still **read-only**. #25 lean / #57 medium dials stay shipped as-is. #51 Space hop is what `main` has until the Range Tech feel PR lands.

- **Aim-offset ballistics / distant impact SFX** — feel better than fulcrumRust; steal closer. Authored-SFX **real packs** still cooking (#54 wiring + placeholders only)
- **Heat cards** — fulcrumRust bright-orange static is wrong; match aim-offset **motion / look**. Locked dials stay on house `heat-card-dial-sheet.md` (aim-offset v77)
- **Lean** — still reverse + too shallow after #25. **Flip + deepen** from aim-offset / CE. Cooking direction — do not claim the flip landed
- **Crossover shoulder / left-corner peek** — left-hand hold; gun crosses chest **right → partial left** for left-corner peeks; more **left spacing** on leans. Not a body slide. Not full weapon swap / infinite travel. Range Tech feeding into feel PR. **H** stays FoW shoulder swap (camera side) — this is viewmodel crossover, not a new H bind. Quiet influence — no franchise name-drop. Do **not** claim shipped
- **Jump (unlocked)** — steal CE **double-jump + land duck + cam shake**. Range Tech cooking feel PR. Prior FPS-first **"no double-jump"** / #51 single-hop-only is superseded. #51 Space hop still what `main` has — do **not** claim the cook landed
- **Texture compression** — atelier roughness packs are **4k 48-bit PNG** (too large). Do **not** ship raw 4k 48-bit into the yard. Lab-Rat **#58 landed** the first quiet grit greyscale wiring on fulcrumRust (vendored 256² bake-downs under loud scars). Hypha LOD-tied mips / compression hooked to Transvoxel **distance rings** still open — do **not** claim the whole roughness→stamp / texture-LOD cook
...
- **Lab-Rat** — **#58 quiet grit greyscales landed** (vendored bake-downs + `sample_channels` quiet height + `grit::rough` wear). Further roughness → stamp stays on **fulcrumRust only**; bake-down first (do not write atelier packs). SVG / density-mask / experiment-log still open
...
- **Evan dizzy-play** (supersedes stale #12 wording where it conflicts):
  ...
  4. **Jump** — **Space** single hop shipped #51 (FoW/aim-offset). **Superseded 2026-09-07:** Evan unlocks double-jump — steal CE double-jump + land duck + cam shake. Range Tech cooking feel PR. Prior FPS-first "no double-jump" / single-jump-only is superseded (same way #51 superseded earlier "no jump"). Do not claim the cook landed
- Do not invert Q/E lean — or unify the three forwards — to "fix" FX. #12 look/move/gun + tracers and #25 lean clamp stay
...
- Out of scope: Range Tech controller / lean / jump / heat / ballistics. Augury Locus. Hypha Transvoxel tables. Atelier repo writes. Hypha ring-mip texture LOD compress still open
...
- **Q / E** — peek left / right. Cooking: **crossover shoulder / left-corner peek** — more left spacing on leans (Range Tech feel PR; not a body slide)
...
- Range Tech: day/night clock + sky (#24), wall-clamped lean (#25), hold-` inspect (#28), bandage use (#31) landed; **#32 reload DNA landed** (Hold-R peek / tap-R reload / double-tap SWAP); **#33 live HoB zero / arcade↔sim launch landed**; **#35 heat-tune dump landed** (hold-J; **I** is Augury stim #37); **#40 Goegap HDRI on extract ToD landed** (/** plate toggle; glasses `HDRI` / `PROC`); **#47 leftover feel-lab FX landed** (brass eject / graze ricochet + spent slug / richer impact geo / `casing_draw_m` **55**); **#51 AXIS_LOCK landed** (cam −Z / CE +X / barrel +Z; Lab-Rat +Y separate; FX on `sim_barrel_basis`; dizzy-play invert look/strafe + F-only door; **jump unlocked** 2026-09-07 — single-hop-only superseded); Voice/Music/FX buses (#21) carry Hypha/#27 spatial + #47 ricochet ping (**FX bus live**); **authored SFX file-slot wiring shipped #54** (placeholders; weapon/move off CE/FoW packs into those buses; rustles / rattles / slides come over) — authored-SFX **real packs** still cooking (do **not** claim CE/aim-offset WAV packs shipped); one-click Windows `build.bat` **#48 stay-open + `build.log` tee landed**; quality/flag options still cooking / open (Lab-Rat mirror for pycelium later — no dials invented here); **feel medium polish landed #57** (look inertia queue **26**; ADS look **0.86** / blend **6.4**; sprint high-ready **6.2**; slide carry **10.3 / 0.98 / 1.02**; jump land punch **0.052** rad overlay; AXIS_LOCK stay; no materials/range geo); **Evan peek 2026-09-07 leftover cooking** (do **not** claim shipped) — steal closer aim-offset ballistics / distant impact SFX; heat cards match aim-offset motion/look (not bright-orange static; locked dials); lean flip + deepen (still reverse / shallow after #25); **crossover shoulder / left-corner peek** — gun crosses chest right → partial left, more left spacing on leans (not a body slide / not full swap / not infinite travel; **H** stays FoW shoulder swap); **double-jump unlocked** — steal CE double-jump + land duck + cam shake (Range Tech cooking feel PR)
