...
Cooking on Range Tech after the #57 medium pass. Steal closer. Do **not** mark these as landed. #25 lean / #51 Space hop / #57 medium dials stay shipped as-is until Range Tech cooks them.
...
- **Jump** — still shallow after #51 / #57 land punch. Pull CE **double-jump + land duck + cam shake** from polished CE / FoW controller. Cooking — #51 Space single hop still what `main` has; do not claim double-jump landed
- **Atelier** — open for **read** (HDRI + small roughness sample landed). Crew must **not write** while Evan pushes
- **Lab-Rat** — wire roughness → stamp on **fulcrumRust only** (do not write atelier packs)
...
- No double-jump day-one — **Space** single hop locked by #51 (supersedes earlier "no jump")
