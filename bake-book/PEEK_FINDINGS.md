...
- **Evan dizzy-play look** — **subtract** mouse X (invert horizontal); **invert A/D** vs the #12 camera-right lock. Q/E lean signs stay +lean = left (#25). Do not invert lean — or unify the three forwards — to "fix" FX
- #12 look/move/gun + tracers and #25 lean clamp stay; this lock does not restyle the three forwards into one frame
- Detail: house `AXIS_LOCK.md` + fulcrumRust `docs/AXIS.md` / `engine/src/axis.rs`
