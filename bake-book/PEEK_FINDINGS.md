...
- **Controls** — Look scale sits on feel-lab sens: `LOOK_MUL` default **1.0**, min **0.25**, max **2.0**, step **0.05**; Invert Y toggle. Binds stay README. Hint: `LOOK SITS ON FEEL-LAB SENS · BINDS IN README`. **Later landed #163** — split **MOUSE H/V · HIP · ADS** on that same lock (persist `look_mul_h` / `look_mul_v` / `hip_look_mul` / `ads_look_mul`)
...
- Intact / out of scope: Hypha res / FOV slider / AA / AO / post **later landed #164** · WARP / STREAM · Lab-Rat texture / mip · Augury remap / Tab · PreferredHand · AIM TUNE PX · stamps · death chrome. Options default hip **Chest (default)** vs **Low** toggle + Powder B — Range still cooking; do **not** claim landed. Do **not** invent implementation
...
- **Options Graphics / RES / FOV** — Hypha. House `OPTIONS_SHELF` Resolution / Graphics / FOV seat. [PR #164](https://github.com/initialvisuals/fulcrumRust/pull/164) (merge `159b3d11`). Seat: **Hypha**. CREDITS + STEAL_MAP + `OPTIONS_GRAPHICS_DIAL_SHEET` already claimed in-PR — house shelf only. Do **not** invent dials. Do **not** invent A-note PRs. Do **not** claim Range / Lab-Rat / Augury / Beabim shipped this
- **Closed (Graphics rows):** **RES** beside WINDOW · **FOV** hip base · **AA** FXAA/OFF · **AA STR** live #55 mix · **AO** OFF/LOW/HIGH · **POST** OFF/LOW/HIGH master. Augury **#45** chrome. Persist `project.json`
- **Closed (persist):** `resolution` / `fov_hip` / `aa_type` / `aa_strength` / `ao_quality` / `post_quality` on the existing Hypha save path. Old `"ao"` / `"aa"` bools still load. New keys win when present
- **Dial sheet:**

| Dial | Persist | Lock |
|------|---------|------|
| **RES** | `resolution` | Default **NATIVE** (desktop native — no invented pair). Shelf presets + UW 4K CLASS (3840×1600 class) + Evan 3440×1440 + Steam survey fill **2560×1600 / 1920×1200 / 1366×768**. Borderless stays desktop-native |
...
- Intact / out of scope: Lab-Rat texture / mip · Range mouse V/H + hip/ADS (**#163** already shelved) · Range READY HIP Chest/Low · Augury remap / Tab · STREAM `StreamPolicy` · Powder / PreferredHand / ballistics / AIM TUNE · Beabim net · TAA · bloom / godRays / brightness / gamma. Do **not** invent implementation
...
## Holding / locked intent — Options shelf (remap / Tab still holding; Graphics/RES/FOV landed #164 · mouse landed #163)

Evan lock (2026-09-10 InitialVisuals). **Partial shipped #163+#164** — Graphics/RES/FOV **landed #164** · Controls mouse **landed #163** · remaps/Tab still **holding**. Overnight cooks steal from `OPTIONS_SHELF.md` + fulcrumRust `docs/OPTIONS_GRAPHICS_DIAL_SHEET.md` + `docs/OPTIONS_MOUSE_SENS_DIAL_SHEET.md` — not chat. Do **not** invent numeric defaults unless already tip-locked elsewhere. No code.
...
- **Closed (#164):** Graphics **RES / FOV / AA / AA STR / AO / POST**. Persist `resolution` / `fov_hip` / `aa_type` / `aa_strength` / `ao_quality` / `post_quality`. Old `ao` / `aa` bools still load. RES default **NATIVE**. FOV hip **90** (optic ADS locks still win). AA **FXAA / OFF**. AA STR **0.5** live #55 mix. AO **OFF / LOW / HIGH** (Low 8-tap · r **0.55** · High Mycelium 24-tap). POST **OFF / LOW / HIGH** default **HIGH**
- **Intent (still holding):** remaps, Tab inventory press-toggle. Refer Concrete Echo for remap patterns. Lab-Rat texture / mipmap Options rows still holding. TAA later. Bloom / godRays still no path
- **Resolution:** **landed #164.** Dropdown Native · 1280×720 · 1920×1080 · Ultrawide 2560×1080 · 2560×1440 · 3840×2160 (4K) · UW 4K CLASS (3840×1600 class) · 3440×1440 (Evan native curved UW) · Steam survey fill **2560×1600 / 1920×1200 / 1366×768**. Borderless stays desktop-native
- **Graphics:** AA Type / AA strength / AO quality / post-processing quality **landed #164**. Texture quality · mipmap quality still Lab-Rat holding
- **Camera / feel:** FOV slider **landed #164**. Mouse V/H + hip/ADS **landed #163**
- **Input:** remapping missing — steal CE patterns (Augury). Tab → inventory: press to open, press again to close — **no hold-flash** while bouncing Tab. **Still holding**
- **STREAM:** `StreamPolicy` window/wide/resident already Hypha — link TERRAIN / LAST_PASS; do not re-own here
- **Hip pose (not shipped):** Evan locked Options default hip **Chest (default)** vs **Low** toggle. In-play two poses only (chosen hip + canted). Shotgun may force Low later. Range still cooking this tip + Powder B separately. Do **not** claim the hip toggle or Powder B landed. Do **not** invent implementation
...
| **Range Tech** | Mouse V/H + hip/ADS **landed #163**; AIM TUNE PX-wall cook parallel. Options default hip Chest vs Low + Powder B still cooking |
| **Augury** | Input remapping (CE), Tab inventory press-toggle no hold-flash |
| **Beabim** | Off unless net-related |
| **Clerk** | `OPTIONS_SHELF.md` + `OPTIONS_GRAPHICS.md` + `OPTIONS_MOUSE_SENS.md` |

Invented default numbers parked. Powder A/B/C parked (Powder B still cooking — do **not** claim landed). PX clamp fix is Range cook, not this sheet’s implementation. Do **not** claim remaps / Tab press-toggle / texture/mipmap Options rows / TAA / bloom / hip toggle shipped. Graphics/RES/FOV **are** shipped #164. V·H·ADS split **is** shipped #163.
