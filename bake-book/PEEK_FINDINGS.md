...
- **Texture compression** — atelier roughness packs are **4k 48-bit PNG** (too large). Lab-Rat: bake greyscales **down before density** (8-bit / half-res / BC4-style height packs); do **not** ship raw 4k 48-bit into the yard. Hypha: LOD-tied mips / compression hooked to Transvoxel **distance rings** (grit vs loud scars)
- **Atelier** — still **read-only** for crew writes while Evan pushes (HDRI + small roughness sample landed)
- **Lab-Rat** — wire roughness → stamp on **fulcrumRust only**; bake-down first (do not write atelier packs)
...
- Atelier: still **read-only** for crew writes while Evan pushes (HDRI + small roughness sample landed). Lab-Rat wires roughness → stamp on fulcrumRust only — bake-down first; do **not** ship raw 4k 48-bit PNG into the yard
