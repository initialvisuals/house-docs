...
Parked from Evan’s first full `main` peek (2026-09-07). Growth yard + curl read OK. Same-day feel dump **landed #59**. Hypha ring-mip texture LOD **landed #60**. Hypha near LOD raise **landed #61**. Range Tech handmade atelier SFX vendor **landed #62**. First big-map brief (2026-09-08) is **Holding** — do **not** claim shipped.
...
- **Hypha** host: drop outer walls · extend **~8×** · chunked Transvoxel · **load chunks by distance from players** (listen-server aware — #34 handshake exists; terrain sync still parked). Near LOD raise **already landed #61** (32/16/4)
- **Lab-Rat** stamps: slope/angle materials · dirt/scatter/deform · PBR bake-down. Atelier **150 roughness + textures/PBR ~26 sets landed**; plugs wait for Evan **clean** yell. #58/#60 stay the only yard plugs until then
- **Range Tech**: kits + FX draw-distance on the wider yard; store `dBXpg` after clean
- **Augury**: menu video bg / brand after clean

Do **not** claim the 8× map, wall drop, chunk stream, slope materials, or PBR plugs shipped. See `TERRAIN_NORTHSTAR.md`.

## Open — texture / atelier leftovers (roughness→stamp still open)

Range Tech Evan peek feel **landed #59**. Hypha ring-mip texture LOD **landed #60**. Hypha near LOD raise **landed #61**. Range Tech handmade atelier SFX vendor **landed #62**. Further Lab-Rat roughness→stamp still open (atelier PBR batch **in**; plugs wait for **clean**). Atelier still **read-only**. Do **not** claim the whole roughness→stamp cook.
...
- **Atelier** — still **read-only** for crew writes. PBR batch **in** (150 roughness + textures/PBR ~26 sets). #58 optional `FULCRUM_GRIT=` / `FULCRUM_ATELIER=` are read-only load paths. No atelier writes. Plugs wait for Evan **clean** yell
- **Lab-Rat** — **#58 quiet grit greyscales landed** (vendored bake-downs + `sample_channels` quiet height + `grit::rough` wear — the near source for #60). Further roughness → stamp stays on **fulcrumRust only**; bake-down first (do not write atelier packs). SVG / density-mask / experiment-log still open. Slope/angle + scatter/deform sit on the big-map brief
- Day-one FILE_SLOTS vendor **landed #62**. Shot propagation / full CE pack dump still later — that is audio files, not the #59 feel dials
