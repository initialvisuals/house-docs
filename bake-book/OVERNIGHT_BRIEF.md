...
- **Range Tech** — kits + FX draw-distance on the wider yard; kit metal/grit PBR stub **landed #64**; store `dBXpg` still **open**; Music playlist beds **landed #64**. **1P viewmodel ≠ 3P biped gun** (MP honesty) is **holding / locked intent** — steal from `FULCRUMRUST_LAST_PASS_LOCK.md` / `PEEK_FINDINGS.md`. Range Tech owns 1P dials / AIM TUNE; Beabim owns the 3P gun path. Do **not** claim #83 5-box is full 3P kit honesty. Lab-Rat stamps stay out. **Not shipped.**
...
| **Range Tech** | Quality/flag options on the #42 `build.bat` still cooking / open. Kits + FX draw-distance retune on the wider yard. Store `dBXpg` still **open** (kit metal/grit PBR stub **landed #64**). Music playlist beds **landed #64**. **SFX remix DNA** (pitch/speed/effects; indie underground; don’t overuse the same stem) — first ±6% fire/foot/reload jitter **landed #64**; full remix minting still **open**. **Embodied feel pass landed #57** / **Evan peek landed #59** / **#62 SFX vendor landed** / **#67 Patch A muzzle landed**. **1P ≠ 3P** locked intent — 1P / AIM TUNE stay Range; do **not** push artistic 1P poses onto the peer mesh. **Scope glass** — AIM TUNE placements first (#97); Hypha owns glass when LPVO. **Asset-ask** — clone CE / aim-offset attachment tables first; missing → ask Evan this week |
| **Beabim** | **1P viewmodel ≠ 3P biped gun** — locked intent, **not shipped**. 3P gun / gear path still **open**. #83 5-box is pose presence only — not full 3P kit honesty. Steal from `FULCRUMRUST_LAST_PASS_LOCK.md` |
