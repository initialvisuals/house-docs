...
- **World depth:** stay **shallow** unless a compound needs a basement — not a deep tunnel sim. **#114** is a shallow enterable Subtract network under the pad (mouth bowl `CRAWL_DROP` **0.38** m, above `SLAB_Y0` **−0.7**) — not a tunnel sim. **#130** sandbox pedon is an off-stream leftover overlay on a table (CHANNELS iterate without Hypha STREAM remesh) — not deeper guts, not the #114 pad crawl
