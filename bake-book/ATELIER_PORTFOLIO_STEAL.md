# ATELIER_PORTFOLIO_STEAL — Initial Visuals
...
**Clean yell (2026-09-08 ~00:00 ET).** Evan yelled **clean**. Atelier plugs **open** (was read-only while he pushed). HDRI + PBR batch stay landed. Crew may plug — bake-down first; do **not** ship raw 4k 48-bit. Lab-Rat **#58** quiet grit + Hypha **#60** ring-mips + Lab-Rat **#80** slope/PBR/dirt/scatter/deform stay the **live** yard plugs. `FULCRUM_GRIT=` / `FULCRUM_ATELIER=` stay read-only **load** paths. Further roughness → stamp stays on fulcrumRust. See `PEEK_FINDINGS.md` Holding.

**PBR batch in (2026-09-08).** Atelier **150 roughness + textures/PBR ~26 sets landed**. Lab-Rat slope/PBR/dirt/scatter/deform plugs **landed #80**. Range Tech kit metal/grit PBR stub + Music playlist **landed #64**; store `dBXpg` still **open**. Augury FoW brand / menu video **when cut ready**. Hypha big-map host **landed #81**. See `TERRAIN_NORTHSTAR.md`.

**Holocron gift (2026-09-08).** Atelier `tools_for_ai_and_dev/Holocron_Visualizer.py` + `Analyze-Holocron.ps1` — tree nested-rectangle viewer for file bases. Cut down monoliths (agent context windows; avoid overwrite loss). Slope/PBR plugs **landed #80**. Lab-Rat owns a rust-friendly rewrite still waiting on SVG / density-mask / monolith splits. Later targets: `channels.rs` / stamp stacks / `feel` / `kit_mesh`. See `TOOLS.md`.
...
2. Store `dBXpg`: **50 Handcrafted 4K Greeble/Tech/Trim/Panel Maps** (~$10 Extended) — **Range Tech** shelf seed (doors/trims/circuitry/grids/panels/greebles). **Open**/missing (clean yell). Kit metal/grit PBR stub **landed #64** from TRIMSHEET_MICRO + MetalPanel/Corroded + scratch/print — **not** `dBXpg`. Do **not** claim the greeble pack plugged.
3. Process: greyscale maps as roughness + bump + blend-masks; Hard Ops folder scroll.
4. Process: Alien Isolation hard-surface trim language (3D Prop Design + Mixer MKii 1:1).
5. Process: Cthonic Binary-style Blender procedural texture loops for stamp generation.

## Seat ownership (plugs open)

- **Lab-Rat** — imperfections + greyscale seamless → stamp height/rough/mask. Bake greyscales **down before density** (8-bit / half-res / BC4-style). Do not ship raw 4k 48-bit into the yard. **#58** first landed 256² bake-down set on fulcrumRust. Atelier **150 roughness + textures/PBR ~26 sets landed**. Slope/PBR/dirt/scatter/deform plugs **landed #80**. NRM/GLOSS GPU parked. Holocron rust rewrite still waits on SVG / density-mask / monolith splits — see `TOOLS.md`
- **Range Tech** — greeble/tech/panel pack + kit metal/rust; Mixer/Alien Isolation mood. Store `dBXpg` still **open**. Kit metal/grit PBR stub **landed #64**. Music playlist beds **landed #64**. Kits + FX draw-distance on the wider yard (big-map brief)
...
- Paths for leftover cobble / brand/UI kit inbound pushes (plugs no longer gated on a clean yell)
