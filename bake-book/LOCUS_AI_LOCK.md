...
- **Lab-Rat** owns stamps / growth plots (Standard pad **right of creeper**; Inked pad **left of 2D webbing**; ink disc is Augury chrome, not a growth stamp)
...
| Yard pad `YARD_INKED` | **(−5.10, 0, 8.20)** — player-left of 2D webbing; darker/hooded/thinner + cyan eyes + cheap ink-zone disc under feet |

## Glasses (labels only)
Near body: `LOCUS  STANDARD  IDLE|ALERT|…` or `LOCUS  INKED  IDLE|ALERT|…` — Augury smart-glasses, never a second ammo HUD (`AESTHETIC_DIEGETIC_LOCK`).
...
- **Inked:** look slightly **left** of 2D webbing → dark hood, cyan eyes, ink disc under feet
LMB (or seated kit) wounds either; far map guts stay cold — only these two yard bodies warm.

Source: fulcrumRust `engine/src/locus.rs` + PR #18 + PR #26; shared gate `engine/src/activation.rs` + Hypha PR #23. Steal map Augury Locus row → **partial** (Standard + Inked shipped; family TODO remains).
