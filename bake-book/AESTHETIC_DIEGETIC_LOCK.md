...
Hold-**O** paints EXTRACT intent on the nearest in-front hatch/shaft — elbow card, **no popup** (Range #78 wired `Session::extract_checking`; Augury owns the glasses EXTRACT chrome — **landed #85**). Release clears. Hold-**F** hatch OPEN/CLOSED + shaft RIDE/EXIT chrome **landed #115** (idle `HATCH  HOLD F  OPEN` / `SHAFT  HOLD F  RIDE`; holding `HOLD F  OPEN  42%`). Hold-O EXTRACT reads `OPEN` / `CLOSED` / `SHAFT`. No modal popup. Timed surface kill still **~**.
...
Elbow smart-labels (fulcrumRust #85): interact cards sit off-center with an L-leader to the world pin — still labels only, never a second ammo HUD. Hold-**O** `EXTRACT` is that same chrome (no popup). Hatch toggle + shaft ride **landed #115** — hold-F OPEN/CLOSED + RIDE/EXIT; hold-O EXTRACT reads `OPEN` / `CLOSED` / `SHAFT`. Timed surface kill still **~**. Cite `FULCRUMRUST_LAST_PASS_LOCK.md`.
