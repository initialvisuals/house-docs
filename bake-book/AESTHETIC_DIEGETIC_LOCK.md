...
Shipped label examples: stamp materials · `LOCUS  STANDARD|INKED  <brain>` · `INK HOTSPOT` · ToD/exposure · `HDRI` / `PROC` (Goegap plate #40) · `INSPECT` (hold-` #28) · `RELOAD` / `SWAP` (mag swap #32) · `BANDAGE` / `EMPTY` (bandage use #31) · `Z{n}  SIM` (HoB zero #33 / SIM-only #76) · `HEAT TUNE` (hold-J heat-tune #35) · `AIM TUNE` (End tuner #97) · `DEBUG` (Home debugger #110) · `HOST` / `JOIN` / `PEER` / `INVITE` (listen-server #34 / #83 / **#91**) · `DOWNED` · `DEAD` · `STIM` / `NO STIM` · `RALLY` · `NEED STAB` · `STAB STUB  NO NET` (down/death stub #36; #37 bind `I STIM`) · `DRY` / `YARD` / `OUT` (reverb volumes #56) · `EXTRACT` (hold-O #85; **#115** `OPEN` / `CLOSED` / `SHAFT`; **#120** `EXTRACT  N%` while armed) · `HATCH` / `SHAFT` / `HOLD F  OPEN` (hatch #115) · `DOOR  F  DEPLOY` / `DOOR  DEPLOY  N%` (#120, analysis-core white mono) · lean/slide/speed/height peeks.
...
Live HoB zero / launch (fulcrumRust #33 / **#76** SIM-only): glasses status strip `Z{n}  SIM` (e.g. `Z100  SIM`); toast `ZERO  {n} M` — labels only, never a numeric ammo HUD. Arcade↔sim **P** unused; `LAUNCH  ARCADE` / `LAUNCH  SIM` gone. **P** drop look-at probe **later landed #125** (Home debugger — not a launch toggle).
...
Aim-offset tuner (fulcrumRust #97): glasses may flash `AIM TUNE` while End is open — still labels only, never a second ammo HUD. Home debugger **landed #110** / LOGS COPY **#121** / occlusion + probes **#125** — glasses may flash `DEBUG` while the overlay is open — still labels only, never a second ammo HUD.
...
- CE Home debugger (fulcrumRust #110 / LOGS COPY **#121** / occlusion + probes **#125**): **Home** opens the native overlay (**LOGS / TELE / COLL / PERF / SPWN / CHEAT**). Analysis-core chrome — thin white frames, fade-in white mono, fills under glyphs (overlay buffer grows so LOGS `ENTER COPY` is not eaten). INV/STATUS / HP-AR / Options fog-cam-WARP / JOIN HOLD unsmashed. **P** drops labeled look-at probes; TELE/COLL/PERF **Enter** copies `fulcrum.probes`. Glasses may flash `DEBUG` while open — still labels only, never a second ammo HUD. Augury owns chrome. **End** stays AIM TUNE
...
- Live HoB zero / launch (fulcrumRust #33 / **#76** SIM-only): glasses may show `Z{n}  SIM` (e.g. `Z100  SIM`) and toast `ZERO  {n} M` — still labels only, never a numeric ammo HUD. Arcade↔sim **P** unused; `LAUNCH  ARCADE` / `LAUNCH  SIM` gone. **P** drop look-at probe **later landed #125** (Home debugger — not a launch toggle)
