...
Aim-offset tuner (fulcrumRust #97): glasses may flash `AIM TUNE` while End is open — still labels only, never a second ammo HUD. Home debugger **landed #110** — glasses may flash `DEBUG` while the overlay is open — still labels only, never a second ammo HUD.
...
- CE Home debugger (fulcrumRust #110): **Home** opens the native overlay (LOGS / TELE / CHEAT). Analysis-core chrome — thin white frames, fade-in white mono. Glasses may flash `DEBUG` while open — still labels only, never a second ammo HUD. Augury owns chrome. **End** stays AIM TUNE
