...
Hold-**O** paints EXTRACT intent on the nearest in-front hatch/shaft — elbow card, **no popup** (Range #78 wired `Session::extract_checking`; Augury owns the glasses EXTRACT chrome — **landed #85**). Release clears. Hold-**F** hatch OPEN/CLOSED + shaft RIDE/EXIT chrome **landed #115** (idle `HATCH  HOLD F  OPEN` / `SHAFT  HOLD F  RIDE`; holding `HOLD F  OPEN  42%`). Hold-O EXTRACT reads `OPEN` / `CLOSED` / `SHAFT`. Door / extract cancel chrome **landed #120** — **F** arms `DOOR  DEPLOY  N%`; OPEN quiet hatch arms `EXTRACT  N%` (same HOLD % language; analysis-core white mono, not gold). Hold-O still intent only — countdown wins while armed. Walk away cancels. No modal popup. Timed surface kill still **~**.
...
Shipped label examples: stamp materials · `LOCUS  STANDARD|INKED  <brain>` · `INK HOTSPOT` · ToD/exposure · `HDRI` / `PROC` (Goegap plate #40) · `INSPECT` (hold-` #28) · `RELOAD` / `SWAP` (mag swap #32) · `BANDAGE` / `EMPTY` (bandage use #31) · `Z{n}  SIM` (HoB zero #33 / SIM-only #76) · `HEAT TUNE` (hold-J heat-tune #35) · `AIM TUNE` (End tuner #97) · `DEBUG` (Home debugger #110 / #121 / #125 / **#128**) · `HOST` / `JOIN` / `PEER` / `INVITE` (listen-server #34 / #83 / **#91**) · `DOWNED` · `DEAD` · `STIM` / `NO STIM` · `RALLY` · `NEED STAB` · `STAB STUB  NO NET` (down/death stub #36; #37 bind `I STIM`) · `DRY` / `YARD` / `OUT` (reverb volumes #56) · `EXTRACT` (hold-O #85; **#115** `OPEN` / `CLOSED` / `SHAFT`; **#120** `EXTRACT  N%` while armed) · `HATCH` / `SHAFT` / `HOLD F  OPEN` (hatch #115) · `DOOR  F  DEPLOY` / `DOOR  DEPLOY  N%` (#120, analysis-core white mono) · `SIGNAL  THINS` / `CARRIER  LOST` (soft rim leftover **#156**; thin hold **#158** spring-stop `approaching && t >= 0.995` stays `CARRIER  LOST`; mid-glasses radio, not an error toast; mute under Slain) · lean/slide/speed/height peeks.
...
Live HoB zero / launch (fulcrumRust #33 / **#76** SIM-only): glasses status strip `Z{n}  SIM` (e.g. `Z100  SIM`); toast `ZERO  {n} M` — labels only, never a numeric ammo HUD. Arcade↔sim **P** unused; `LAUNCH  ARCADE` / `LAUNCH  SIM` gone. **P** drop look-at probe **later landed #125** (Home debugger — not a launch toggle). **#128** SPWN+P / Insert **SPAWN** writes `kind:spawn`; surface `name` is `ground` / … — not mesh id `extract`.
...
Aim-offset tuner (fulcrumRust #97): glasses may flash `AIM TUNE` while End is open — still labels only, never a second ammo HUD. Home debugger **landed #110** / LOGS COPY **#121** / occlusion + probes **#125** / binds lock **#128** — glasses may flash `DEBUG` while the overlay is open — still labels only, never a second ammo HUD.
Listen-server (fulcrumRust #34 / Beabim #83 / **#91**): glasses may show `HOST  fulcrum://ip:port`, then `JOIN` / `PEER` after HELLO/WELCOME — still labels only, never a second ammo HUD. **#91** leftover INVITE sheet uses the same JOIN analysis-core chrome. Peer names fade-in white mono over the remote head (hairline only) — **HOST** / **P{id}** or `FULCRUM_NAME`. Does **not** steal Augury elbow cards (door / loot / hatch / EXTRACT).
...
Elbow smart-labels (fulcrumRust #85): interact cards sit off-center with an L-leader to the world pin — still labels only, never a second ammo HUD. Hold-**O** `EXTRACT` is that same chrome (no popup). Hatch toggle + shaft ride **landed #115** — hold-F OPEN/CLOSED + RIDE/EXIT; hold-O EXTRACT reads `OPEN` / `CLOSED` / `SHAFT`. Door / extract cancel chrome **landed #120** — **F** arms `DOOR  DEPLOY  N%`; OPEN hatch arms `EXTRACT  N%` (analysis-core white mono, not gold). Soft rim leftover **landed #156** — `SIGNAL  THINS` / `CARRIER  LOST` mid-glasses radio on Hypha `RimHook` (not an error toast; mute under Slain; thin hold **#158** spring-stop `approaching && t >= 0.995` stays BLOCK / `CARRIER  LOST`). Timed surface kill still **~**. Cite `FULCRUMRUST_LAST_PASS_LOCK.md`.
...
- Aim-offset tuner (fulcrumRust #97): **End** opens the live viewmodel / attachment offset sheet on existing ViewmodelDials + kit_mesh optic/can sockets; glasses `AIM TUNE` only — never a numeric ammo HUD. #97 End tuner nudges **#98** authored defaults. **#161** Insert cycles WEAPON → ATTACH → **MODEL**; MODEL writes Beabim `leftover_hit` (`hitscan` / `ballistic_A`) — still labels only, never a second ammo HUD
- CE Home debugger (fulcrumRust #110 / LOGS COPY **#121** / occlusion + probes **#125** / binds lock **#128**): **Home** cycles **off → LOGS → TELE → COLL → PERF → SPWN → CHEAT → off** (not a separate unbound toggle). **Arrows + Enter** navigate; **Insert** tabs page options. **WASD / Space** stay move / hop — they do not drive Home chrome. Analysis-core chrome — thin white frames, fade-in white mono, fills under glyphs (overlay buffer grows so LOGS `ENTER COPY` is not eaten). INV/STATUS / HP-AR / Options fog-cam-WARP / JOIN HOLD unsmashed. **P** drops labeled look-at probes; SPWN+P / Insert **SPAWN** writes `kind:spawn`; surface `name` is `ground` / … — not mesh id `extract`. TELE/COLL/PERF **Enter** copies `fulcrum.probes`. Glasses may flash `DEBUG` while open — still labels only, never a second ammo HUD. Augury owns chrome. **End** stays AIM TUNE
- **1P viewmodel ≠ 3P biped + weapons + gear** (MP honesty, 2026-09-09): artistic 1P posing (hip fire / canted CQC) may stay aggressive; peers must not see guns through eyeballs. **Partial shipped #91** — peer biped hip gun stub. **#153** held 3P authored `kit_boxes` clone on PeerBody lean. Hands / gear / Mixamo still open. Range Tech owns 1P / AIM TUNE; Beabim owns 3P sync. Do **not** claim Mixamo / full kit honesty. See `FULCRUMRUST_LAST_PASS_LOCK.md`
...
- Live HoB zero / launch (fulcrumRust #33 / **#76** SIM-only): glasses may show `Z{n}  SIM` (e.g. `Z100  SIM`) and toast `ZERO  {n} M` — still labels only, never a numeric ammo HUD. Arcade↔sim **P** unused; `LAUNCH  ARCADE` / `LAUNCH  SIM` gone. **P** drop look-at probe **later landed #125** (Home debugger — not a launch toggle). **#128** SPWN+P / Insert **SPAWN** writes `kind:spawn`; surface `name` is `ground` / … — not mesh id `extract`
...
- **Crossover shoulder / left-corner peek** (travel landed #59; tilt path **landed #84**; RH hip bias **landed #94**; one more body-width + ready-hip Y **landed #98**): **H** springs the **viewmodel** across the chest from authored hip +X ~**0.24** to a partial left (~**−0.041** X / ~**−0.181** Y, cap `shoulder_x_min` **−0.055**). ADS keeps **0.32**. Extra left probe `shoulder_viewmodel` **0.12**. Left hold pitch/yaw/roll **0.04 / 0.10 / 0.08** (supersedes #84 chest-cross 0.08/0.32/0.39). Live hip_low is **#99** shotgun low-ready (MP9-Z **0.2403 / −0.3528 / −0.1513** / pitch **0.145**; U-cycle glasses **LOW HIP**; LowHip ADS still irons). Not a body slide, not a mesh mirror / `scale.x = −1`, not infinite travel. Mag chrome travels with the kit. Existing H bind, not a new key. Quiet influence — no franchise name-drop. See `FULCRUMRUST_LAST_PASS_LOCK.md`
...
| Seat | Owns |
|------|------|
| **Augury** | Title + HOLD chrome (#45): thin white mono, tight white frames, white hairline, darker ground. HOLD = **SYSTEM PAUSED** (CE pause language); Resume / Options / Quit to menu. Options list **Graphics / Audio / Gameplay / Controls**. Layout/colors/buttons remain Augury. Logo/title mark already #41. FoW pixellation / floor-warp: **aesthetic note only** — Hypha owns the post dial **#90**. Home occlusion **landed #125** — fills under glyphs; JOIN HOLD is its own title row (`HOLD JOIN`); Options fog/cam/WARP sit above the GRAPHICS hint. Home binds lock **landed #128** — Home cycles off→tabs→off; arrows/Enter; Ins pages; WASD/Space stay move/hop (title / HOLD still read them) |
...
**Crossover shoulder / left-corner peek** (travel landed #59; tilt path **landed #84**; RH hip bias **landed #94**; one more body-width + ready-hip Y **landed #98**): **H** springs the viewmodel across the chest from authored hip +X ~**0.24** to a partial left (~**−0.041** X / ~**−0.181** Y, cap `shoulder_x_min` **−0.055**). ADS keeps **0.32**. Left hold pitch/yaw/roll **0.04 / 0.10 / 0.08** (supersedes #84 chest-cross 0.08/0.32/0.39). Live hip_low is **#99** shotgun low-ready (MP9-Z **0.2403 / −0.3528 / −0.1513** / pitch **0.145**; U-cycle glasses **LOW HIP**; LowHip ADS still irons). Not a body slide. Not a mesh mirror / `scale.x = −1`. Existing H bind, not a new key. Quiet influence — no franchise name-drop. See `FULCRUMRUST_LAST_PASS_LOCK.md`.
